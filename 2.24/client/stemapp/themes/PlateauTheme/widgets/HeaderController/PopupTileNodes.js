...
define([
  ...
  function (
    declare, lang, array, html, on, keys, domConstruct,
    query, _WidgetBase, _TemplatedMixin, ViewStack, swipe, tap, utils) {
    /* global jimuConfig */
    ...
    return declare([_WidgetBase, _TemplatedMixin], {
      ...
      templateString: '<div><div class="close" tabindex="0" data-dojo-attach-point="closeNode"></div>' +
        '<div class="pages" data-dojo-attach-point="pagesNode"></div>' +
      ...
      postCreate: function () {
        ...
        this.pages = [];
        this.createCloseBtn();
        //ESC key press on focus out from more pane, set focus to close button
      ...
      startup: function () {
        ...
        //Change page on swipe end
        on(this.pagesNode, swipe.end, lang.hitch(this, function (e) {
          if (CURRENT_PAGE < this.maximumPages - 1 && e.dx && e.dx < 0) {
            CURRENT_PAGE++;
            this._selectPage(CURRENT_PAGE);
          } else {
            if (e.dx && e.dx > 0 && CURRENT_PAGE > 0) {
              CURRENT_PAGE--;
              this._selectPage(CURRENT_PAGE);
            }
          }
        ...
        this.resize();
        //focus first node once the more pane is open
      ...
      resize: function () {
        ...
        (this.numWidget && this.numWidget.length <= 8)) {
          this.hide();
        }
      },
      ...
      */
      createPages: function (gridParam) {
        ...
        for (p = 0; p < pages; p++) {
          ...
          if (pages > 1) {
            ...
            this.own(on(pointNode, 'keydown', lang.hitch(this, function(p, isLastPage, evt){
              //focus first widget icon
              if(evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE){
                this._onPageNodeClick(p);
                var widgets = query('.page.view', this.viewStack.domNode)[p].children;
                widgets[0].focus();
              }else if(isLastPage && !evt.shiftKey && evt.keyCode === keys.TAB){
                evt.preventDefault();//keep current focus state
              }
            }, p, isLastPage)));
          }
      ...
      _selectPage: function (p) {
        if (this.pages.length > 1) {
          query('.point', this.domNode).removeClass('point-selected jimu-main-background');
          html.addClass(this.pages[p].pointNode, 'point-selected jimu-main-background');
        }
      ...
      createItemNode: function (i, pageNode) {
        ...
        this.own(on(node, tap, lang.hitch(this, function () {
          this.onNodeClicked(node);
        })));

        //Add ENTER key event on node's inside more pane
      ...
      createEmptyItemNode: function (pageNode) {
        ...
        this.own(on(node, 'focus', lang.hitch(this, function (evt) {
          if (!utils.isInNavMode()) {
            return;
          }
          //one page, back to focus first icon in this pageView
          if (this.viewStack.domNode.children.length === 1) {
            evt.target.parentNode.children[0].focus();
          } else {
            var selectedPoint = query('.point-selected', this.pointsNode)[0];
            selectedPoint.previousSibling.focus();
          }
        })));
        this.nodes.push(node);
        return node;
      ...
      createCloseBtn: function() {
        ...
        this.own(on(this.closeNode, 'click', lang.hitch(this, function() {
          this.hide();
        })));
      ...
      openNodebyId: function(id){
        for (var i = 0, len = this.nodes.length; i < len; i++) {
          ...
          if (id && id === html.getAttr(node, "settingId")) {
            this.onNodeClicked(node);
            return node;
          }
        }
