...
define([
  ...
  var clz = dojoDeclare([drawFeedback], {
    ...
    */
            ...
            switch (this._points.length) {
              case 1:
                ...
                if (dojoHas("esri-touch")) {
                  //in case of mobile or touch screen devices
    ...
    _setMajorAxisInfo: function () {
      this.getProjectedGeometry(this._majGraphic.geometry, new esriSpatialReference(4326)).then(
        dojoLang.hitch(this, dojoLang.hitch(this, function (projectedGeometry) {
          this.majorAxisLength = esriGeometryEngine.geodesicLength(projectedGeometry, 9001);
      ...
      var angleDegrees = this.getAngle(
        ...
        esriWebMercatorUtils.webMercatorToGeographic(this._majGraphic.geometry.getPoint(0, 1))
      );
    ...
    */
    ...
    _onDoubleClickHandler: function () {
      if (this._points.length >= 3) {
        ...
        var elipseGeom = new esriPolygon(this._points[0].spatialReference);

        var pointList = [];
        pointList.push(this._majGraphic.geometry.getPoint(0, 0));
        pointList.push(this._majGraphic.geometry.getPoint(0, 1));
        pointList.push(this._minGraphic.geometry.getPoint(0, 1));

        this.geomService.project(pointList, this.map.spatialReference,
          dojoLang.hitch(this, function (projectedGeometries) {

            var centerScreen = this.map.toScreen(projectedGeometries[0]);
            var majorScreen = this.map.toScreen(projectedGeometries[1]);
            var minorScreen = this.map.toScreen(projectedGeometries[2]);
            ...
            elipseGeom.geometry = esriGeometryEngine.rotate(ellipse,
              this.convertAngle(this.orientationAngle), projectedGeometries[0]);
    ...
    disconnectOnMouseMoveHandler: function () {
      if (this._onMouseMoveHandlerConnect) {
