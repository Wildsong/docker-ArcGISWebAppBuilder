    ...
    setLabels: function (useUnclipped) {
      ...
      for (var i = 0; i < rings.length; i++) {
        ...
        for (var j = 0; j < rings[i].length - 1; j++) {
          ...
          if (slope !== prevSlope) {
            var corner = "";
            if (slope === 0 && prevSlope === 90) {
              corner = "lower-right";
            } else if (slope === 270 && prevSlope === 180) {
              corner = "lower-right";
            } else if (slope === 0 && prevSlope === 270) {
              corner = "lower-left";
            } else if (slope === 90 && prevSlope === 180) {
              corner = "lower-left";
            } else if (slope === 90 && prevSlope === 0) {
              corner = "upper-left";
            } else if (slope === 180 && prevSlope === 270) {
              corner = "upper-left";
            } else if (slope === 180 && prevSlope === 90) {
              corner = "upper-right";
            } else if (slope === 270 && prevSlope === 0) {
              corner = "upper-right";
            }
            if (corner) {
      ...
      if (this.minMaxType === "utm") {
        if (lowerLeft.xOffset) {
          lowerLeft.label.xOffset = 70;
        }
        if (upperLeft.xOffset) {
          upperLeft.label.xOffset = 70;
        }
        if (upperRight.xOffset) {
          upperRight.label.xOffset = 70;
        }
        if (lowerRight.xOffset) {
          lowerRight.label.xOffset = 70;
        }

        if (lowerLeft.yOffset) {
          lowerLeft.label.yOffset = 40;
        }
        if (upperLeft.yOffset) {
          upperLeft.label.yOffset = 40;
        }
        if (upperRight.yOffset) {
          upperRight.label.yOffset = 40;
        }
        if (lowerRight.yOffset) {
          lowerRight.label.yOffset = 40;
        }
      }

      var labels = [];
      if (!lowerLeft.hidden && lowerLeft.label) {
        labels.push(lowerLeft.label);
      }
      if (!upperLeft.hidden && upperLeft.label) {
        labels.push(upperLeft.label);
      }
      if (!upperRight.hidden && upperRight.label) {
        labels.push(upperRight.label);
      }
      if (!lowerRight.hidden && lowerRight.label) {
        labels.push(lowerRight.label);
      }
