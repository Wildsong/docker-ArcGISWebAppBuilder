  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin, Evented], {
    ...
    postCreate: function () {
      ...
      this.own(on(window, "resize", lang.hitch(this, function () {
        this._setNewZonePopupDimensions();
      })));
      ...
      //for runtime disable the inputs
      if (this.disableInputs) {
        this.zoneDescriptionTextBox.set("disabled", true);
        this.zoneDistanceTextBox.set("disabled", true);
        if (this.currentRowData && this.currentRowData.distance) {
          this.zoneDistanceTextBox.set("value", this._convertValue(this.currentRowData.distance));
          this.zoneDistanceTextBox.set("title", this._convertValue(this.currentRowData.distance));
        }
      }
    ...
    /**
     * This function is used to convert distance in runtime
     */
    _convertValue: function (dist) {
      var retVal = this.unitAbbr === this.nls.metersAbbr ? dojoNumber.parse(dist) / 3.280839895 :
        dojoNumber.parse(dist);
     ...
     **/
    _setNewZonePopupDimensions: function () {
      if (this.newZonePopup) {
        //If app is running in mobile mode, change the popup dimensions
        if (window.appInfo.isRunInMobile) {
          this.newZonePopup.set("width", window.innerWidth - 100);
          this.newZonePopup.set("height", 450);
        } else {
          //Reset popup dimensions to default
          this.newZonePopup.set("width", 670);
        }
      }
    }
  });
