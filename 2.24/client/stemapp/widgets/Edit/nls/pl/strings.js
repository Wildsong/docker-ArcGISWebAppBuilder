define({
  "_widgetLabel": "Edytuj",
  "_featureAction_Edit": "Edytuj",
  "title": "Wybierz szablon, aby utworzyć obiekty",
  "pressStr": "Naciśnij przycisk ",
  "ctrlStr": " CTRL ",
  "snapStr": " , aby włączyć dociąganie.",
  "close": "Zamknij",
  "featureLayers": "Warstwy obiektów",
  "searchTemplates": "Szablony wyszukiwania",
  "noAvailableTempaltes": "Brak dostępnych szablonów",
  "noCanCreateLayerAreCurrentlyVisible": "Żadne warstwy, które można utworzyć, nie są obecnie widoczne.",
  "layersAreInvisibleOrOutOfScaleRange": "Warstwy są niewidoczne lub poza zakresem skali.",
  "relatedTables": "Powiązane tabele:",
  "relatedRecords": "Powiązane rekordy:",
  "noRelatedRecotds": "Nie znaleziono powiązanych rekordów.",
  "startToDraw": "Kliknij, aby zacząć rysowanie",
  "startToAddPoint": "Kliknij, aby dodać punkt",
  "editRelatedFeature": "Edycja powiązanego obiektu",
  "duplicateValueWarning": " Nie można wstawić zduplikowanej wartości w polu ${fieldName} tabeli ${tableName}. Zduplikowana wartość to: ${value}"
});