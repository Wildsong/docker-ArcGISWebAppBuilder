define({
  ...
  "noOutputParameterWithGeometryType": "선택한 지오프로세싱 서비스에는 지오메트리 유형이 지정된 결과 매개변수가 하나 이상 있어야 합니다. 다른 지오프로세싱 서비스를 선택하세요.",
  "invalidOutputGeometry": "선택한 지오프로세싱 서비스의 결과 지오메트리 유형이 프로젝트 설정과 호환되지 않습니다. 지오프로세싱 서비스의 결과를 저장할 수 없습니다.",
  ...
  "projectSetting": {
    "title": "프로젝트 설정",
    "note": "참고: 프로젝트 설정은 선택 사항입니다. 이 설정을 구성한 경우에는 사용자가 네트워크 중단 영역 및 입력 매개변수와 함께 원하는 웹맵 레이어에 프로젝트를 저장할 수 있습니다. 사용자는 '결과' 탭에서 다른 결과 매개변수를 저장할 수 있습니다.",
    "projectPolygonLayer": "프로젝트 폴리곤 레이어",
    "outputParameterName": "결과 매개변수 이름",
    "projectPointLayer": "프로젝트 포인트 레이어",
    "selectLabel": "선택",
    "polygonLayerHelp": "<p>다음과 같은 조건의 폴리곤 레이어가 표시됩니다.<br/><ul><li>레이어에 '생성', '삭제', '업데이트'라는 편집 기능이 있어야 함</li><li>레이어에 다음과 같은 이름과 데이터 유형이 사용된 2개의 필드가 있어야 함:</li><ul><li>name(문자열 유형 필드)</li><li>globalid(GlobalID 유형 필드)</li></ul></ul><p/>",
    "outputParameterHelp": "<p>작업 URL의 결과 폴리곤 레이어가 표시됩니다.<p/>",
    "pointLayerHelp": "<p>다음과 같은 조건의 포인트 레이어가 표시됩니다. <br/><ul><li>레이어에 '생성', '삭제', '업데이트'라는 편집 기능이 있어야 함</li><li>레이어에 다음과 같은 이름과 데이터 유형이 사용된 2개의 필드가 있어야 함:</li><ul><li>inputtype(문자열 유형 필드)</li><li>projectid(GUID 유형 필드)</li></ul></ul><p/>"
  },
  "inputOutputTab": {
    "flag": "플래그",
    "barrier": "경계",
    ...
    "summaryTextTitle": "입력 탭에 대한 표시 텍스트 제공"
  },
  "summaryTab": {
    "title": "결과",
    ...
    "saveToLayerHelp": "<p>다음과 같은 조건의 레이어가 표시됩니다.<br/><ul><li>레이어에 '생성', '삭제', '업데이트'라는 편집 기능이 있어야 함</li><li>레이어에 다음과 같은 이름과 데이터 유형이 사용된 2개의 필드가 있어야 함:</li><ul><li>parametername(문자열 유형 필드)</li><li>projectid(Guid 유형 필드)</li></ul></ul><p/>",
    "exportToCsvDisplayText": "CSV",
    "summaryTextTitle": "출력 탭에 대한 표시 요약 텍스트 제공",
    "addSummaryItemsTitle": "요약 항목 추가"
  },
  ...
  "hintText": {
    ...
    "inputTextHint": "힌트: 요약 항목 추가 버튼을 사용하여 아래의 식을 작성하세요.",
