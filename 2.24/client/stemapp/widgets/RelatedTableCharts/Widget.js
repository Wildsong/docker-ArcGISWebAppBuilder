...
define([
  ...
  return declare([BaseWidget], {
    ...
    _setFocusOnFirstNode: false,
    openPanels: [], // array to store opened chart panels
    _prevFeature: null,
    _isSearchResultSelected: null,
    _isMapClicked: null,
    postCreate: function () {
      ...
      this._setFocusOnFirstNode = false;
      this.openPanels = [];
      this._prevFeature = null;
      this._isSearchResultSelected = null;
      this._isMapClicked = null;
      LayerInfos.getInstance(this.map, this.map.itemInfo).then(lang.hitch(this, function (layerInfosObj) {
        ...
        if (!this._operationalLayers.length) {
          ...
          domAttr.set(this.errorNode, "aria-label", this.nls.noChatsConfigured);
          this._createWidgetAccessible();
        }
     ...
     */
    onOpen: function () {
      this._setFocusOnFirstNode = true;
    ...
    _createSearchInstance: function () {
      ...
      this._searchInstance.on("select-result", lang.hitch(this, function (evt) {
        ...
        this._clearResults();
        var selectedFeature = this._getSelectedFeatureFromResult(evt);
        this._prevFeature = {
        ...
        var deferred = this._zoomToFeature(); // point, line & polygon is covered in this
        deferred.then(lang.hitch(this, function () {
          this._refreshAllFeatureLayers();
          this._initWorkflow(featureObj);
        }));
      }));
      ...
      this._searchInstance.on("search-loaded", lang.hitch(this, function () {
        setTimeout(lang.hitch(this, function () {
          ...
          this.setTheme();
          // create widget accessible
          this._createWidgetAccessible();
        }), 1000);
      }));
      // once widget is created call its startup method
      this._searchInstance.startup();
      this.own(on(this._searchInstance, 'setFirstFocusNode', lang.hitch(this, function () {
        this._setFocusOnFirstNode = true;
    ...
    _onMapClick: function (evt) {
      this._isMapClicked = true;
      // Starts - keep data available needed for zoom to feature
      this._clearResults();
      ...
      this._selectedLocation = selectedFeature;
      // Ends - keep data available needed for zoom to feature
      var deferred = this._zoomToFeature();
      deferred.then(lang.hitch(this, function () {
        this._refreshAllFeatureLayers();
        if (this.config.generalSettings.showLocationTool) {
          this._disconnectMapEventHandler();
    ...
    _clearResults: function () {
      ...
      // add class to no result found error node
      if (this.errorNode.innerHTML.trim() === this.nls.errMsgNoResultsFound) {
    ...
    _initAccordionPane: function () {
      ...
      on(this.accordionPanel, "chartSelected", lang.hitch(this, function (selectedChartIndex) {
        ...
        if (this.config.charts[selectedChartIndex].selectedFeature) {
          if (this.config.charts[selectedChartIndex].selectedFeature.geometry.type === "point") {
            if (this._isSearchResultSelected) {
              this._isSearchResultSelected = false;
            } else if (this._isMapClicked) {
              this._isMapClicked = false;
            } else {
              this.map.centerAndZoom(this.config.charts[selectedChartIndex].selectedFeature.geometry,
                this.map.getLevel()).then(lang.hitch(this, function () {
                  this._refreshAllFeatureLayers();
                }));
            }
          } else {
            if (this._isSearchResultSelected) {
              ...
              var feature = this.config.charts[selectedChartIndex].selectedFeature;
              this.map.setExtent(feature.geometry.getExtent().expand(1.5)).then(
                lang.hitch(this, function () {
                  this._refreshAllFeatureLayers();
                }));
            }
          ...
          this._highlightGraphicsLayer.clear();
          this._highlightSelectedLocation(this._selectedLocation, false);
          this._highlightSelectedChartFeature(selectedChartIndex);
        }
      }));
      on(this.accordionPanel, "setFocusNodes", lang.hitch(this, function () {
        this._createWidgetAccessible();
      }));
        ...
        lang.hitch(this, function (relRecords) {
          ...
          if (features.length > 0) {
            if (currentObject.chartConfig.chartType !== "PolarChart") {
              if (currentObject.chartConfig.chartType === "LineChart") {
                this._createLineChartData(currentObject, features, isLabelFieldDate, def);
              } else {
    ...
    _createChartData: function (currentObject, features, isLabelFieldDate, def) {
      ...
      array.forEach(features, lang.hitch(this, function (currentFeature, index) {
        var chartLabels = {},
          chartSeries = {},
          labelValue = "",
          addToChartData, tableFieldInfos, fieldDetails,
          displayTime;
        addToChartData = true;
        ...
        if (currentObject.chartConfig.labelField === "esriCTEmptyOption") {
          ...
          chartLabels.sortValue = currentFeature.attributes[currentObject.chartConfig.labelField] || "";
          chartSeries.sortValue = currentFeature.attributes[currentObject.chartConfig.labelField] || "";
        }
    ...
    /**
     * This function is used to create line chart data
     * @param {object} currentObject: current chart settings object
     * @param {array} features: query response
     * @param {boolean} isLabelFieldDate: flag to check label field
     * @param {object} def: deferred object
     * @memberOf widgets/RelatedTableCharts/Widget.js
     */
    _createLineChartData: function (currentObject, features, isLabelFieldDate, def) {
      var chartData, dataSeriesFieldArr, i, dataSeriesFieldAlias;
      chartData = {
        chartSeries: [],
        chartLabels: []
      };
      dataSeriesFieldAlias = this._getDataSeriesAlias(currentObject);
      dataSeriesFieldArr = currentObject.chartConfig.dataSeriesField.split(",");
      // loop through each field
      array.forEach(dataSeriesFieldArr, lang.hitch(this, function (dataSeriesFieldItem, dataSeriesFieldIndex) {
        var chartLabelsArr, chartSeriesArr;
        chartLabelsArr = [];
        chartSeriesArr = [];
        // loop through each feature
        array.forEach(features, lang.hitch(this, function (currentFeature, index) {
          var chartLabels, chartSeries, labelValue, addToChartData, tableFieldInfos, fieldDetails, displayTime;
          chartLabels = {};
          chartSeries = {};
          labelValue = "";
          addToChartData = true;
          chartSeries.y = currentFeature.attributes[dataSeriesFieldItem];
          chartSeries.tooltip = currentFeature.attributes[dataSeriesFieldItem] + "&nbsp;&nbsp;&nbsp;&nbsp;";
          chartSeries.seriesName = dataSeriesFieldAlias[dataSeriesFieldIndex];
          chartLabels = {
            "value": index + 1,
            "text": currentFeature.attributes[dataSeriesFieldItem]
          };
          // if label field is not configured set the value as as empty else set from attributes
          if (currentObject.chartConfig.labelField === "esriCTEmptyOption") {
            labelValue = "";
            ...
            if (currentFeature.attributes[currentObject.chartConfig.labelField] === 0) {
              ...
              if (isLabelFieldDate) {
                ...
                labelValue = this._getLocaleFormatedDate(
                  currentFeature.attributes[currentObject.chartConfig.labelField], displayTime);
              } else {
                labelValue = currentFeature.attributes[currentObject.chartConfig.labelField] || "";
              }
            }
            // set sort value which will be used to sort chart data
            chartLabels.sortValue = currentFeature.attributes[currentObject.chartConfig.labelField] || "";
          ...
          chartLabels.text = chartLabels.text + "";
          if (addToChartData) {
            chartLabelsArr.push(chartLabels);
            chartSeriesArr.push(chartSeries);
          }
        }));
        //Skip the series when thier is not data
        if (chartSeriesArr.length > 0) {
          chartData.chartSeries.push(chartSeriesArr);
        }
        if (chartData.chartLabels.length === 0) {
          chartData.chartLabels.push(chartLabelsArr);
        }
      }));
      if (currentObject.chartConfig.labelField !== "esriCTEmptyOption") {
        // Sort chart data according to text(labels)
        // Sort X-Axis
        chartData.chartLabels[0].sort(this._sortChartData);
        chartData.chartLabels[0].reverse();
        // Sort Y-xis
        array.forEach(chartData.chartSeries, lang.hitch(this, function (chartSeriesObj, index) {
          chartData.chartSeries[index] = chartSeriesObj.sort(this._sortChartData);
          chartData.chartSeries[index].reverse();
        }));
        // As we have sorted chart data the values need to organized again
        for (i = 0; i < chartData.chartLabels[0].length; i++) {
          chartData.chartLabels[0][i].value = i + 1;
        }
      }
      // set the configured color settings for chart
      this._setChartColorSettings(def, chartData, currentObject);
    ...
    _highlightSelectedLocation: function (selectedFeature, updateSelectedLocation) {
      ...
      if (selectedFeature) {
        ...
        symbol = symbolJsonUtils.fromJson(this.config.generalSettings.symbol.graphicLocationSymbol);
    ...
    _highlightSelectedChartFeature: function (selectedIndex) {
      ...
      if (graphic !== '' && graphic !== null && graphic !== undefined) {
    ...
    _autoRefresh: function () {
      ...
      if (this._autoRefreshInterval && this._refreshElapsedTime) {
        ...
        if (this._selectedLocation) {
          if (parseInt(this._getElapsedTimeInMinutes(), 10) >=
            parseInt(this.config.generalSettings.refreshInterval, 10)) {
            this._resetTimer();
    ...
    /**
     * This function is used to handle the click event of the select location button.
     */
    _onSelectLocationClick: function () {
      if (domClass.contains(this.selectLocation, "esriCTSelectLocationActive")) {
        this._disconnectMapEventHandler();
      } else {
        domClass.replace(this.selectLocation, "esriCTSelectLocationActive", "esriCTSelectLocation");
        this._connectMapEventHandler();
      }
    },

    /**
     * This function is to get cofig data series alias from data series values of line chart
     * @param {object} currentObject: current chart settings object
     * @memberOf widgets/RelatedTableCharts/Widget
     */
    _getDataSeriesAlias: function (currentObject) {
      var fields, fieldsFromDefinition, layerFields, fNames, alias, i, len;
      fields = currentObject.chartConfig.dataSeriesField.split(',');
      fieldsFromDefinition = currentObject.layerDetails.relatedLayerInfo &&
        currentObject.layerDetails.relatedLayerInfo.fields &&
        currentObject.layerDetails.relatedLayerInfo.fields.length > 0;
      // to get fields of related layer to extract alias of config data series values
      if (fieldsFromDefinition) {
        layerFields = currentObject.layerDetails.relatedLayerInfo.fields;
    ...
    /**
     * This function is used to create widget accessible
     */
    _createWidgetAccessible: function () {
      // set first focus node
      this._setFirstFocusNode();
      // set last focus node
      this._setLastFocusNode();
      // set the tab event to all the search menu items
      if (this._searchInstance) {
        this._searchInstance.attachTabEventToSearchMenuItems();
      }
    },
    ...
    _setFirstFocusNode: function () {
      var firstNode;
      // 1. set layer selection drop-down as a first focus node
      if (this._hasMulitpleSourcesInSearch) {
        if (this._searchInstance && this._searchInstance.search && this._searchInstance.search.sourcesBtnNode) {
          ...
          // set focus to the first node when widget is opened
          if (this._setFocusOnFirstNode) {
            this._setFocusOnFirstNode = false;
            firstNode = this._searchInstance.search.sourcesBtnNode;
          }
        }
      } else {
        // 2. set layer search input as a first focus node
        if (this._searchInstance && this._searchInstance.search && this._searchInstance.search.inputNode) {
          ...
          if (this._setFocusOnFirstNode) {
            this._setFocusOnFirstNode = false;
            firstNode = this._searchInstance.search.inputNode;
          }
        }
      }
    ...
    /**
     * This function is used to set last focus node
     */
    _setLastFocusNode: function () {
      // 5. set "no result found" as a last focus node
      if (!(domClass.contains(this.errorNode, "esriCTHidden"))) {
        jimuUtils.initLastFocusNode(this.domNode, this.errorNode);
        focusUtil.focus(this.errorNode);
        return;
      }
      // get chart title array
      var chartTitlePanelNodeArr = query(".esriCTChartTitle", this.domNode);
      var chartTitlePanelLastNode = chartTitlePanelNodeArr[chartTitlePanelNodeArr.length - 1];
      // if title last node exists
      if (chartTitlePanelLastNode) {
        // if chart title pane is in open state
        if (domClass.contains(chartTitlePanelLastNode, "esriCTChartSelected")) {
          var chartParentContainerArr = query(".esriCTLayoutContent", chartTitlePanelLastNode.parentNode);
          if (chartParentContainerArr.length > 0) {
            var lastChartParentContainerNode = chartParentContainerArr[chartParentContainerArr.length - 1];
            // set tab event to all the checkbox present in the legend
            var checkBoxWidgetDivArr = query(".dijitCheckBox", lastChartParentContainerNode);
            if (checkBoxWidgetDivArr.length > 0) {
              array.forEach(checkBoxWidgetDivArr, lang.hitch(this, function (checkBoxDiv) {
                var checkBoxWidget = registry.getEnclosingWidget(checkBoxDiv);
                if (checkBoxWidget !== '' && checkBoxWidget !== null && checkBoxWidget !== undefined) {
                  this._attachTabEventToCheckbox(checkBoxWidget);
                }
              }));
            }
            if (checkBoxWidgetDivArr && checkBoxWidgetDivArr.length > 0 &&
              checkBoxWidgetDivArr[0].childNodes && checkBoxWidgetDivArr[0].childNodes.length > 0) {
              var legendFirstCheckBox = checkBoxWidgetDivArr[0].childNodes[0];
              jimuUtils.initLastFocusNode(this.domNode, legendFirstCheckBox);
              return;
            }
            // 4. set chart container as a last focus node
            jimuUtils.initLastFocusNode(this.domNode, lastChartParentContainerNode);
            return;
          }
        }
        // 3. set chart name as a last focus node
        jimuUtils.initLastFocusNode(this.domNode, chartTitlePanelLastNode);
        return;
      }
      // 2. set location as a last focus node if it exists
      if (this.config.generalSettings.showLocationTool) {
        if (this.selectLocation) {
          jimuUtils.initLastFocusNode(this.domNode, this.selectLocation);
          return;
        }
      }
      // 1. set submit node as a last focus node
      if (this._searchInstance && this._searchInstance.search && this._searchInstance.search.submitNode) {
        jimuUtils.initLastFocusNode(this.domNode, this._searchInstance.search.submitNode);
        return;
      }
    },

    /**
     * This function is used to attach tab event to all the checkboxes present in the legend
     */
    _attachTabEventToCheckbox: function (checkbox) {
      this.own(on(checkbox, 'keydown', lang.hitch(this, function (evt) {
    ...
    _initWorkflow: function (evt) {
      ...
      if (evt && evt.feature && evt.feature.geometry) {
        if (evt.feature.geometry.type === "point") {
          this._highlightSelectedLocation(evt.feature, true);
    ...
    _zoomToFeature: function () {
      var extent, zoomScale, featureGeometry, deferred;
      ...
      if (featureGeometry.type === "point") {
        ...
        if (zoomScale && this.map.getScale() > zoomScale) {
          extent = scaleUtils.getExtentForScale(this.map, this._prevFeature.zoomScale).centerAt(featureGeometry);
        } else {
          extent = this.map.extent.centerAt(featureGeometry); // not a deferred
          if (!extent) {
        ...
        extent = featureGeometry.getExtent().expand(1.5); // for polyline & polygon, not a deferred
      }
    ...
    /**
     * This function is used to refresh all the feature layers
     */
    _refreshAllFeatureLayers: function () {
      var featureLayerArr = this._layerInfosObj.getLayerInfoArray();
      array.forEach(featureLayerArr, lang.hitch(this, function (featureLayer) {
        var featureLayerObj = this.map.getLayer(featureLayer.id);
