define({
  "_widgetLabel": "חיפוש גיאוגרפי",
  "description": "עבור למיקום של <a href='./widgets/GeoLookup/data/sample.csv' tooltip='Download an example sheet' target='_blank'> גיליון אלקטרוני </a> או גרור אותו לכאן כדי להציג אותו או להוסיף לו נתונים ממפה.",
  "selectCSV": "בחר קובץ CSV",
  "loadingCSV": "טוען קובץ CSV",
  "savingCSV": "CSVExport",
  "clearResults": "נקה",
  "downloadResults": "הורד",
  "plotOnly": "נקודות תרשים בלבד",
  "plottingRows": "שורות בתרשים",
  "projectionChoice": "CSV ב-",
  "projectionLat": "רוחב/אורך",
  "projectionMap": "היטל מפה",
  "messages": "הודעות",
  "error": {
    "fetchingCSV": "אירעה שגיאה בעת הבאת פריטים ממאגר CSV‏: ${0}",
    "fileIssue": "לא ניתן לעבד את הקובץ.",
    "notCSVFile": "רק קבצים המופרדים בפסיקים (‎.csv) נתמכים כעת.",
    "invalidCoord": "שדות המיקום לא חוקיים. בדוק את קובץ ה-CSV.",
    "tooManyRecords": "מצטערים, ניתן להשתמש ב-${0} רשומות לכל היותר כעת.",
    "CSVNoRecords": "הקובץ לא מכיל שום רשומות.",
    "CSVEmptyFile": "אין תוכן בקובץ."
  },
  "results": {
    "csvLoaded": "נטענו ${0} רשומות מקובץ ה-CSV",
    "recordsPlotted": "אותרו ${0}/${1} רשומות במפה",
    "recordsEnriched": "${0}/${1} עובדו, ${2} הועשרו לעומת ${3}",
    "recordsError": "ב-${0} רשומות נמצאו שגיאות",
    "recordsErrorList": "נמצאה בעיה בשורה ${0}",
    "label": "תוצאות CSV"
  }
});