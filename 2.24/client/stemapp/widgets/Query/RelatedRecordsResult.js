...
define([
  ...
  function(query, keys, Evented, html, lang, array, declare, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin,
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      setResult: function(/*layerName,*/ popupTemplate, featureSet){
        ...
        if(featureSet.features.length > 0){
          //has result
          array.forEach(featureSet.features, lang.hitch(this, function(feature, i){
            if(this.layer){
              this.layer.add(feature);
            }
            this._createItem(popupTemplate, feature, i);
          }));
          var itemTitles = query('.item-title', this.content);
          this.lastFocusItem = itemTitles[itemTitles.length - 1];//default
      ...
      _onNoResultTipKeydown: function(evt){
        if(evt.keyCode === keys.TAB && !evt.shiftKey){
          evt.preventDefault();
          this.emit('focus-result-header');
        }
      },
      ...
      _createItem: function(popupTemplate, feature, i){
        var title = popupTemplate.getTitle(feature) || "";

        var isLastItem = '';
        if(i === this.featureSet.features.length - 1){
          isLastItem = 'data-isLastItem="true"';
        }

        var str = '<div class="item">' +
                    '<div class="item-title" ' + isLastItem + ' role="button" tabindex="0">' + title + '</div>' +
                    '<div class="item-content" ' + isLastItem + ' tabindex="0"></div>' +
                  '</div>';
      ...
      _onContentClicked: function(event){
        ...
        //if last item
        if(html.getAttr(target, 'data-isLastItem') === 'true'){
          this.lastFocusItem = isSelected ? itemTitleDom.nextElementSibling : itemTitleDom;
        }
      },

      _onContentKeydown: function(evt){
        if(evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE){
          this._onContentClicked(evt);
        }else if(evt.keyCode === keys.TAB && !evt.shiftKey &&
          html.getAttr(evt.target, 'data-isLastItem') === 'true'){//last itemTitle(unSelected) or itemContent(selected)
          var isItemSelected = html.hasClass(evt.target.parentNode, 'selected');
          if(!isItemSelected || (isItemSelected && html.hasClass(evt.target, 'item-content'))){
            evt.preventDefault();
            this.emit('focus-result-header');
          }
        }
