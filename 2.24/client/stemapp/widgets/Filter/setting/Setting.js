  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    _getConfigWithValidDataSource: function(config) {
      var validConfig = {
        ...
        taskOper: config.taskOper,
        groupByLayer: config.groupByLayer,
        zoomto: config.zoomto,
        zoombackto: config.zoombackto,
        allowCustom: config.allowCustom,
        allowResetAll: config.allowResetAll,
        allowTurnOffAll: config.allowTurnOffAll,
        filters: []
      };
    ...
    postCreate: function(){
      ...
      this.groupByLayerTip.innerHTML = this.nls.groupByLayerTip || "";
      this.zoomtoTip.innerHTML = this.nls.zoomtoTip || "";
      this.zoombacktoTip.innerHTML = this.nls.zoombacktoTip || "";
      this.allowCustomTip.innerHTML = this.nls.custom || "";
      this.resetAllTip.innerHTML = this.nls.resetAll || "";
      this.turnOffAllTip.innerHTML = this.nls.turnOffAll || "";
      if(this.config){
    ...
    _createSingleSetting: function(target){
      if(this.listContent.childNodes.length >= 2 ){
        html.setStyle(this.matchMsgTask, 'display', 'block');
      }else{
        html.setStyle(this.matchMsgTask, 'display', 'none');
        //default Any when new a filter widget or add more than one filters from one filter.
        this.allAnyTaskSelect.value = 'OR';
      }
    ...
    setConfig: function(config){
      if(config.taskOper && config.filters.length >= 2){
        html.setStyle(this.matchMsgTask, 'display', 'block');
        this.allAnyTaskSelect.value = config.taskOper;
      }
      this.cbxGroupByLayer.setValue(!!config.groupByLayer);
      this.cbxZoomto.setValue(!!config.zoomto);
      this.cbxZoomBackto.setValue(!!config.zoombackto);
      this.cbxAllowCustom.setValue(!!config.allowCustom);
      this.cbxAllowResetAll.setValue(!!config.allowResetAll);
      this.cbxAllowTurnOffAll.setValue(!!config.allowTurnOffAll);
    ...
    getConfig: function(){
      ...
      var config = {
        taskOper: this.allAnyTaskSelect.value,
        groupByLayer: this.cbxGroupByLayer.getValue(),
        zoomto: this.cbxZoomto.getValue(),
        zoombackto: this.cbxZoomBackto.getValue(),
        allowCustom: this.cbxAllowCustom.getValue(),
        allowResetAll: this.cbxAllowResetAll.getValue(),
        allowTurnOffAll: this.cbxAllowTurnOffAll.getValue(),
        filters: []
      };
