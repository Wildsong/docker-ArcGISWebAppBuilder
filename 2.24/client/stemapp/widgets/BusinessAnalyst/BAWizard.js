...
function (array, declare, on, when, Wizard, webMercatorUtils, CreateBuffersPage, ErrorPage, RunReportsPage, SelectPointPolygonPage, GEUtil, GeocodeUtil) {
  ...
  return declare([Wizard], {
    ...
    // drawnPolygonGraphic: Graphic of the hand drawn polygon by the user for the study area
    selectedPoint: {
      tradeAreas: []
    },
    ...
    postCreate: function () {
      ...
      when(GEUtil.checkCapabilities(this.appConfig), function (capabilities) {

        // If user does not have GE permission, display no permissions page
        if (
          !capabilities.hasPrivilege ||
          !capabilities.geServiceConfigured ||
          !capabilities.dirRoutingServiceConfigured
        ) {
          self.pages[ERROR_PAGE] = new ErrorPage({
            nls: self.nls,
            displayErrors: {
              gePrivilege: !capabilities.hasPrivilege,
              geServiceNotConfigured: !capabilities.geServiceConfigured,
              dirRoutingServiceNotConfigured: !capabilities.dirRoutingServiceConfigured
            }
          });
          self.pages[ERROR_PAGE].startup();

          self.loadPage(ERROR_PAGE);
          return;
        }
    ...
    // Trigger a resize of potential images on Select Point/Polygon page
    resize: function() {
      this.pages[SELECT_POINT_POLYGON] && this.pages[SELECT_POINT_POLYGON].resize();
    },
    ...
    clearSelectedPolygon: function () {
      ...
      if (this.drawnPolygonGraphic) {
        this.map.infoWindow.clearFeatures();
        this.map.graphics.remove(this.drawnPolygonGraphic);
        this.drawnPolygonGraphic = null;
      }
    },
    ...
    _featureSelected: function () {
      ...
      // Only continue if different feature from currently selected one
      var currentFeatureGeometry = this.selectedPoint.geometry ? this.selectedPoint.geometry : this.selectedPolygon.geometry;

      if (currentFeatureGeometry === feature.geometry) {
        return;
      }
      ...
      if (feature.geometry.type === "point") {
        ...
        this.selectedPoint.attributes = feature.attributes ? feature.attributes : {};
        ...
        when (GEUtil.getCountryForPolygon(feature.geometry, this.config, this.selectedCountryID), function(result) {
          self.setSelectedCountry(result); // Set country to be used for loading Reports/Infographics
    ...
    _featureUnselected: function () {
      ...
      this.pages["SELECT_POINT_POLYGON"].search && this.pages["SELECT_POINT_POLYGON"].search.clear()
