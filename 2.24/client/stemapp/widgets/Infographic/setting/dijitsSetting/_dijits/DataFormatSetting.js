...
define([
  ...
  function (declare, lang, on, html, array,
    Evented, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, template, Toggle) {
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      postCreate: function () {
        ...
        this.digitSeparator = new Toggle();
        this.digitSeparator.setState(true);
        this.digitSeparator.placeAt(this.digitSeparatorDiv);
        this.own(on(this.digitSeparator, 'change', lang.hitch(this, function (use) {
          if(this.config.digitSeparator === use){
            return;
          }
      ...
      setConfig: function (configObj) {
        ...
        if ("undefined" !== typeof configObj.decimalPlaces) {
          ...
          this.decimalPlacesText.setValue(Math.ceil(this.config.decimalPlaces));//Math.ceil() for old config display
        }
        if ("undefined" !== typeof configObj.digitSeparator) {
          this.config.digitSeparator = configObj.digitSeparator;
          this.digitSeparator.setState(this.config.digitSeparator);
        }
