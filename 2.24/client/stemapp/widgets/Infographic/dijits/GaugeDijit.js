...
define([
  ...
  var clazz = declare([BaseDijit], {
    ...
    _upgradeConfig: function(config) {
      ...
      var cvDataFormat = currentValue && currentValue.dataFormat;
      if (cvDataFormat && typeof cvDataFormat.digitSeparator === 'undefined') {
        cvDataFormat.digitSeparator = true;
      }

      var dataLabels = config && config.display && config.display.dataLabels;
      var dlDataFormat = (dataLabels && dataLabels.dataFormat) || {};
      if (typeof dlDataFormat.digitSeparator === 'undefined') {
        dlDataFormat.digitSeparator = true;
      }
      dataLabels.dataFormat = dlDataFormat;
    },
    ...
    //just use for setting preview
    setMainFeatures: function(features){
      if (features) {
        this._features = utils.cleanFeatures(features);
      }
    },
    ...
    _createChartOption: function() {
      ...
      var format = config.display && config.display.dataLabels && config.display.dataLabels.dataFormat;
      var option = {
        tooltip: {
          confine: true,
          trigger: 'item'
    ...
    _createGaugeOption: function() {
      ...
      if (indicatorInfo.targetValue) {
        ...
        var targetValue = showTargetValue ? indicatorInfo.targetValue : [];
        gaugeOption.targetValue = targetValue.map(function(tv) {
          return {
            value: tv,
            format: format
          };
        ...
        indicators.forEach(function(item) {
          if (item.value) {
            item.value.forEach(lang.hitch(this, function(val) {
              var v = item.isRatio ? splitValueObj.max * (val / 100) : val;
              values.push(this.betterDisplayNumberOfMultiDecimal(v, 2));
            }));
          }
        }.bind(this));
    ...
    betterDisplayNumberOfMultiDecimal: function(n, maxPlaces) {
      if (typeof n !== 'number') {
        return n;
      }
      var decimalPlace = 0;
      ...
      if (decimalPlace) {
        if (decimalPlace > maxPlaces) {
          decimalPlace = maxPlaces;
        }
        var str = n.toFixed(decimalPlace);
