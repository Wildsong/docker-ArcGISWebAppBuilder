...
define(
  ...
  function(
    ...
    return declare([
      ...
      _updateNameText: function (disabled) {
        if (this.tr) {
          var td = query('.normal-text-div', this.tr)[0];
          if (td) {
            td.title = disabled ? this.nls.locatorError : td.textContent;
            domStyle.set(td, 'color', disabled ? 'red' : '');
          }
          this.tr.isDisabled = disabled;
        }
      },
      ...
      isValidConfig: function () {
        ...
        if (config.url && config.name && config.singleLineFieldName &&
          (this.enableSingleField.checked || this.enableMultiField.checked || this.enableXYField.checked) && config.isValidMultiFieldOptions) {
          return true;
        } else {
          return false;
        }
      ...
      getConfig: function() {
        return {
          ...
          type: "locator",
          isValidMultiFieldOptions: this.multiEnabled ? !domClass.contains(this.noFields, "display-block") : true
        };
      },
      ...
      _editFields: function(type) {
        ...
        switch (type) {
          case 'single':
            node = query('.jimu-state-disabled', this.enableSingleField.domNode);
            if (this.singleEnabled && node.length === 0) {
              this._editSingleAddressFieldsTableValues(this.singleAddressFields);
            }
            break;
          case 'multi':
            node = query('.jimu-state-disabled', this.enableMultiField.domNode);
            if (this.multiEnabled && node.length === 0) {
      ...
      _disableSourceItems: function() {
        ...
        var domNodes = [this.enableSingleField.domNode, this.enableMultiField.domNode];
        this._toggleSourceFields(domNodes, ['.checkbox', '.label'], true);

        this._toggleNode(this.editSingleFields, false);
        this._toggleNode(this.editMultiFields, false);
      },

      _toggleSourceFields: function (domNodes, classes, disable) {
        array.forEach(domNodes, function (domNode) {
          array.forEach(classes, function (c) {
            var node = query(c, domNode)[0];
            if (disable) {
              html.addClass(node, 'jimu-state-disabled');
              html.addClass(node, 'pointer-events-none');
            }
          });
        });
      },
