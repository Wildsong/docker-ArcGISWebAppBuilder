...
define(['dojo/_base/declare',
  ...
  function (declare,
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      isDuplicate: false,
      isDuplicateLocated: false,
      isUnMatchedLocated: false,
      isUnMatched: false,
      ...
      postCreate: function () {
        ...
        this.pageHintDiv.innerHTML = this.pageHint;
        this.pageTitleDiv.innerHTML = this.pageTitle;
        domAttr.set(this.featureTitleTable, "aria-label", this.pageTitle + " " + this.pageHint);
        this._initUnMatched();
        this._initToolbar(this.featureToolbar);
        this._updateAltIndexes();
        this.lastFocusNode = this.locationTr;
      },

      _initUnMatched: function () {
        this.isUnMatched = this.label.indexOf('UnMatchedFeatures') !== -1;
        if (this.isUnMatched && this.feature.fieldInfo && this.feature.fieldInfo.length) {
          for (var i = 0; i < this.feature.fieldInfo.length; i++) {
            var field = this.feature.fieldInfo[i];
            //verify that the feature was actually unmatched and not just filtered out based on the min score
            if (field.name === 'matchScore') {
              this.isUnMatchedLocated = field.value !== 0;
            }
          }
        }
      },
      ...
      _updateAltIndexes: function () {
        if (this.pageContainer) {
          var reviewView = this.pageContainer.getViewByTitle('Review');
          if (reviewView) {
            this.altHomeIndex = reviewView.index;
          }
        }
      },
      ...
      startup: function () {
        ...
        this._getFeature().then(lang.hitch(this, function (f) {
          ...
          if (!this.isDuplicate && this._feature && this._feature.geometry) {
            this._toggleEditControls(false);
          }
        }));
      ...
      onShown: function () {
        ...
        if (this.isDuplicate && this._useGeomFromFile && this._duplicateFlag.fromSelect.value === 'make-change') {
          this._toggleEditControls(false);
        }
      },
      ...
      _showView: function () {
        domClass.remove(this.featureTitleTable, 'display-none');
      ...
      _initDuplicateSelect: function () {
        var fromSelect = new Select({
          'aria-label': this.nls.review.item,
          style: {
            display: "table",
            'min-width': "100px",
            height: "28px"
          ...
          options: [{
            ...
            label: this.nls.review.duplicateModify,
            value: 'make-change'
          }, {
            label: this.nls.review.duplicateSave,
            value: 'not-duplicate'
          }],
      ...
      _updateDuplicateUI: function (v) {
        ...
        this._toggleEditControls(v === 'make-change' ? false : true);
        ...
        this._updateLocationPanelLastNode();
      },
      ...
      _initDuplicateReviewRows: function (fields) {
        ...
        domConstruct.create('div', {
          ...
          innerHTML: this.nls.review.target
        }, tdLabel);
        ...
        domConstruct.create('div', {
          className: "duplicate-col-headers main-text float-left",
          innerHTML: this.nls.review.source
        }, _tdLabel);
      ...
      _initRows: function (fields, table) {
        ...
        array.forEach(fields, lang.hitch(this, function (f) {
          if (this._skipFields.indexOf(f.name) === -1) {
            ...
            tr.parent = this;
            var tdLabel = domConstruct.create('td', {
              className: "field-control-td text-left field-row-width pad-left-5"
            }, tr);
            ...
            if (this.isDuplicate) {
              this._initValidationBox(f.label, tr, f.duplicateFieldInfo.value, false, false);
            }
            this._initValidationBox(f.label, tr, f.value, true, false);
            rowIndex += 1;
          }
        }));
        ...
        array.forEach(this.addressFields, lang.hitch(this, function (f) {
          ...
          tr.parent = this;
          var tdLabel = domConstruct.create('td', {
            className: "field-control-td text-left pad-left-5"
          }, tr);
          domConstruct.create('div', {
            className: "main-text float-left",
            innerHTML: f.label
          ...
          this._initValidationBox(f.label, tr, field.value, false, true);
        }));
      },

      _syncAddressInfo: function (evt) {
      ...
      _locate: function (evt) {
        this._featureToolbar._locate(evt);
      },
        ...
        if (!sync) {
          ...
          array.forEach(this.locationControlTable.rows, lang.hitch(this, function (row) {
            ...
            if (row.addressValueTextBox) {
              if (!this.csvStore.useAddr) {
                keyField = this.csvStore.xFieldName === keyField ? 'x' : 'y';
              }
      ...
      _initLabel: function (tr, value, isFile, isAddress) {
        ...
        this._addLocalizeFloatSupport(valueTextBox, value, isAddress, field);
      ...
      _initValidationBox: function (rowLabel, tr, value, isFile, isAddress) {
        var field = this._getField(tr);
        var tdControl = domConstruct.create('td', {
        ...
        if (field && field.type && field.type === 'date') {
          ...
          var hasValue = [null, undefined, ""].indexOf(value) === -1 ? true : false;
          valueTextBox = new DateTextBox({
            ...
            title: !hasValue ? "" : options ?
              jimuUtils.localizeDateByFieldInfo(value, options) : jimuUtils.localizeDate(value),
            onChange: this._onChange
          });
        } else if (field && field.domainValues) {
          //DOMAIN
          //check if the value matches the code or the name
          var getDomainValue = function (value) {
            var names;
            if (field.values.indexOf(value) > -1) {
              names = field.domainValues.filter(function (dv) {
                return dv.label === value;
              });
            }
            return (names && names.length > -1) ? names[0].value : value;
          };

          value = [null, undefined, ""].indexOf(value) > -1 ? '' :
            field.codes.indexOf(value) > -1 ? value : getDomainValue(value);
          var selectOptions = [{
            label: this.nls.review.selectValue,
            value: null,
            selected: true
          }];
          var title = value;
          array.forEach(field.domainValues, function (dv) {
            if (value.toString() === dv.value.toString() || value.toString() === dv.label.toString()) {
              title = dv.label;
            }
            selectOptions.push({
              label: dv.label,
          ...
          tdControl.style.overflow = "hidden";
        } else {
          //TEXT
        ...
        valueTextBox.placeAt(tdControl);
        valueTextBox.startup();
        this.lastValueTextBox = valueTextBox.domNode;
        valueTextBox.isFile = isFile;
        valueTextBox.isAddress = isAddress;
        valueTextBox.row = tr;
        ...
        valueTextBox.on("keyUp", function (v) {
          var valueChanged;
          var changeIndex;
          var newValue = typeof (v.target.value) !== 'undefined' ?
            this.parent._getValue(v.target.value) : this.fieldType === 'date' ? '' : this.value;
          if (this.isAddress) {
      ...
      _addLocalizeFloatSupport: function (valueTextBox, v, isAddress, field) {
          ...
          if ((isAddress && !this.csvStore.useAddr) || (field && field.type && field.type === 'float')) {
            ...
            valueTextBox.format = function (value) {
              if ([null, undefined, ''].indexOf(value) === -1 && !isNaN(value)) {
                ...
                return _v.toString().replace('.', ''); //replace '.' when it's added for formatting like 1.234,9
              } else {
                return value;
              }
            };
            valueTextBox.parse = function (value) {
              if (value && value.toString && value.toString().indexOf(this.decimalSeperator) > -1) {
                return value.toString().replace(this.decimalSeperator, '.');
              } else {
                return value;
              }
            };
      ...
      _onChange: function (v) {
        ...
        if (!isValid) {
          ...
          var rfv = this.parent._getValue(this.row.fileValue);
          var rlv = this.parent._getValue(this.row.layerValue);
          valueChanged = this.isFile ? newValue.toString() !== rfv.toString() : newValue.toString() !== rlv.toString();
          var rows = this.isFile ? this.parent._changedFileAttributeRows : this.parent._changedLayerAttributeRows;
          changeIndex = rows.indexOf(this.row.rowIndex);
          if (changeIndex === -1 && valueChanged) {
            rows.push(this.row.rowIndex);
      ...
      _valuesValidForType: function (v) {
        ...
        switch (this.fieldType) {
          case 'int':
            return /^-?[0-9]+$/.exec(v);
          case 'float':
            return this.decimalSeperator !== '.' ? /^-?[0-9]+[.,]?[0-9]*$/.exec(v) : /^-?[0-9]+[.]?[0-9]*$/.exec(v);
          case 'date':
            if (v) {
              try {
                var d = new Date(v);
      ...
      _initSelectRow: function (useString, table, func, type) {
        ...
        domConstruct.create('div', {
          className: "main-text float-left pad-left-5",
          innerHTML: useString
        }, tdUseLabel);

        this._createSelect(tr, func, type);
      },

      _createSelect: function (tr, func, type) {
        ...
        fromSelect.startup();
        if (type === "geom") {
          this.own(on(tr.fromSelect, "change", lang.hitch(this, function () {
            this._updateLocationPanelLastNode();
          })));
        }
      },
      ...
      _toggleFeature: function (evt) {
        ...
        if (this._featureExpanded) {
          ...
          domAttr.set(this.featureTr, "aria-expanded", "true");
          domAttr.set(this.locationTr, "aria-expanded", "false");
          this.lastFocusNode = this.locationTr;
        } else {
          domAttr.set(this.featureTr, "aria-expanded", "false");
        }
        this._updateInfoRows();
        this.parent.updateFirstLastNode(false);
      },

      _toggleLocation: function (evt) {
        if (evt.type === "keydown" && (evt.keyCode !== keys.ENTER && evt.keyCode !== keys.SPACE)) {
          return;
        }
        ...
        if (this._locationExpanded) {
          ...
          domAttr.set(this.featureTr, "aria-expanded", "false");
          domAttr.set(this.locationTr, "aria-expanded", "true");
        } else {
          domAttr.set(this.locationTr, "aria-expanded", "false");
          this.lastFocusNode = this.locationTr;
        }
      ...
      _updateLocationPanelLastNode: function () {
        var focusableElements, isNodeFound = false;
        if (domClass.contains(this.toggleLocation, "bg-toggle-down-img")) {
          this.lastFocusNode = this.locationTr;
        } else {
          focusableElements = registry.findWidgets(this.locationInformation);
          if (focusableElements && focusableElements.length > 0) {
            //Reverse the elements array, this will help us in getting the last focusable element
            focusableElements.reverse();
            array.some(focusableElements, lang.hitch(this, function (element) {
              if (element && !element.disabled) {
                isNodeFound = true;
                this.lastFocusNode = element.domNode;
                return true;
              }
            }));
          }
        }
        //If node is not found, then set the location row as last focus node
        if (!isNodeFound) {
          this.lastFocusNode = this.locationTr;
        }
        this.parent.updateFirstLastNode(false);
      ...
      updateImageNodes: function () {
        ...
        //up arrows
        removeClass = isDark ? 'bg-toggle-up-img' : 'bg-toggle-up-img-white';
        addClass = isDark ? 'bg-toggle-up-img-white' : 'bg-toggle-up-img';
        imageNodes = query('.' + removeClass, this.domNode);
        array.forEach(imageNodes, function (node) {
          domClass.remove(node, removeClass);
          domClass.add(node, addClass);
        });
        ...
        //down arrows
        removeClass = isDark ? 'bg-toggle-down-img' : 'bg-toggle-down-img-white';
        addClass = isDark ? 'bg-toggle-down-img-white' : 'bg-toggle-down-img';
        imageNodes = query('.' + removeClass, this.domNode);
        array.forEach(imageNodes, function (node) {
          domClass.remove(node, removeClass);
          domClass.add(node, addClass);
      ...
      _toggleLocationControls: function (disabled) {
        ...
        if (_disabled) {
          ...
          this._featureToolbar._enableEdit();
        }
      },
      ...
      resetAttributeValues: function () {
        array.forEach(this.featureControlTable.rows, lang.hitch(this, function (r) {
          if (r.fileValueTextBox) {
            r.fileValueTextBox.set('value', r.fileValue === undefined ? '' : r.fileValue);
          }
