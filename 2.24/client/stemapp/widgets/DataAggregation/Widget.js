  ...
  function (declare,
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _initEditLayerNode: function () {
        ...
        var hasNoLayers = true;
        if (layerSettings && layerSettings.length > 0) {
          this.layerStructure = LayerStructure.getInstance();
          array.some(layerSettings, lang.hitch(this, function (layerSetting) {
            var layerNode, layerInfo;
            if (layerSetting && layerSetting.layerInfo &&
              layerSetting.layerInfo.featureLayer && layerSetting.layerInfo.featureLayer.id) {
              layerInfo = this.operLayerInfos.getLayerInfoById(layerSetting.layerInfo.featureLayer.id);
              layerNode = this.layerStructure.getNodeById(layerSetting.layerInfo.featureLayer.id);
              if (layerNode && layerInfo && layerInfo.layerObject && layerInfo.layerObject.isEditable()) {
                this._configLayerInfo = layerSetting.layerInfo;
                this._symbol = layerSetting.symbol;
                this.editLayerNode = layerNode;
                hasNoLayers = false;
                return true;
              }
            }
        ...
        if (hasNoLayers) {
          this._showPopup([this.nls.warningsAndErrors.noLayersWarning], "");
          return;
        }
      ...
      _updateSources: function (worldLocators) {
        if (worldLocators.length > 0) {
          ...
          }, this.nls.startPage.cannotUseLocator), true);
        }
      },
      ...
      _showPopup: function (messages, title, isFocusRequired) {
        var _this = this, content = domConstruct.create('div', { "tabindex": "0", "role": "region", "style": "outline-offset:-2px" }), ariaLabelString = "";
        ...
        if (ariaLabelString) {
          domAttr.set(content, "aria-label", jimuUtils.stripHTML(ariaLabelString));
        }
        var warningMessage = new Popup({
          ...
          onClose: function () {
            ...
            setTimeout(lang.hitch(this, function () {
              if (isFocusRequired) {
                jimuUtils.focusFirstFocusNode(_this.domNode);
              }
            }), 200);
          }
        });
      ...
      _updateFsFields: function (lyr, infoTemplate) {
        ...
        var layerDef = this.editLayer || this.editLayerDefinition;
      ...
      updateFirstLastNode: function (focusFirstNode) {
        var selectedViewTitle, firstNode, lastNode;
        selectedViewTitle = this._pageContainer.getSelectedTitle();
        view = this._pageContainer.getViewByTitle(selectedViewTitle);

        var homeOrBackAsFirstNode, backButton;
        backButton = this._pageContainer.backImage;
        if (domClass.contains(backButton, "control-disbaled")) {
          homeOrBackAsFirstNode = this._pageContainer.homeImage;
        } else {
          homeOrBackAsFirstNode = this._pageContainer.backImage;
        }
        //for e.g. MatchedFeatures_0
        if (selectedViewTitle.indexOf("_" > -1)) {
          firstNode = homeOrBackAsFirstNode;
          lastNode = view.lastFocusNode;
        }
        switch (selectedViewTitle) {
          case "Home":
            firstNode = view.firstFocusNode;
            lastNode = view.lastFocusNode;
            break;
          case "StartPage":
            firstNode = homeOrBackAsFirstNode;
          ...
          case "FieldMapping":
            ...
            firstNode = this._pageContainer.backImage;
            lastNode = homeView.fieldMappingView.lastFieldSelect;
            break
          case "Addresses":
            firstNode = homeOrBackAsFirstNode;
            lastNode = view.lastFocusNode
          ...
          case "Review":
            firstNode = homeOrBackAsFirstNode;
            lastNode = view.downloadButton
            break;
          case "MatchedFeatures":
            firstNode = homeOrBackAsFirstNode;
            lastNode = view.lastFocusNode;
            break;
      ...
      _initPageContainer: function (validate) {
        ...
        this._initHomeView(validate).then(lang.hitch(this, function (homeView) {
          if (this._pageContainer && !this._pageContainer._destroyed) {
            ...
            this.own(on(this._pageContainer, "view-changed", lang.hitch(this, function () {
              //on view change update first last node and set focus to first node
              this.updateFirstLastNode(true);
            })));
          }

          var startPageView = this._initStartPageView();
          this.own(on(startPageView, "targetLayerChanged", lang.hitch(this, function (selectedTargetLayer) {
            var layerSettings = this.config.layerSettings;
            ...
            for (var i = 0; i < layerSettings.length; i++) {
              if (layerSettings[i].layerInfo.featureLayer.id === selectedTargetLayer) {
                this._configLayerInfo = layerSettings[i].layerInfo;
                this._symbol = layerSettings[i].symbol;
                // this.layerStructure = LayerStructure.getInstance();
            ...
            this._initEditLayerInfo().then(lang.hitch(this, function () {
              if (this._configLayerInfo) {
                //lang.mixin({
                homeView._fsFields = lang.clone(this._fsFields);
                homeView.targetLayerUpdated();
              }
            }));
          })));
      ...
      _getHome: function () {
        if (!this.config.hasOwnProperty("editorDescription") || !this.config.editorDescription) {
          this.config.editorDescription = this.nls.startPage.defaultStartPageInstructions;
        }
        return new Home(lang.mixin({
          pageInstructionText: this.editorXssFilter.sanitize(this.config.editorDescription),
      ...
      validatePrivileges: function () {
        ...
        this.portal.getUser().then(lang.hitch(this, function (user) {
          ...
          if (!this.userCredits && this.portal.isPortal) {
            this.userCredits = 1;
          }
      ...
      erroInCSV: function () {
        var messages = [this.nls.warningsAndErrors.invalidCSV + "</br>"];
        this._showPopup(messages, "");
      }
    });
  });