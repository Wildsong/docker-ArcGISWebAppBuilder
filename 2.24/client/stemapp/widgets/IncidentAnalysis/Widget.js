    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _processOperationalLayers: function() {
        ...
        for (var i = 0; i < this.config.tabs.length; i++) {
          ...
          if (t.layers && t.layers !== "") {
            t.tabLayers = this._getTabLayers(t.layers, false);
            if(t.tabLayers.length === 0) {
              t.tabLayers = this._getTabLayers(t.layers, true);
            }
          }
        }
        //this._getTabLayersVisibility();
      ...
      _createUI: function() {
        ...
        if(this.config.incidentLabel) {
          defTab.label = this.config.incidentLabel;
        }
        if(this.config.locateLabel) {
          this.locate_lbl.innerHTML = utils.sanitizeHTML(this.config.locateLabel ? this.config.locateLabel : '');
        }
        ...
        array.forEach(this.config.tabs, lang.hitch(this, function(tab) {
          array.forEach(tab.tabLayers, lang.hitch(this, function(layer) {
            if (typeof(layer.visible) !== 'undefined') {
              this.tabLayersVisibility[layer.id] = (layer.visible) ? true : false;
            }
          }));
        }));
        array.forEach(this.config.tabs, lang.hitch(this, function(tab) {
      ...
      _getIncidentAddress: function(pt) {
        ...
        this.locator.locationToAddress(pt, 100);
      },
      ...
      _showIncidentAddress: function(evt) {
        if (evt.address.address) {
          ...
          this.div_reversed_address.innerHTML = utils.sanitizeHTML(str_complete_address ? str_complete_address : '');
      ...
      // get tab layers
      _getTabLayers: function(names, useTitle) {
        var lyrs = [];
        array.forEach(this.opLayers._layerInfos, lang.hitch(this, function(layer) {
          ...
          if (layer.newSubLayers.length > 0) {
            this._recurseOpLayers(layer.newSubLayers, lyrs, names, useTitle);
          } else {
            var n = layer.id;
            if (useTitle) {
              n = layer.title;
      ...
      _recurseOpLayers: function(pNode, pLyrs, pNames, useTitle) {
        var nodeGrp = pNode;
