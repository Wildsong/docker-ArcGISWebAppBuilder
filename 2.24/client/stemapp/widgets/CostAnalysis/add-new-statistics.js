...
define([
  ...
  return declare([BaseWidget, Evented, _WidgetsInTemplateMixin], {
    ...
    startup: function () {
      ...
      this.own(on(this.okButtonNode, "keydown", lang.hitch(this, function (evt) {
      ...
      this.own(on(this.cancelButtonNode, "keydown", lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          Event.stop(evt);
          this.emit("showAssetStatisticsPanel");
        }
    ...
    _getUpdatedStatistics: function () {
      ...
      array.forEach(addStatisticsRows, lang.hitch(this, function (rowEntry) {
        if (rowEntry.layerNameDropDown &&
          ...
          rowObj.label = utils.sanitizeHTML(rowEntry.statisticsLabelTextBox.get('value'));
