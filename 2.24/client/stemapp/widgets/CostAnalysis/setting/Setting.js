...
define([
  ...
  return declare([BaseWidgetSetting, Evented], {
            ...
            currentLayerSettings.attributeSetting.push(
              {
                ...
                projectField: projectLayer.globalIdField
              }
            );
    ...
    getConfig: function () {
      var generalSettingsInfo, projectSettingsInfo, costingInfo, projectCost;
      ...
      if (costingInfo.isValid) {
        ...
        this.config.updateCostEquationCheckBoxStatus =
          this._costingInfoTab.getConfig().updateCostEquationCheckBoxStatus;
      } else {
      ...
      // fetch project cost
      projectCost = this._statisticsSettingsTab.additionalProjectCostObj.validateProjectCostData();
      if (projectCost.isValid) {
        this.config.projectCostSettings =
          this._statisticsSettingsTab.additionalProjectCostObj.getConfig();
      } else {
        // display error if there is an error in additional project cost
        this._showMessage(this.nls.projectCostSettings.invalidProjectCostMessage);
        AppUtils.switchToTab(this._configurationTabParentContainer,
          this.nls.statisticsSettings.tabTitle);
        return false;
      }
      return this.config;
    },
    ...
    _initTabs: function () {
      ...
      this.own(on(this._configurationTabParentContainer, "tabChanged", lang.hitch(this,
        function (tabName) {
          ...
          if (tabName === this.nls.costingInfo.tabTitle) {
            var projectLayer = this._projectSettingsTab.projectLayerDropDown.value;
            var projectMultiplierTable = this._projectSettingsTab.projectMultiplierTable.value;
            var projectAssetsTable = this._projectSettingsTab.projectAssetsTable.value;
            //if full config then enable Update project Equation Checkbox
            if (projectLayer !== "" && projectMultiplierTable !== "" && projectAssetsTable !== "") {
              this._costingInfoTab.disableUpdateCostEquationChk(false);
            } else {
              // if minimum config then disable Update project Equation Checkbox
              this._costingInfoTab.disableUpdateCostEquationChk(true);
            }
          }
        })));
    ...
    _initProjectSettings: function () {
      ...
      this._projectSettingsTab.on("onLayerSelected", lang.hitch(this, function (layersInfo) {
        if (this._layerSettingsTab) {
          ...
          this._costingInfoTab._updateCostingInfoTable(layersInfo, true);
          // for updating statistics tab
          this._statisticsSettingsTab._updateStatisticsTable(layersInfo, true);
        }
      }));
      ...
      this._projectSettingsTab.on("onProjectLayerSelected",
        lang.hitch(this, function (layerID) {
          this.projectLayerId = layerID;
          if (this._layerSettingsTab) {
            this._layerSettingsTab.projectLayer = layerID;
            this._layerSettingsTab.updateAttributeButtonState(layerID);
          }
        }));
