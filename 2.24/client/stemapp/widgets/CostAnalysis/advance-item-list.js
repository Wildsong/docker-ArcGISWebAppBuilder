  ...
  return declare([BaseWidget, Evented, _WidgetsInTemplateMixin], {
    ...
    */
    _createImageLogo: function (item, itemContainer, type) {
      ...
      if (type === "icon") {
        ...
        domAttr.set(imageContainer, "tabindex", "0");
        domClass.add(imageContainer, "esriCTWorkBenchDOM");
        domAttr.set(imageContainer, "role", "button");
        domAttr.set(imageContainer, "aria-label", item.actionIconTitle);
    ...
    */
    _createItemTitle: function (item, itemContainer) {
      ...
      if (item.title === this.nls.projectOverview.projectSummaryTitle) {
        domClass.add(itemTitleContainer, "esriCTProjectSummary");
      }
      if (item.title === this.nls.projectOverview.assetItemsTitle) {
        domClass.add(itemTitleContainer, "esriCTAssetItems");
      }
      else {
        domAttr.set(itemTitleContainer, "aria-expanded", "false");
      }

      if (this.fromProjectOverview) {
        domClass.add(itemTitleContainer, "esriCTProjectOverview");
        domClass.add(itemTitleContainer, "esriCTWorkBenchDOM");
      }
      //Item highlighter
      ...
      if (item.actionIcon) {
        ...
        this.own(on(imageContainer, "keydown", lang.hitch(this, function (evt) {
    ...
    */
    _togglePanel: function (node, isManual) {
      var title, panel, arrow, bufferHeight = 0, itemHighlighter, toggleIconArr,
        editEquationIconArr, assetItemTableRowArr, groupSummaryTotalRowArr;
      if (!isManual) {
        bufferHeight = 30;
      }
      ...
      if (title && panel) {
        if (!domClass.contains(panel, "esriCTItemContentActive")) {
          ...
          if (this.showArrow && this.togglePanels) {
            ...
            domAttr.set(title.parentNode, "aria-expanded", "true");
            this._setTabIndexToNodes(toggleIconArr, "0");
            this._setTabIndexToNodes(editEquationIconArr, "0");
            this._setTabIndexToNodes(assetItemTableRowArr, "0");
            this._setTabIndexToNodes(groupSummaryTotalRowArr, "0");
          }
        } else {
          //set the heights to panel
          ...
          if (this.showArrow && this.togglePanels) {
            ...
            domAttr.set(title.parentNode, "aria-expanded", "false");
            this._setTabIndexToNodes(toggleIconArr, "-1");
            this._setTabIndexToNodes(editEquationIconArr, "-1");
            this._setTabIndexToNodes(assetItemTableRowArr, "-1");
            this._setTabIndexToNodes(groupSummaryTotalRowArr, "-1");
          }
        }
    ...
    */
    ...
    /**
     * This function is used to set tabIndex to toggleIcon and
     * editEquation icon in asset details panel
     */
    _setTabIndexToNodes: function (nodesArr, tabIndex) {
      if (nodesArr && nodesArr.length > 0) {
        array.forEach(nodesArr, lang.hitch(this, function (node) {
          domAttr.set(node, "tabindex", tabIndex);
        }));
      }
    }
  });
