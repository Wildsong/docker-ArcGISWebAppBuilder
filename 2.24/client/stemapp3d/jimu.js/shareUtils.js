...
define([
  ...
  function(lang, esriRequest, all,basePortalUrlUtils, Deferred,
    portalUtils, /*Role,*/ jimuUtils, esriUrlUtils) {
    /* global Role*/
    var su = {};

    su.getBaseHrefUrl = function(portalUrl) {
      var webappviewer = "webappviewer3d";//window.appInfo.appType === "HTML3D" ? "webappviewer3d" : "webappviewer";
      var href = "";
      if (window.isXT) {
    ...
    su.getItemByUserAndItemId = function(item, itemUser, user, portalUrl) {
      ...
      if ((item.folderId !== undefined && item.folderId !== null && item.folderId !== '/') ||
        (item.ownerFolder !== undefined && item.ownerFolder !== null && item.ownerFolder !== '/')) {
