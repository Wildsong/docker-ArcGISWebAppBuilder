...
function(declare, lang, array, html, on, query, Deferred, all,
  ...
  var clazz = declare([BaseWidget], {
            ...
            if (layerInfo) {
              ...
              if (!outputParam.label) {
                ...
                if (labelNode && labelNode.length > 0 && layerInfo.title) {
                  labelNode[0].innerHTML = utils.sanitizeHTML(layerInfo.title);
                }
              }
    ...
    _getInputParamValues: function(){
      var retDef = new Deferred(), retValues = {}, defs = [], optionalDefs = [], def, errorMessage = '';

      array.forEach(this.inputNodes, function(node){
        def = node.inputEditor.getGPValue();
        def.param = node.param;
        if (node.param.required) {
          defs.push(def);
        } else {
          optionalDefs.push(def);
        }
      }, this);
      ...
      all(defs).then(lang.hitch(this, function(values){
        ...
        return all(optionalDefs);
      }).then(lang.hitch(this, function(values){
        for(var i = 0; i < values.length; i++){
          retValues[optionalDefs[i].param.name] = values[i];
        }
        retDef.resolve(retValues);
      }), function() {
        // optional parameter does not prevent executing
        retDef.resolve(retValues);
      });
    ...
    _updateOutputNode: function(param, value) {
      ...
      array.some(this.resultNodes, lang.hitch(this, function(resultNode) {
        if ((param.dataType !== 'MapServiceLayer' && resultNode.param.name === param.name) ||
    ...
    _createOutputPlaceholders: function() {
      array.forEach(this.config.outputParams, function(param){
        ...
        if (param.ignore !== true) {
          ...
          var placeholderNode = html.create('div', {
