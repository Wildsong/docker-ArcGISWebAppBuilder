...
function (declare, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin,
  template, lang, html, array, on, query, keys, Graphic, Draw, jsonUtils) {
  return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    postCreate : function() {
      ...
      this.own(items.on('keydown', lang.hitch(this, function(evt) {
      ...
      array.forEach(items, lang.hitch(this, function(item) {
        ...
        if(this.config.hasOwnProperty("drawTools")) {
          if(this.config.drawTools.length > 0) {
            var display = array.indexOf(this.config.drawTools, geoType) >= 0 ? 'block' : 'none';
