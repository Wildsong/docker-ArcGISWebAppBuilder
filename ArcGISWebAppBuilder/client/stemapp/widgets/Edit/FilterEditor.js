...
define([
  ...
  function (declare, lang, array, html, jimuUtils, domConstruct,
    ...
    return declare([_WidgetBase, _TemplatedMixin], {
      ...
      _createLayerFilter: function() {
        ...
        for (var i = 0; i < this._editWidget._layerObjectsParaForTempaltePicker.length; i++) {
          ...
          var option = domConstruct.create("option", {
            ...
            innerHTML: jimuUtils.sanitizeHTML(layerObject.name)
          });
      ...
      update: function() {
        ...
        // clear layerFilter options
        for(i = this.selectDropDown.options.length - 1; i >= 0; i--) {
          if(this.selectDropDown.options[i].value !== "all") {
            this.selectDropDown.remove(i);
          }
        }
        // reset layerFilter options
        for (i = 0; i < this._editWidget._layerObjectsParaForTempaltePicker.length; i++) {
          layerObject = this._editWidget._layerObjectsParaForTempaltePicker[i];
          var option = domConstruct.create("option", {
            value: layerObject.id,
            innerHTML: jimuUtils.sanitizeHTML(layerObject.name)
