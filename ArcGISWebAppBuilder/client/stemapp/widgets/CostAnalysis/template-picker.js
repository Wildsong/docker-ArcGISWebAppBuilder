...
define([
  ...
  return declare([BaseWidget, Evented, _WidgetsInTemplateMixin], {
    ...
    startup: function () {
      ...
      this.own(on(this.sketchButton, "keydown", lang.hitch(this, function (evt) {
      ...
      this.own(on(this.selectButton, "keydown", lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          // if disable, activate select button
          if (!(domClass.contains(this.selectButton, "esriCTSelectEnable"))) {
            this._toolBarActive = false;
    ...
    **/
    ...
    initEditingToolbar: function () {
      ...
      this.own(on(this._editor, 'load', lang.hitch(this, function () {
    ...
    _displayCopyAssetsPopup: function () {
      ...
      var enabledButtons = this._copyFeaturesDialog.enabledButtons;
      this.own(on(enabledButtons[0], "keydown", lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          Event.stop(evt);
          this._copyFeaturesObj.toggleWorkBenchDOMTabindexes("0");
          jimuUtils.initFirstFocusNode(this.widgetDomNode, this.featureModeTextNode);
          jimuUtils.initLastFocusNode(this.widgetDomNode, this.refreshButton);
          focusUtils.focus(this.featureModeTextNode);
        }
      })));
      this.own(on(enabledButtons[1], "keydown", lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          Event.stop(evt);
          this._copyFeaturesObj.toggleWorkBenchDOMTabindexes("-1");
          this._copyFeaturesObj._setFirstAndLastFocusNodes();
        }
      })));
    },
    ...
    */
    ...
    onBeforeEditComplete: function (results) {
      ...
      if (!this._onBeforeEditCompleteResult[results.target.id]) {
        this._onBeforeEditCompleteResult[results.target.id] = [];
      }
      this._onBeforeEditCompleteResult[results.target.id].push(results);
    },
    ...
    _getSucceededFeatures: function (results, layerId) {
      array.forEach(results.adds, lang.hitch(this, function (feature, index) {
        if (!feature.success) {
          this._onBeforeEditCompleteResult[layerId][0].adds.splice(index, 1);
          this._splitAssetInfo.splice(index, 1);
        }
      }));
      array.forEach(results.updates, lang.hitch(this, function (feature, index) {
        if (!feature.success) {
          this._onBeforeEditCompleteResult[layerId][0].updates.splice(index, 1);
          this._splitAssetInfo.splice(index, 1);
        }
      }));
      array.forEach(results.deletes, lang.hitch(this, function (feature, index) {
        if (!feature.success) {
          this._onBeforeEditCompleteResult[layerId][0].deletes.splice(index, 1);
          this._splitAssetInfo.splice(index, 1);
        }
      }));
      return this._onBeforeEditCompleteResult[layerId].shift();
    },
    ...
    _createAndAddFeatures: function (features) {
      var attributes, graphics = [], fieldName;
      if (this._selectedTemplate) {
        //add attributes of the selected template
        attributes = lang.clone(this._selectedTemplate.template.prototype.attributes);
        //Loop through all the default template attributes and if the required attribute is NULL 
        //then get the user entered value from the attribute inspector
        if (this._editor.attributeInspector && this._editor.attributeInspector._currentFeature) {
          for (fieldName in attributes) {
            array.forEach(this._selectedTemplate.featureLayer.fields, lang.hitch(this, function (field) {
              if (field.name === fieldName && !field.nullable &&
                (attributes[fieldName] === null || attributes[fieldName] === undefined || attributes[fieldName] === "")) {
                attributes[fieldName] = this._editor.attributeInspector._currentFeature.attributes[fieldName];
                return true;
              }
            }));
          }
        ...
        array.forEach(features, lang.hitch(this, function (feature, index) {
          ...
          if (this.selectedProjectFeatureAttr) {
            this._updateFeatureAttributes(graphics[index]);
          }
        }));
    ...
    /**
     * This function is used update the attributes based on configuration
     * @memberOf widgets/CostAnalysis/template-picker
     */
    _updateFeatureAttributes: function (asset) {
      var layerAttrSetting, layer, projectLayer, layerFieldLength,
      prjLayerFieldLength, mappedFieldData;
      //get the layers attributes settings
      array.some(this.config.layerSettings, lang.hitch(this,
        function (eachLayerSetting) {
          //Fetch layers attribute settings
          if (eachLayerSetting.attributeSetting &&
            eachLayerSetting.id === this._selectedTemplate.featureLayer.id) {
            layerAttrSetting = eachLayerSetting.attributeSetting;
            return;
          }
        }));
      if (layerAttrSetting) {
        layer = this.map.getLayer(this._selectedTemplate.featureLayer.id);
        projectLayer = this.map.getLayer(this.config.projectSettings.projectLayer);
        ...
        array.forEach(layerAttrSetting, lang.hitch(this, function (attrSetting) {
          ...
          prjLayerFieldLength = projectLayer.getField(attrSetting.projectField).length;
          mappedFieldData = this.selectedProjectFeatureAttr[attrSetting.projectField];
    ...
    _displaySelectScenarioPopup: function () {
      ...
      domAttr.set(this._scenarioSelectionObj.scenarioDropDown, "aria-label",
        this.nls.scenarioSelection.scenarioLabel);
      domAttr.set(this._scenarioSelectionObj.regionNameTextBox, "aria-label",
        this.nls.scenarioSelection.regionLabel);
      this._scenarioSelectionObj.startup();

      var enabledButtons = this._selectScenarioPopup.enabledButtons;
      this.own(on(enabledButtons[0], "keydown", lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          Event.stop(evt);
    ...
    */
    ...
    _createCustomSelectTool: function () {
      ...
      this.own(on(this.customToolbar, 'draw-complete', lang.hitch(this, function (evt) {
        ...
        var layerArray = this._getOtherLayersFromMap();
        this._editor._selectionHelper.selectFeatures(
          ...
          lang.hitch(this, function () {
            ...
            if (selectedFeatures.length > 0) {
              ...
              showMessageDialog = this.appUtils.showMessage(this.nls.copyFeatures.noFeaturesSelectedMessage);
            }
          }));
        if (showMessageDialog && showMessageDialog.enabledButtons) {
          this.own(on(showMessageDialog.enabledButtons[0], "keydown", lang.hitch(this, function (evt) {
            if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
              Event.stop(evt);
              focusUtils.focus(this.selectButton);
            }
    ...
    _deactivateAllTools: function () {
      this.customToolbar.deactivate();
      if (this._editor && this._editor._drawToolbar) {
        this._editor._drawToolbar.deactivate();
      }
    },

    /**
     * This function is used to get the other layers from map
     * which may be added by a different widget
     */
    _getOtherLayersFromMap: function () {
      var filteredFeatureLayerArr;
      filteredFeatureLayerArr = [];
      var allMapLayersArr = this.layerInfosObj.getLayerInfoArray();
      array.forEach(allMapLayersArr, lang.hitch(this, function (layerObj) {
        var layerObjDetail = this.map.getLayer(layerObj.id);
        if ((layerObjDetail.type === "Feature Layer")) { // 1st filter
          var layerPresentInTemplatePicker = false;
          array.forEach(this._editor._settings.layers, lang.hitch(this, function (layer) {
            if (layer.id === layerObjDetail.id) {
              layerPresentInTemplatePicker = true;
            }
          }));
          //layers which are in templatePicker should be nonEditable
          //if layer is not present in templatePicker it means it is added from other widget
          if (layerPresentInTemplatePicker && this._nonEditableLayers.indexOf(layerObjDetail.id) > -1) {
            layerPresentInTemplatePicker = false;
          }
          if (!layerPresentInTemplatePicker) { // 2nd filter
            // 3rd filter
            if (this._selectedTemplate.featureLayer.geometryType === layerObjDetail.geometryType) {
              //4th filter
              if (layerObjDetail.id !== this.config.projectSettings.costingGeometryLayer &&
                layerObjDetail.id !== this.config.projectSettings.projectLayer) {
                filteredFeatureLayerArr.push(layerObjDetail);
              }
            }
          }
        }
      }));
      return filteredFeatureLayerArr;
    }
  });
});