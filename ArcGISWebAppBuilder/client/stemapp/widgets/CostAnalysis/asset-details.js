  ...
  return declare([BaseWidget, Evented, _WidgetsInTemplateMixin], {
    ...
    */
    showAssetDetails: function (assetDetails, fromDeleteAsset) {
      var index, hasAnyDetails = false, expandCollapseAriaLabel;
            ...
            for (var regionName in assetDetails[layerId][templateName]) {
              for (var scenario in assetDetails[layerId][templateName][regionName]) {
                ...
                if (features.length > 0) {
                  ...
                  expandCollapseAriaLabel = string.substitute(
                    this.nls.statisticsType.expandCollapseAriaLabel,
                    { displayTitle: displayTitle , featureCount: features.length});
                  this._createExpandCollapseButton(titleParent, groupId, layerId, expandCollapseAriaLabel);
                  //Create div to show group title
                  domConstruct.create("div", {
                    "class": "esriCTGroupTitle",
                    "innerHTML": displayTitle + "(" + features.length + ")"
                  }, titleParent);
