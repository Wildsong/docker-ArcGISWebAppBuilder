...
define([
  ...
  function(declare, BaseWidget, Directions, Locator, RouteParameters, esriRequest, graphicsUtils, LayerInfos, queryUtil,
    ArcGISDynamicMapServiceLayer, on, lang, html, array, dojoConfig, Deferred, esriLang,
     all, when, portalUtils, jimuUtils, FeatureLayer, ActiveStateManager, Message,  LoadingIndicator, a11y, searchUtils) {
    ...
    var clazz = declare([BaseWidget], {
      ...
      _isInitDirectionFlag: false,
      _isInitPresetStopsFlag: false,//for preset stops
      ...
      postCreate: function() {
        ...
        this.activeStateManager = new ActiveStateManager({widget: this});
      },
      ...
      _hide: function(){
        if (this._dijitDirections) {
          this.activeStateManager.cacheStates(/*{ isKeepMapClick: true }*/);
          this.activeStateManager.deactivateDijit();
        }
      },
      _show: function(){
        if (this._dijitDirections) {
          this.activeStateManager.revertToLastState();
        }
      },
        ...
        this._getLayerInfos().then(lang.hitch(this, function () {
          ...
          if (queryUtil.havePresetBarrierLayers(this.config)) {
            ...
            this.own(on(this.layerInfosObj, 'layerInfosFilterChanged', lang.hitch(this, function (layers) {
              this._updateBarriersByConfig(layers);
            })));
          }
          ...
          all(defs).then(lang.hitch(this, function (results) {
            ...
            this.setDoNotFetchTravelModes(options).then(lang.hitch(this, function () {
              ...
              esriRequest({
                ...
                this.activeStateManager.setDirectionDijit(this._dijitDirections);

                this.own(on(this._dijitDirections, 'load', lang.hitch(this, this._onDirectionsLoadedOrCleared)));
                ...
                this.own(on(this._dijitDirections,'error',lang.hitch(this, this._handleError)));
      ...
      _onDirectionsStart: function(){
        this._directionStartFlag = true;
        //this.cacheStates();//for restore state ,#16376
        //this._updateBarriersByConfig();
      },

      _onDirectionsFinish: function(evt){
        var isError = evt&&evt.result&&evt.result.stack&&(evt.result.stack.indexOf("Error") >-1);

        if(evt && evt.result && !isError){
          ...
          if(lang.isArrayLike(routeResults) && routeResults.length > 0){
            ...
            if (this._directionStartFlag && queryUtil.havePresetBarrierLayers(this.config)) {
        ...
        this._directionStartFlag = false;
        this.a11y_updateFocusNodes();
      },
      ...
      _handleError: function (error) {
        console.error("Directions Error:" + error.error);
        if (error.error && error.error.indexOf("The number of input locations loaded") > -1) {
          if (error.target.set) {
            error.target.set("canModifyStops", false);
          }
        }
      },
      ...
      _preProcessConfig:function(){
        ...
        if(this._IS_CONFIG_BEFORE_63){
          ...
          when(searchUtils.getConfigInfo(this.config.searchOptions)).then(lang.hitch(this, function(config) {
            ...
            this.config.routeTaskUrl = this._replaceRouteTaskUrlWithAppProxy(this.config.routeTaskUrl);//for appProxies ,#17739
      ...
      _onDirectionsLoadedOrCleared: function () {
        if (this.config.defaultLocations &&
          ...
          (this.config.defaultLocations[0] || this.config.defaultLocations[1]) &&
          false === this._isInitPresetStopsFlag) {

          this._isInitPresetStopsFlag = true;

          //async get wayPoints depends on map'cood
          searchUtils.config.getWayPoints(this.config, this._IS_CONFIG_BEFORE_63, this.map).then(lang.hitch(this, function (wayPoints) {
        ...
        if (!this._isInitDirectionFlag) {//for init state, #17326
          this.activeStateManager.setMapClickActiveImmediately(this.activeStateManager.mapClickActive);
          this.activeStateManager.setMapClickActive(this.activeStateManager.mapClickActive);
        }
        this.activeStateManager.cacheStates({ mapClickActive: this.activeStateManager.mapClickActive });

        this._isInitDirectionFlag = true;
      },
      ...
      openRoute: function (feature) {
        this.getDirectionsDijit().then(lang.hitch(this, function (directionsDijit) {
          ...
          directionsDijit.reset().then(lang.hitch(this, function () {
            this.activeStateManager.cacheStates({mapClickActive: this._activeBeforReset});
      ...
      //Barriers Layers
      _updateBarriersByConfig: function (layers) {
        var needUpdate = false;
        if (layers) {
          needUpdate = queryUtil.findBarrierLayer(layers, this.config);
        } else {
          needUpdate = queryUtil.havePresetBarrierLayers(this.config);/*&& !this._updateBarriersFlag*/
        }
      ...
      _queryAndUpdateBarriersFeatureSet: function () {
        ...
        all(barriersDefs).then(lang.hitch(this, function(results){
          ...
          if(null !== this._activeBeforReset && "undefined" !== typeof this._activeBeforReset){//for ,#16376
            this.activeStateManager.cacheStates({mapClickActive: this._activeBeforReset});
      ...
      _updateBarriersFeatureSet: function () {
        ...
        // }
        if (this._barriersFeatureSet.point &&
          this._barriersFeatureSet.point.features && this._barriersFeatureSet.point.features.length > 0) {

          this._updateBarriersFlag = true;
          this._dijitDirections.setBarriers(this._barriersFeatureSet.point);
        }
        if (this._barriersFeatureSet.polyline &&
          this._barriersFeatureSet.polyline.features && this._barriersFeatureSet.polyline.features.length > 0) {

          this._updateBarriersFlag = true;
          this._dijitDirections.setPolylineBarriers(this._barriersFeatureSet.polyline);
        }
        if (this._barriersFeatureSet.polygon &&
          this._barriersFeatureSet.polygon.features && this._barriersFeatureSet.polygon.features.length > 0) {
