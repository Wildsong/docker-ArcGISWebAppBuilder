    ...
    return declare([BaseWidgetSetting, Evented, _TemplatedMixin], {
      ...
      _initControls: function () {
        ...
        this.own(on(this.dataTypeDropdown, "change", lang.hitch(this, function (selectedDataType) {
          ...
          if (this.chooseFromLayerInfo) {
            this.chooseFromLayerInfo = null;
          }
        ...
        if (node) {
          ...
          this.own(on(this.presetValueDijitNode.domNode, mouse.enter, lang.hitch(this, function () {
            if (this._selectedRelativeDate &&
              this.dataTypeDropdown.get("value") === "esriFieldTypeDate" && !this._checkIfDomainFieldSelected()) {
              ...
              //when domain fields is selected if preset value is blank then show no date defined on hover
              //else show preset value on hover
              if (this.presetValueDijitNode.get("value") !== "") {
                this.presetValueDijitNode.set("title", this.presetValueDijitNode.get("value"));
              } else {
                this.presetValueDijitNode.set("title", this.nls.relativeDates.noDateDefinedTooltip);
              }
            }
          })));
        ...
        this.presetPopup = new Popup({
          ...
          "buttons": [{
            ...
            onClick: lang.hitch(this, function () {
              ...
              if(this.chooseFromLayerInfo){
                groupInfo.chooseFromLayerInfo = this.chooseFromLayerInfo;
              }
              //store applied on fields object
              groupInfo.appliedOn = this._layerAndFieldsApplyOnObj.getCheckedFields(groupInfo);
              //If no field is selected, show error message
              if (!this._hasFieldsInPresetGroup(groupInfo)) {
                Message({
      ...
      _onSelectPresetValueButtonClick: function () {
        ...
        if (isDomainFieldSelected) {
          ...
          this._relativeDatesObj = new RelativeDates({
            ...
            map: this.map,
            layerInfos: this.layerInfos,
            selectedPresetValue: this._selectedRelativeDate && this._selectedRelativeDate.chooseFromLayerInfo ?
              (this._selectedRelativeDate.chooseFromLayerInfo.selectedValue) : ""
          });
          ...
          this.ChooseFromLayerObj = new ChooseFromLayer({
            ...
            dataType: this.dataTypeDropdown.get("value"),
            layerInfos: this.layerInfos,
            chooseFromLayerInfo: this.chooseFromLayerInfo ? this.chooseFromLayerInfo: null,
            selectedPresetValue: this.chooseFromLayerInfo ? this.chooseFromLayerInfo.selectedValue : ""
          });
          on(this.ChooseFromLayerObj, "updatePresetValue", lang.hitch(this, function (chooseFromLayerInfo) {
            var selectedValue = chooseFromLayerInfo.selectedValue;
            //If the string data type is selected and the selected
            var field = this.layerInfos.getLayerOrTableInfoById(
              chooseFromLayerInfo.layerId).layerObject.getField(chooseFromLayerInfo.field);
            if (field.domain && this.dataTypeDropdown.get("value") === "esriFieldTypeString") {
              array.some(field.domain.codedValues, lang.hitch(this, function (codedDomain) {
                //to avoid type mismatch convert domain coded value to string
                if (codedDomain.code.toString() === chooseFromLayerInfo.selectedValue.toString()) {
                  selectedValue = codedDomain.name;
                }
              }));
            }
            this.chooseFromLayerInfo = chooseFromLayerInfo;
            this.presetValueDijitNode.set("value", selectedValue);
          }));
          on(this.ChooseFromLayerObj, "cancelButtonClicked", lang.hitch(this, function () {
          }));
        }
      },
