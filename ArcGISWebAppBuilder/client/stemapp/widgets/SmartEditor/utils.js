...
define([
  ...
  //added code to fix gitHub ticket #268
  mo.updatePopupDateFormat = function (configuredFormat) {
    var popupDateFormats = {
      "shortDate": { datePattern: "M/d/y" },
      "shortDateLE": { datePattern: "d/M/y" },
      "longMonthDayYear": { datePattern: "MMMM d, y" },
      "dayShortMonthYear": { datePattern: "d MMM y" },
      "longDate": { datePattern: "EEEE, MMMM d, y", selector: "date" },
      "shortDateShortTime": { datePattern: "M/d/y", timePattern: "h:mm a", timeEnabled: true },
      "shortDateLEShortTime": { datePattern: "d/M/y", timePattern: "h:mm a", timeEnabled: true },
      "shortDateShortTime24": { datePattern: "M/d/y", timePattern: "H:mm", timeEnabled: true },
      "shortDateLEShortTime24": { datePattern: "d/M/y", timePattern: "H:mm", timeEnabled: true },
      "shortDateLongTime": { datePattern: "M/d/y", timePattern: "h:mm:ss a", timeEnabled: true },
      "shortDateLELongTime": { datePattern: "d/M/y", timePattern: "h:mm:ss a", timeEnabled: true },
      "shortDateLongTime24": { datePattern: "M/d/y", timePattern: "H:mm:ss", timeEnabled: true },
      "shortDateLELongTime24": { datePattern: "d/M/y", timePattern: "H:mm:ss", timeEnabled: true },
      "longMonthYear": { datePattern: "MMMM y" },
      "shortMonthYear": { datePattern: "MMM y" },
      "year": { datePattern: "y" }
    };

    var selectedDatePattern = "";
    for (var datePattern in popupDateFormats) {
      if (jimuUtils.isEqual(configuredFormat, popupDateFormats[datePattern])) {
        selectedDatePattern = datePattern;
        break;
      }
    }
    return selectedDatePattern;
  };
      ...
      array.forEach(popupInfo.fieldInfos, function (fieldInfo) {
        array.some(layerInfo.layerObject.fields, function (field) {
          if (field.name === fieldInfo.fieldName) {
            ...
            //added code to fix gitHub ticket #268
            if (mergedFieldInfo.format && mergedFieldInfo.format.dateFormat &&
              (mergedFieldInfo.format.dateFormat.hasOwnProperty("datePattern") ||
                mergedFieldInfo.format.dateFormat.hasOwnProperty("timePattern"))) {
              mergedFieldInfo.format.dateFormat = this.updatePopupDateFormat(mergedFieldInfo.format.dateFormat);
            }
    ...
    if (layerInfo && layerInfo.layerObject) {
      array.forEach(layerInfo.layerObject.fields, function (field) {
        ...
        if (fieldInfo.format && fieldInfo.format.dateFormat &&
          (fieldInfo.format.dateFormat.hasOwnProperty("datePattern") ||
            fieldInfo.format.dateFormat.hasOwnProperty("timePattern"))) {
          fieldInfo.format.dateFormat = this.updatePopupDateFormat(fieldInfo.format.dateFormat);
        }
  ...
  mo.createDefaultConfigInfo = function (layerInfo) {
    ...
    //Adding this code to resolve the issue #257
    //If layer does not allow updates then
    //disable geometry editing checkbox should be checked and disable
    allowGeometryUpdates = allowGeometryUpdates && allowsUpdate;
    var configInfo = {
      'featureLayer': {
  ...
  mo.isDuplicateGroupName = function (groupName, existingGroups) {
    var isDuplicateName = false;
    array.some(existingGroups, lang.hitch(this, function (currentGroupName) {
      //If same group name is found in the same smart/attribute action
      //break the loop and send the flag value
      if (groupName.toLowerCase() === currentGroupName.toLowerCase()) {
