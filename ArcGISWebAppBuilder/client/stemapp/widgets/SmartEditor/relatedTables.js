  ...
  return declare([_WidgetBase, Evented], {
    ...
    */
    _createTable: function () {
      ...
      array.forEach(this.relationshipInfo, lang.hitch(this, function (relationship, index) {
        if (relationship.featureLayer && relationship.hasOwnProperty('relationshipId')) {
          this._createRelatedTableItem(relationship, contentWrapper, index);
        }
      }));
    ...
    /**
     * This function will retrun the primarykey field available in attributes irrespetive of case.
     */
    _getKeyField: function (primaryKeyField) {
      var updatedPrimaryKey, lowerCaseKeyField, attr;
      updatedPrimaryKey = primaryKeyField;
      lowerCaseKeyField = primaryKeyField.toLowerCase();
      //loop through all the attributes and check if its text matches with the keyfeild
      //check both the strings in lowercase to check regardless of case.
      //Irrespective of CamelCase, Uppercase, Lowercase, it will check both the string.
      for (attr in this.parentFeature.attributes) {
        if (lowerCaseKeyField === attr.toLowerCase()) {
          updatedPrimaryKey = attr;
          break;
        }
      }
      return updatedPrimaryKey;
    },

    /**
    * The function will create individual item
    */
    _createRelatedTableItem: function (relationship, contentWrapper, index) {
      var titleContainer, tableTitle, createNewFeature, tableInfo, createNewBtnLabel,
        relationShipInfo = {};
      ...
      tableTitle = domConstruct.create("div", {
        "class": "relatedTableTitle esriCTEllipsis relatedTableFields",
        "innerHTML": tableInfo.title,
        "tabindex":"-1",
        "title": tableInfo.title,
        "aria-label": tableInfo.title,
        "role": "button"
      }, titleContainer);
      on(tableTitle, "keydown", lang.hitch(this, function (evt) {
      ...
      if (this._canAddFeatures(relationship, tableInfo) && tableInfo.layerObject.type === "Table") {
        createNewBtnLabel = string.substitute(this.nls.createNewFeatureLabel, {
          layerTitle : tableInfo.title
        });
        //create add new feature button based on layer/table config and capabilities
        createNewFeature = domConstruct.create("div", {
          "class": "relatedTableIcons itemTitleCreateNew relatedTableFields",
          "title": this.nls.addNewFeature,
          "aria-label": createNewBtnLabel,
          "role": "button",
          "tabindex": "-1"
        }, titleContainer);
        ...
        on(createNewFeature, "click", lang.hitch(this, function (evt) {
          this._createNewButtonClick(evt, relationship, tableInfo, titleContainer);
        }));
        on(createNewFeature, "keydown", lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            this._createNewButtonClick(evt, relationship, tableInfo, titleContainer);
          }
        }));
        //Since create new feature button is enabled
        //Check if the related record item is the last in the table and add a last item class
        if(this.relationshipInfo.length - 1 === index) {
          domClass.add(createNewFeature, "esriCTLastRelatedItem");
        }
      } else {
        //Since create new feature button is disabled
        //Check if the related record item is the last in the table and add a last item class
        if(this.relationshipInfo.length - 1 === index) {
          domClass.add(tableTitle, "esriCTLastRelatedItem");
        }
      }
      if (relationShipInfo.isValidRelation) {
    ...
    _createNewButtonClick: function (evt, relationship, tableInfo, titleContainer) {
      var currentFeature;
      evt.stopPropagation();
      ...
      //Fix gitHub Ticket #183
      //If the primary key field attribute is not available,
      //check if case insensetive atttribute is available and update the primary key field
      if (!this.parentFeature.attributes.hasOwnProperty(currentFeature.primaryKeyField)) {
        currentFeature.primaryKeyField = this._getKeyField(currentFeature.primaryKeyField);
      }
    ...
    */
    ...
    displayRelatedRecordCount: function (relatedRecordCount, relationshipid) {
      var countNode, countParent, relatedTableTitleNode, newWidth,
        createNewIcons, loadingIconNode, featureCountLabel;
      ...
      featureCountLabel = string.substitute(this.nls.relatedFeatureCount, {
        layerTitle:relatedTableTitleNode.innerHTML,
        featureCount: relatedRecordCount
        });
      domAttr.set(relatedTableTitleNode, "aria-label", featureCountLabel);
