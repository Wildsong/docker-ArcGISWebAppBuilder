...
define([
  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    _footNoteEditorObj: null, // to store editor object
    ...
    postCreate: function () {
      ...
      this._pageSize = [{
        ...
        {
          "value": "MILLIMETER",
          "label": this.nls.downloadTab.pageSizeUnits.millimeters
        },
        {
          "value": "INCH",
          "label": this.nls.units.inches
        },
        {
          "value": "POINT",
          "label": this.nls.downloadTab.pageSizeUnits.points
        },
        {
          "value": "CENTIMETER",
          "label": this.nls.units.centimeters
        }
      ];
      ...
      if (this.downloadOptions && this.downloadOptions.layers) {
        ...
        array.forEach(this.downloadOptions.layers, lang.hitch(
          this,
          function (layer) {
            var layerUrl, requestHandle;
            layerUrl = layer.url;
            requestHandle = esriRequest({
              "url": layerUrl + "?f=pjson"
            });
            deferredArr.push(requestHandle);
          }));
        var defList = new DeferredList(deferredArr);
        defList.then(lang.hitch(this, function (defResults) {
          array.forEach(defResults, lang.hitch(this, function (defResult, index) {
            var updatedLayerObj;
            if (defResult[0]) {
              updatedLayerObj = {
                version: defResult[1].currentVersion,
                capabilities: defResult[1].capabilities
              };
              if (this.downloadOptions.layers[index]) {
                this._setFileOptionDataInTable(this.downloadOptions.layers[index],
                  updatedLayerObj);
              }
            } else {
              if (defResult[1].name && defResult[1].name === "Error") {
                return;
              }
              var layerInfo;
              layerInfo = this.layerInfosObj.getLayerInfoById(
                this.downloadOptions.layers[index].id);
              updatedLayerObj = {
                version: layerInfo.layerObject.version,
                capabilities: ""
              };
              this._setFileOptionDataInTable(this.downloadOptions.layers[index],
                updatedLayerObj);
      ...
      if (this.reportSettings && this.reportSettings.footnote &&
        ...
        this._footNoteEditorObj.set("value", this.reportSettings.footnote);
      }
      args = {
      ...
      // Set no data text representation to config value
      if (this.reportSettings.hasOwnProperty("noDataConfiguredText")) {
        this.noDataTextRepresentationTextBox.set('value', this.reportSettings.noDataConfiguredText);
      }
      // Set N/A text representation to config value
      if (this.reportSettings.hasOwnProperty("naTextConfiguredValue")) {
        this.naTextRepresentationTextBox.set('value', this.reportSettings.naTextConfiguredValue);
      }
      // Set display summary checkbox status
      if (this.reportSettings.hasOwnProperty("displaySummaryInReport")) {
        this.displaySummaryInReportCheckBox.setValue(this.reportSettings.displaySummaryInReport);
      }
      // Set checkbox status of hide rows having all values zero
      if (this.reportSettings.hasOwnProperty("hideZeroValueRow")) {
        this.hideAllZeroRowCheckBox.setValue(this.reportSettings.hideZeroValueRow);
      }
      // Set checkbox status of hide rows having all values NULL
      if (this.reportSettings.hasOwnProperty("hideNullValueRow")) {
        this.hideNullRowCheckBox.setValue(this.reportSettings.hideNullValueRow);
      }
     ...
     * @memberOf Screening/setting/Download
     */
    _setFileOptionDataInTable: function (layer, updatedLayerInfo) {
      var tuple, layerItem;
      tuple = this._syncEnableOptionTable.addRow({});
      if (!tuple.success || !tuple.tr) {
        return;
      }
      // code added to read capabilities options from map
      layerItem = {
        "url": layer.url,
        "id": layer.id,
        "layerName": layer.layerName,
        "allowDownload": layer.allowDownload,
        "layerVersion": updatedLayerInfo.version,
        "capabilities": updatedLayerInfo.capabilities
      };
      this.checkLayerForDownloadOptions(layerItem, tuple.tr.rowIndex);
    },
    ...
    checkLayerForDownloadOptions: function (layer, index) {
      ...
      layerInformation = {
        ...
        "layerName": layer.layerName,
        "id": layer.id,
        "allowDownload": (layer.hasOwnProperty("allowDownload")) ? layer.allowDownload : true
      };
    ...
    /**
     * Function to get no data text representation
     * @memberOf Screening/setting/Download
     */
    getNoDataText: function () {
      return this.noDataTextRepresentationTextBox.value;
    },

    /**
     * Function to get N/A text representation
     * @memberOf Screening/setting/Download
     */
    getNaTextValue: function () {
      return this.naTextRepresentationTextBox.value;
    },
    ...
    getFootnoteForReport: function () {
      return this._footNoteEditorObj.focusNode.innerHTML ?
        this._footNoteEditorObj.focusNode.innerHTML : "";
    },
    ...
    _createLogoPreview: function () {
      ...
      if (this.reportSettings && this.reportSettings.logo) {
        ...
        if (imageInfo.indexOf("${appPath}") > -1) {
          ...
          imageSrc = string.substitute(imageInfo, {
            appPath: baseURL
          });
        } else {
          imageSrc = imageInfo;
    ...
    _createCustomLayoutTable: function () { // 6
      ...
      html.setStyle(this._customLayoutTable.domNode, {
        'height': '100%'
      });
    ...
    _addCustomOptionsInTable: function () { // 7
      var choiceListOption = this._customDropdownOption[0].value,
        choiceListExists;
    ...
    _getDuplicateValue: function (arrayToTraverse) {
      var index = 0,
        results, currentEle, arrayToTraverseLen = arrayToTraverse.length;
    ...
    /**
     * This function is used to create rich text in footnote editor
     */
    _initEditor: function () {
      this._initEditorPluginsCSS();
      this._footNoteEditorObj = new Editor({
        plugins: [
          "undo", "redo",
          "|",
          ...
          'foreColor',
          "|",
          "insertHorizontalRule", "removeFormat", "delete",
          "|"
        ],
        extraPlugins: [
          'createLink', 'unlink',
      ...
      html.setStyle(this._footNoteEditorObj.domNode, {
        width: '100%',
        height: '100%'
      });
      this._footNoteEditorObj.startup();
      if (has('ie') !== 8) {
        this._footNoteEditorObj.resize({
          w: '100%',
          h: '100%'
        });
      }
    ...
    _initEditorPluginsCSS: function () {
      ...
      if (!tcCss) {
        jimuUtils.loadStyleLink("editor_plugins_resources_TextColor", tcCssHref);
      }
      var epCssHref = window.apiUrl + "dojox/editor/plugins/resources/editorPlugins.css";
      var epCss = query('link[href="' + epCssHref + '"]', head)[0];
      if (!epCss) {
        jimuUtils.loadStyleLink("editor_plugins_resources_editorPlugins", epCssHref);
      }
      ...
      if (!pfCss) {
        jimuUtils.loadStyleLink("editor_plugins_resources_PasteFromWord", pfCssHref);
      }
    },

    /**
     * This function is used to get the checkbox status of display summary
     */
    getDisplaySummaryCheckBoxStatus: function () {
      return this.displaySummaryInReportCheckBox.get("checked");
    },
    /**
     * This function is used to get the checkbox status of Rows having all values zero
     */
    getRowshavingAllZeroValueCheckBoxStatus: function () {
      return this.hideAllZeroRowCheckBox.get("checked");
    },
    /**
     * This function is used to get the checkbox status of Rows having all values null
     */
    getRowshavingAllNullValueCheckBoxStatus: function () {
      return this.hideNullRowCheckBox.get("checked");
    }
  });
});