    ...
    return declare([BaseWidget], {
      ...
      postCreate: function () {
        ...
        if (this.symbolData && this.symbolData.length > 0) {
          ...
          if(this.symbolData[1].isDefault){
            this.selectSymbol(this.symbolData[1]);
          } else{
      ...
      **/
      setDefault: function () {
        ...
        if(this.symbolData && this.symbolData.length>1 && this.symbolData[1].isDefault){
          this.selectSymbol(this.symbolData[1]);
        } else{
          this.selectSymbol(this.symbolData[0]);
        }
      ...
      **/
      _createTooltipDialog: function () {
        ...
        // Attach 'keydown' event on domeNode to show tooltip dialog on clicking of it
        this.own(on(this.domNode, 'keydown', lang.hitch(this, function (
          event) {
      ...
      **/
      _createSymbolListMenu: function () {
        ...
        symbolListContent = domConstruct.create("div", {
          ...
          "role": "menu" //code for accessibility
        }, symbolList);
      ...
      **/
      _attachRowClick: function (symbolListItemNode, symbolJson) {
        ...
        //code for accessibility
        this.own(on(symbolListItemNode, "keydown", lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE || evt.keyCode === keys.ESCAPE) {
            this.selectSymbol(symbolJson);
            if (this.hideOnSelect) {
              this._hideTooltipDialog();
              focusUtils.focus(this.domNode);
            }
          }
        })));
      },
      ...
      **/
      ...
      /**
      * This function is used to focus first symbol in symbol selector
      * code for accessibility
      * @memberOf widgets/ParcelDrafter/SymbolSelector
      **/
      focusFirstSymbolOption: function () {
        var SymbolOptionNode = query(".esriCTSymbolOption", this._tooltipDialog.domNode);
        focusUtils.focus(SymbolOptionNode[0]);
      }
    });
  });