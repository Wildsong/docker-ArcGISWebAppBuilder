  ...
  return dojoDeclare(null, {
    ...
    constructor: function (args) {
      ...
      if (this.geometry.type === 'polygon') {
        if (this.geometry.hasOwnProperty('drawType')) {
          if (this.geometry.drawType === 'ellipse') {
            ...
            this.formattedStartPoint = dojoString.substitute('${xStr}, ${yStr}', {
              xStr: dojoNumber.format(this.startPoint.y, {
                places: 4
              }),
              yStr: dojoNumber.format(this.startPoint.x, {
                places: 4
              })
            });
          }
          ...
          this.formattedEndPoint = dojoString.substitute('${xStr}, ${yStr}', {
            xStr: dojoNumber.format(this.endPoint.y, {
              places: 4
            }),
            yStr: dojoNumber.format(this.endPoint.x, {
              places: 4
            })
          });
        }
        ...
        this.formattedStartPoint = dojoString.substitute("${xStr}, ${yStr}", {
          xStr: dojoNumber.format(this.wgsGeometry.getCentroid().y, {
            places: 4
          }),
          yStr: dojoNumber.format(this.wgsGeometry.getCentroid().x, {
            places: 4
          })
        });
      } else {
        if (this.geographicGeometry.spatialReference.wkid === 4326 ||
          this.geographicGeometry.spatialReference.wkid === 3857) {
          this.geodesicGeometry = esriGeometryEngine.geodesicDensify(this.geographicGeometry, 10000);
        } else {
          this.geodesicGeometry = this.geographicGeometry;
        }
        this.startPoint = this.geodesicGeometry.getPoint(0, 0);
        this.endPoint = this.geodesicGeometry.getPoint(
          0,
