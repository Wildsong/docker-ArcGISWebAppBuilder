...
define([
  ...
  function (declare, FilterListTemplate, array, html, lang, query, on, domAttr, keys,
    _WidgetsInTemplateMixin, domClass, jimuUtils, BaseWidget, FilterUtils, FilterParameters,
    esriSymbolJsonUtils, jimuSymUtils, LayerInfos, FilterManager, esriRequest, Evented) {
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin, Evented], {
      ...
      _itemTemplate: '<li aria-label="${layerTitle} ${filterNotAppliedLabel}" role="listitem" tabindex="0" ' +
      ...
      _addFilterInList: function (filterObj, idx) {
        ...
        var parse = {
          ...
          filterNotAppliedLabel: this.nls.filterNotAppliedLabel
        };
      ...
      _bindMapUpdateEvents: function (node, needApply) {
        on.once(this.map, 'update-start', lang.hitch(this, function () {
          ...
          domAttr.set(node, 'aria-label', node.innerText + this.nls.applyingFilterLabel);
        }));
        on.once(this.map, 'update-end', lang.hitch(this, function () {
          if (!needApply) {
            html.removeClass(node, 'applied');
            domAttr.set(node, 'aria-label', node.innerText + this.nls.filterNotAppliedLabel);
          } else {
            html.addClass(node, 'applied');
            domAttr.set(node, 'aria-label', node.innerText + this.nls.filterAppliedLabel);
          }
      ...
      configFilter: function (node, filterObj, evt, cb) {
        if (!node.filterParams) {
          esriRequest({
            ...
            node.filterParams.build(filterObj.url, definition, partsObj, layerId, this.id);
      ...
      applyFilterValues: function (node, filterObj, isKeyPressed, evt) {
        ...
        var expr = node.filterParams && (node.expr || this._getFilterExpr(node, filterObj));
        var applyFilter = true, alreadyApplied = false;
        //Make sure to check if valid expression exist and the current filter is not applied
        if (domClass.contains(node, "applied")) {
          alreadyApplied = true;
          var existingFilterExpr = this._getExpr(filterObj.layerId);
          var newExpr = "(" + expr + ")";
          if (existingFilterExpr === newExpr) {
            applyFilter = false;
          }
        } else {
          applyFilter = true;
      ...
      _updateClearAllButtonState: function () {
        if (this.totalAppliedFilters > 0) {
          ...
          domAttr.set(this.clearAllButton, "aria-disabled", "false");
        } else {
          domClass.remove(this.clearAllButton, "esriCTClearAllFilterActive");
          domAttr.set(this.clearAllButton, "aria-disabled", "true");
        }
      },
