...
define([
  ...
  function (exports, lang, array, html, has, Deferred, jimuUtils, esriLang, QueryTask, Query) {
    /*
    ** Modified copy from jimu
    ** adds extra append data for calculated results of SituiationAwareness analysis objects
    ** Changes to both exportCSV and exportCSVFromFeatureLayer
    */
    ...
    ** appendDatas String[]
    */
    //This could also be used if we go the route of individual CSV for each calculated result
    exports.exportCSV = function (filename, datas, columns, appendColumns, appendDatas) {
      return exports._createCSVStr(datas, columns, appendColumns, appendDatas).then(function (content) {
    ...
    ** appendDatas String[]
    */
    //this will handle an array so we can have a single CSV for all calculated results
    exports.exportCalculatedResultsCSV = function (filename, calculatedResults) {
      var contents = "";
      var i = 0;
      var newLine = "\r\n";
      array.forEach(calculatedResults, function (r) {
        contents += r.name + " (" + r.type + ")" + newLine;
        exports._createCSVStr([], [], r.appendColumns, r.appendDatas).then(function (content) {
          contents += content;
          i++;
          if (i === calculatedResults.length) {
            return exports._download(filename + '.csv', contents);
          } else {
    ...
    exports.exportCSVFromFeatureLayer = function(filename, layer, options) {
      ...
      return exports._getExportData(layer, exportOptions).then(function(result) {
        var formattedOptions = {
          ...
          appendColumns: options.appendColumns,
          appendDatas: options.appendDatas
        };
        return exports._formattedData(layer, result, formattedOptions)
          .then(function(formattedResult) {
            return exports.exportCSV(filename, formattedResult.datas, formattedResult.columns,
              formattedResult.appendColumns, formattedResult.appendDatas);
          });
      });
    };
    ...
    */
    ...
    exports._createCSVStr = function(datas, columns, appendColumns, appendDatas) {
      ...
      try {
        ...
        //Added to support append data for calculated results of SituiationAwareness analysis objects
        if (typeof (appendColumns) !== 'undefined' && typeof (appendDatas) !== 'undefined') {
          if (appendColumns.length > 0 && appendDatas.length > 0) {
            comma = "";
            array.forEach(appendColumns, function(_field) {
              content = content + comma + _field;
              comma = ",";
            });
            comma = "";
            content = content + "\r\n";
            array.forEach(appendDatas, function (_val) {
              if (Array.isArray(_val)) {
                array.forEach(_val, function (_val_) {
                  content = content + comma + _val_;
                  comma = ",";
                });
                comma = "";
                content = content + "\r\n";
              } else {
                content = content + comma + _val;
                comma = ",";
              }
            });
          }
    ...
    exports._formattedData = function(layer, dataOptions, formattedOptions) {
      ...
      def.resolve({
        ...
        columns: columns,
        appendColumns: formattedOptions.appendColumns,
        appendDatas: formattedOptions.appendDatas
      });
      return def;
    };

    exports._getExportValue = function(data, field, pk, typeIdField,
      typeData, types, formattedOptions, layerDefinition, attr, layer) {
      ...
      if ((isTypeIdField || isDomain) && layerDefinition && attr) {
        ...
        if (_result && _result.hasOwnProperty('displayValue')) {
          return _result.displayValue;
        }
      }
      if (!isDomain && !isDate && !isOjbectIdField && !isTypeIdField) {
        ...
        if (pk && types && types.length > 0) {
          ...
          if (typeCheck && typeCheck.domains &&
            ...
            var cvsResult = jimuUtils.getDisplayValueForCodedValueOrSubtype(lyr, field.name, attr);
            if(cvsResult && cvsResult.hasOwnProperty('displayValue')){
              codeValue = cvsResult.displayValue;
            }
          }
        }
