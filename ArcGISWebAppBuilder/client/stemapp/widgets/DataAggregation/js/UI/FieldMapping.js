...
define(['dojo/_base/declare',
  ...
  function (declare,
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      targetLayerUpdate: function (targetFields, sourceFields) {
        //clear previous layers fields
        this._fieldsTable.clear();
        //update field mapping status so that if previously field mapping was completed
        //it should clear the complete tick mark
        this.parent._fieldMappingComplete = false;
        this.emit('field-mapping-update', false);
        //update the target and source fields to the selected new layer
        this.targetFields = targetFields;
        this.sourceFields = sourceFields;
        //add rows for new feilds
        this._initFields(this.targetFields);
      },
      ...
      _backView: function (backResult) {
        ...
        if (backResult.currentView.label === this.label) {
          var hasResult = false;
          this._setPreviousResult(this.prevResult ? this.prevResult.results : {});
          if (this.prevResult) {
          ...
          this.parent._fieldMappingComplete = hasResult;
          this.emit('field-mapping-update', hasResult);
        }
        return true;
      },
      ...
      _initFieldsTable: function () {
        ...
        this._fieldsTable = new SimpleTable({
          ...
          selectable: false,
          autoHeight: true,
          _rowHeight: this.appConfig.theme.name === 'LaunchpadTheme' ? 36 : 30
        });
      ...
      _createSelect: function (td) {
        ...
        this.lastFieldSelect = fieldsSelect.domNode;
      },
      ...
      _setPreviousResult: function (results) {
        ...
        array.forEach(rows, function (r) {
          ...
          var targetField = r.cells[0].targetField.name;
          if (results.hasOwnProperty(targetField)) {
            sourceField = results[targetField];
            if (sourceField === undefined) {
              sourceField = noValue;
            }
          }
          r.cells[2].fieldsSelect.set('value', sourceField);
        });
      }
    });
  });