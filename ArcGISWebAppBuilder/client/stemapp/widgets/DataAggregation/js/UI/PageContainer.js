  ...
  function (declare,
    ...
    return declare([_WidgetBase, _TemplatedMixin, Evented], {
      ...
      _onAppConfigChanged: function (appConfig, reason, changedData) {
        this.appConfig = appConfig ? appConfig : this.appConfig;
        switch (reason) {
          case 'themeChange':
            this._validateIsDark(appConfig);
      ...
      _validateIsDark: function(appConfig){
        this._setThemeAndColors(appConfig).then(lang.hitch(this, function(){
          this.updateImageNodes();
          this._updateViewTheme(this.theme);
          for (var i = 0; i < this.views.length; i++) {
            var view = this.views[i];
            view.theme = this.theme;
            view.updateImageNodes();
          }
        }));
      },

      _setThemeAndColors: function (appConfig) {
        var def = new Deferred();
        var ac = appConfig && appConfig.theme && appConfig.theme.name ? appConfig : this.appConfig;
        this.theme = ac.theme.name;
        this._getStyleColor(ac.theme.styles[0]).then(lang.hitch(this, function (style) {
          this.style = style;
          if (style.name === 'light' && this.theme === 'DashboardTheme') {
            this._darkThemes = ['DartTheme'];
          } else {
            this._darkThemes = ['DartTheme', 'DashboardTheme'];
          }
          this.isDarkTheme = this._darkThemes.indexOf(this.theme) > -1;
          this.styleColor = style.styleColor;
          def.resolve(true);
        }));
        return def;
      },
      ...
      _getStyleColor: function (styleName) {
        var def = new Deferred();
        var s = styleName ? styleName : this.appConfig.theme.styles[0];
      ...
      _navView: function (navKey) {
        ...
        var homeIndex = (navKey === 'home-view' && currentView.altHomeIndex) ?
          currentView.altHomeIndex : this._homeIndex;
        ...
        //this occurs when a view has been moved between lists...
        // the currentview no longer exists so set the current view to be the previous view and navigate forward from there
        if (navKey === 'next-view' && typeof (currentView) === 'undefined') {
          if (this._backLabels.length > 0) {
            ...
            this._currentIndex = currentView && currentView.index ? currentView.index : this._currentIndex;
          }
        }
        ...
        if (typeof (navIndex) !== 'undefined' && navKey !== 'home-view') {
          ...
          if (navKey === 'next-view') {
            ...
            if (homeIndex !== this._homeIndex && this._backLabels && this._backLabels.length > 0) {
              deleteBackIndex = true;
            }
      ...
      _updateControlImage: function (node, disable) {
        if (disable) {
      ...
      _updateDomNodes: function (index) {
        ...
        this._updateControlImage(this.nextImage, nextDisabled);
      },
