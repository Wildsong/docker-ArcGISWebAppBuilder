...
define([
   ...
   * @return {object} The created div
   */
  mo.checkboxCtl = function (classes, initiallyChecked, label) {
    var div, control;
    ...
    control.setValue(initiallyChecked);
   ...
   * @return {object} The created div
   */
  mo.textButtonCtl = function (classes, label, tooltip) {
    var div = mo._createDiv(('jimu-btn ' + (classes || '')).trim());
    div.innerHTML = label;
    if (tooltip) {
      div.title = tooltip;
   ...
   * @return {object} The created div
   */
  mo.textInputCtl = function (classes, placeholder, disabled) {
    ...
    control.setDisabled(typeof(disabled) !== 'undefined' ? disabled : false);
    control.startup();

    // Screen input
    var onChange = on.pausable(control, 'change', lang.hitch(control,
      function (newText) {
        onChange.pause();
        control.set("value", generalUtils.sanitizeNoTags(newText));
        onChange.resume();
      }
    ));
    control.own(onChange);
