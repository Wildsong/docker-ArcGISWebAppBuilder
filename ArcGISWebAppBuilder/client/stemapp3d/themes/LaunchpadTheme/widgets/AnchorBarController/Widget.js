...
define(['dojo/_base/declare',
  ...
  var clazz = declare([BaseWidget, PoolControllerMixin], {
    ...
    _highLight: function(widgetId) {
      var targetNode = null;

      if (this.hlDiv) {
        this._removeHighLight();
      }
      array.some(this.iconList, function(iconItem){
        if(iconItem.config.id === widgetId){
          targetNode = iconItem;
          return true;
        }
      });

      if(targetNode === null || targetNode.visible === false){
        return;
      }

      var position = domGeometry.getMarginBox(targetNode.iconItemNode);
      var hlStyle = {
        position: 'absolute',
        left: (position.l) + 'px',
        top: (position.t) + 'px',
