  ...
  function(declare, array, lang, html, query, on,
    ...
    var clazz = declare([BaseWidget, PoolControllerMixin], {
      ...
      setOpenedIds: function(ids) {
        ...
        this._closeMenu();
        var widgetsInWidgetPool = (this.appConfig.widgetPool && this.appConfig.widgetPool.widgets) || [];
        var openIdsInWidgetPool = ids && ids.filter(function(id){
          return widgetsInWidgetPool.some(function(widgetInPool){ return widgetInPool.id === id; });
        });
        if (openIdsInWidgetPool && openIdsInWidgetPool.length > 0) {
          var id = openIdsInWidgetPool[0];
          this.openedId = id;
      ...
      _switchNodeToClose: function(id) {
        ...
        if (node.config.inPanel === false) {
          ...
          if (node.config.name === "SmartEditor") {
      ...
      _showIconContent: function(iconConfig) {
        if (iconConfig.inPanel === false) {
          ...
          this.panelManager.showPanel(iconConfig).then(lang.hitch(this, function(panel) {
            ...
            this.panelManager.activatePanel(panel);//need to active panel again after calling the setPosition function
