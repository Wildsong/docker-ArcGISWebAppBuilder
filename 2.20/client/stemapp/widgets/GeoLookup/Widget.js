  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    startup : function() {
      ...
      array.forEach(this.config.enrichLayers, function(lay) {
        ...
        "' class='result-text' tabIndex=0></div>" + "</td></tr>");
        ...
        this.enrichResultsText[textID].innerHTML = utils.sanitizeHTML(string.substitute(
    ...
    _csvReadComplete : function(items) {
      if (items.length <= parseInt(this.config.maxRowCount, 10)) {
        ...
        this.resultsLoading.innerHTML = utils.sanitizeHTML(string.substitute(this.nls.results.csvLoaded, {
          0 : recCount
        }));
        domClass.replace(this.resultsLoadingImage, "complete", "processing");

        var objectId = 1;
        var reprojectFlag = false;
        ...
        array.forEach(items, function(item, i) {
          ...
          if (isNaN(attributes[this.latField]) || isNaN(attributes[this.longField])) {
            ...
            this.enrichErrors.innerHTML = utils.sanitizeHTML(string.substitute(this.nls.results.recordsError, {
              0 : errorCnt
            }));
          } else {
          ...
          if (!errorFlag) {
            var geometry;
            if(mapProj === 'latlon') {
              if(this.map.spatialReference.wkid === this.srWebMerc.wkid) {
                ...
                this.resultsPlotting.innerHTML = utils.sanitizeHTML(string.substitute(this.nls.results.recordsPlotted, {
                ...
                reprojectFlag = true;
                //use geom service, process array length (for good and error), then call create feature collection.
                var srLatLng = new SpatialReference({
                  wkid : 4326
                });
                geometry = new Point(longitude, latitude, srLatLng);
                var gsvc = new GeometryService(this.config.geometryServiceURL);
                var params = new ProjectParameters();
                params.geometries = [geometry];
                params.outSR = this.map.spatialReference;
                gsvc.project(params, lang.hitch(this, function(projectedPoint) {
                  geometry = projectedPoint[0];
                  var feature = {
                    'geometry' : geometry.toJson(),
                    'attributes' : attributes
                  };
                  ...
                  if(i === (items.length)-1) {
                    this._createFeatureCollection(infoTemplate, featureCollection);
                    this.prepEnrich();
                  }
                }));
                //geometry = new Point(webMercatorUtils.lngLatToXY(longitude, latitude), this.srWebMerc);
    ...
    _createFeatureCollection: function(infoTemplate, featureCollection) {
      if (this.layerLoaded) {
    ...
    _requestComplete : function(args) {
      this.enrichResultsText[args.layerID].innerHTML = utils.sanitizeHTML(string.substitute(this.nls.results.recordsEnriched,
      {
    ...
    _resetResults : function() {
      ...
      for (key in this.enrichResultsProg) {
        if (this.enrichResultsText.hasOwnProperty(key)) {
          ...
          this.enrichResultsText[key].innerHTML = utils.sanitizeHTML(string.substitute(this.nls.results.recordsEnriched,
          {
            0 : 0,
            1 : 0,
            2 : 0,
    ...
    showErrorTable : function() {
      ...
      if (errorDivStatus === 'none') {
        ...
        this.enrichErrorsList.innerHTML = utils.sanitizeHTML(content);
    ...
    _setFirstLastNodeOnMainScreen: function () {
      ...
      firstNode = this.widgetDescription.focusNode;
      lastNode = this.lastNode508;
      this._setWidgetFirstFocusNode(firstNode, lastNode);
      //this.showFileDialogBtn.focus();
    },
    ...
    handleKeyDownEvents: function(){
      this.own(on(this.showFileDialogBtn, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          this.showFileDialog();
        } else if(evt.keyCode === keys.TAB) {
          //this.chkboxPlotOnly.focus();
        } else {

        }
      })));
      this.own(on(this.csvFileInput, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          this.showFileDialog();
        } else if(evt.keyCode === keys.TAB) {
          //this.chkboxPlotOnly.focus();
      ...
      this.own(on(this.clearResultsBtn, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          this.clearCSVResults();
        }
      })));
      this.own(on(this.downloadResultsBtn, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          this.downloadCSVResults();
        }
      })));
      this.own(on(this.chkboxPlotOnly, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          if (this.chkboxPlotOnly.checked) {
            this.chkboxPlotOnly.checked = false;
          } else {
            this.chkboxPlotOnly.checked = true;
          }
        }
      })));
    },
