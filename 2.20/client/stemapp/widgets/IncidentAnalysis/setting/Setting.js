...
define([
  ...
  function(
    ...
    FeaturelayerSource, FieldPicker, BaseWidgetSetting, Message, Popup, LayerInfos, jimuUtils, CheckBox
  ) {
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      postCreate: function() {
        ...
        // this._setLayers();
        // this._setTypes();
        this.chk_routing = new CheckBox({
          label: this.nls.routing_checkBox
        }, this.chk_routing_node);
        this.chk_routing.startup();

        this.chk_weather = new CheckBox({
          label: this.nls.weather_checkBox
        }, this.chk_weather_node);
        this.chk_weather.startup();

        this.chk_celsius = new CheckBox({
          label: this.nls.weather_celsius
        }, this.chk_celsius_node);
        this.chk_celsius.startup();

        this.chk_disable = new CheckBox({
          label: this.nls.disable_checkBox
        }, this.chk_disable_node);
        this.chk_disable.startup();
      ...
      setConfig: function(config) {
        ...
        if (this.config.incidentLabel) {
          this.incident_lbl.set("value", this.config.incidentLabel);
        }

        if (this.config.locateLabel) {
          this.locate_lbl.set("value", this.config.locateLabel);
        }
        ...
        for (var i = 0; i < this.config.tabs.length; i++) {
          ...
          if (aTab.type === this.config.special_layer.value) {
            this.chk_weather.setValue(true);
            this.weatherTabAdditionalLayers = aTab.layers;
            this.currentlySelectedLayer.innerHTML = jimuUtils.sanitizeHTML(this.weatherTabAdditionalLayers ? this.weatherTabAdditionalLayers : '');
          } else {
        ...
        if(this.config.celsius) {
          this.chk_celsius.setValue(true);
        }
        ...
        if (this.config.disableLayerManagement) {
          this.chk_disable.setValue(true);
        }
      },
      ...
      getConfig: function() {
        ...
        this.config.incidentLabel = this.incident_lbl.value;
        this.config.locateLabel = this.locate_lbl.value;
        ...
        array.forEach(trs, lang.hitch(this, function(tr) {
          ...
          aTab.title = selectLayers.attr('displayedValue');
