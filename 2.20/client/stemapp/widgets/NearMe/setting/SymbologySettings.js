          ...
          array.forEach(layerInfos, lang.hitch(this, function (layerInfo) {
            ...
            if (subLayerInfos && subLayerInfos.length > 0) {
              array.forEach(subLayerInfos, lang.hitch(this, function (subLayerInfo) {
                //if layer is valid and not empty then only consider it
                if (subLayerInfo && subLayerInfo.layerObject &&
                  !subLayerInfo.layerObject.empty) {
          ...
          all(layerDefs).then(lang.hitch(this, function () {
            ...
            if (this.config.attributeSymbology &&
              ...
              for (var layerId in this.config.attributeSymbology) {
                if (this.config.attributeSymbology[layerId].hasOwnProperty('layerId')) {
                  this._populateTableRows([this.config.attributeSymbology[layerId]],
                    this.config.attributeSymbology[layerId].layerId);
                } else {
    ...
    **/
    ...
    _createLayerFieldsStore: function (editableLayerRow) {
      ...
      array.forEach(selectedLayerObj.fields, lang.hitch(this, function (field) {
        var fieldName, featureLayersArr, codedDomainObj, fieldsToSkipArr, codedValuesArray;
        featureLayersArr = [];
        ...
        if (fieldsToSkipArr.indexOf(field.type) === -1) {
          ...
          if (!this._layerDetails[selectedLayerID].fields[fieldName]) {
            ...
            if (codedDomainObj) {
              codedDomainObj = lang.clone(codedDomainObj);
              //if field has coded values get it using jimu method
              if (codedDomainObj.codedValues) {
                codedValuesArray =
                  jimuUtils.getCodedValueListForCodedValueOrSubTypes(selectedLayerObj, fieldName);
                codedDomainObj.codedValues = lang.clone(codedValuesArray);
              }
   ...
   */
    ...
    getConfig: function () {
      var symbolConfig = {}, symbologyAttributes, selectedLayer, layerId,
        isValid = true, errMessage, symbologyArray = [];
      array.forEach(this._symbologyTable.getRows(), lang.hitch(this, function (currentRow) {
        if (currentRow.layerNameDropDown) {
          ...
          symbologyAttributes = {};
          symbologyAttributes.layerId = layerId;
          ...
          symbologyArray.push(symbologyAttributes);
        }
      }));
      //If configuration is valid, send the updated config
      if (isValid) {
        return { isValid: isValid, symbolConfig: symbologyArray };
      } else {
