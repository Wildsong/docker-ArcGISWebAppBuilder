...
define([
  ...
  geometryEngine, Extent, Query, QueryTask, StatisticDefinition, FeatureLayer, clientStatistic) {
  ...
  return declare([_WidgetBase, Evented], {
    ...
    constructor: function(options) {
      ...
      this.isDateType = !!data.dateConfig;
      ...
      //for cache hint
      this.extents = this.getBookmarkExtents(options.appConfig);
    },
    ...
    _shouldAccessClientFeatures: function() {
      var a = this.config && this.config.data && this.config.data.mode === 'feature'; //chart feature mode
    ...
    _calculateAverageWithNullValueAsZero: function(features, config) {
      ...
      var valueFields = config.data && config.data.valueFields;
    ...
    _shouldCalcAvgForNullValue: function(config) {
      var mode = config && config.data && config.data.mode;
      var operation = config && config.data && config.data.operation;
      var nullValue = config && config.data && config.data.nullValue;
      return mode && (mode === 'category' || mode === 'field') && nullValue &&
        operation === 'average';
    },
    ...
    _doQuery: function() {
      ...
      if (this.dataSource.filterByExtent && this.map) {
        ...
        query.cacheHint = this.supportCacheHint(extent);
      }
    ...
    supportCacheHint: function(extent) {
      var ac = this.appConfig;
      if (!ac) {
        return false;
      }
      var extents = this.extents || [];
      return extents.some(function(ex) {
        ex = this.getTargetExtent(ex);
        return this.isSameExtent(extent, ex);
      }.bind(this));
    },

    getBookmarkExtents: function(appConfig) {
      var configs = this.getBookmarkConfig(appConfig);
      return this.getExtendsFromBookmarkConfigs(configs);
    },

    getBookmarkConfig: function(appConfig) {
      var ac = appConfig;
      if (!ac) {
        return [];
      }
      ...
      var widgets1 = (ac.widgetOnScreen && ac.widgetOnScreen.widgets) || [];
      widgets = widgets.concat(widgets1);

      var widgets2 = (ac.widgetPool && ac.widgetPool.widgets) || [];
      widgets = widgets.concat(widgets2);

      var groups = (ac.widgetPool && ac.widgetPool.groups) || [];
      groups.forEach(function(group) {
        widgets = widgets.concat(group.widgets);
      });

      var bookmarks = widgets.filter(function(widget) {
        return widget.name === "Bookmark";
      });
    ...
    getExtentsFormBookmarkConfig: function(config) {
      if (!config || !config.bookmarks2D || !config.bookmarks2D.length) {
        return;
      }
      var bookmarks2D = config.bookmarks2D;
      return bookmarks2D.map(function(bookmark2D) {
        return bookmark2D.extent;
      });
    },

    isSameExtent: function(extent1, extent2) {
      var et1 = this.getCleanExtent(extent1);
      var et2 = this.getCleanExtent(extent2);
      return utils.isEqual(et1, et2);
    },

    getTargetExtent: function(extent) { //#15787#issuecomment-1730970
      if (!this.map || !this.map.getTargetExtent) {
        return extent;
      }
      return this.map.getTargetExtent(new Extent(extent));
    },

    getCleanExtent: function(extent) {
      if (!extent) {
        return;
      }
      return {
    ...
    getExtendsFromBookmarkConfigs: function(configs) {
      var extents = [];
      configs.forEach(function(config) {
        var es = this.getExtentsFormBookmarkConfig(config) || [];
        extents = extents.concat(es);
      }.bind(this));
      return extents;
    }
