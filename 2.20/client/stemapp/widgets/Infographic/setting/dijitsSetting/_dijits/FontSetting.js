...
define([
  ...
  function (declare, lang, on, html, array, jimuUtils,
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      postCreate: function () {
        ...
        this.textSizeSelect.validator = lang.hitch(this, function () {
          ...
          if (s !== null && s !== "") {
            return jimuUtils.isLocalizedNumber(s);
          }
          return false;
        });
        ...
        this.own(on(this.textSizeSelect, 'change', lang.hitch(this, function (value) {
          ...
          this.onSettingChange({
            fontSize: jimuUtils.parseNumberToUSENFormat(value)
          });
        })));
      ...
      setTextSize: function (size) {
        var localizedNumber = jimuUtils.localizeNumber(size);
        if (localizedNumber !== this.textSizeSelect.getValue()) {
          this.textSizeSelect.set('value', localizedNumber);
        }

        var number = jimuUtils.parseNumberToUSENFormat(size);
        if (number !== this.textSizeSlider.getValue()) {
          if (number > this._MAX_TEXT_SIZE) {
            number = this._MAX_TEXT_SIZE;
          } else if (number < this._MIN_TEXT_SIZE) {
            number = this._MIN_TEXT_SIZE;
          }

          this.textSizeSlider.set('value', number);
        }
      },
