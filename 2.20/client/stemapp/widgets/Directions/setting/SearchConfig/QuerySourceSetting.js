  ...
  return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
    ...
    getConfig: function() {
      var json = {
        ...
        type: 'query',
        autoComplete: this.autoComplete ? this.autoComplete.getValue() : true,
        searchDelay: this.searchDelay.get('value') || 350,
        minCharacters:this.minCharacters.get('value') || 3
      };
      if(true === this.config._isUpgrade){
        json._isUpgrade = true;
      }
    ...
    _disableSourceItems: function() {
      ...
      this.autoComplete.set('disabled', true);
      this.searchDelay.set('disabled', true);
      this.minCharacters.set('disabled', true);
    },
    ...
    _enableSourceItems: function() {
      ...
      this.autoComplete.set('disabled', false);
      this.searchDelay.set('disabled', false);
      this.minCharacters.set('disabled', false);
    },
    ...
    _setSourceItems: function() {
      ...
      this.autoComplete.setValue("undefined" !== this.config.autoComplete ? this.config.autoComplete : true);
      this.searchDelay.set('value', this.config.searchDelay || 350);
      this.minCharacters.set('value', this.config.minCharacters || 3);

      if (this.config._oldConfig && !this.config._isUpgrade) {
        if(this.config._oldConfig.placeholder){
          this.placeholder.set('value', jimuUtils.stripHTML(this.config._oldConfig.placeholder));
        }
        if(this.config._oldConfig.maxLocations){
          this.maxSuggestions.set('value', this.config._oldConfig.maxLocations);
        }
        if (this.config._oldConfig.autoComplete) {
          this.autoComplete.setValue(this.config._oldConfig.autoComplete);
        }
        if (this.config._oldConfig.searchDelay) {
          this.searchDelay.set('value', this.config._oldConfig.searchDelay);
        }
        if (this.config._oldConfig.minCharacters) {
          this.minCharacters.set('value', this.config._oldConfig.minCharacters);
        }

        this.config._isUpgrade = true;
      }
    ...
    _onFieldsSelectorClick: function() {
      ...
      html.addClass(this.fieldsPopup.domNode, 'direction-setting jimu-widget-search-query-source-setting-fields');
    },
