...
define([
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    _shapeFields: ["shape_length", "shape_area", "shape__length",
      "shape__area", "shape.len", "shape.area"
    ...
    postCreate: function () {
      ...
      this._editableConfiguredLayers = [];
      this.inherited(arguments);
      this._init();
      this._handleClickEvent();

      // to create additional project cost table
    ...
    /**
     * This function is used to create additional project cost table
     * @memberOf setting/StatisticsSettings
     */
    _createProjectCostTable: function () {
      this.additionalProjectCostObj = new ProjectCost({
        nls: this.nls,
        map: this.map,
        config: this.config
      }, domConstruct.create("div", {}, this.additionalProjectCostWrapper));
      this.additionalProjectCostObj.startup();
    },
     ...
     */
    _init: function () {
      for (var key in this.config.layerSettings) {
        if(this.config.layerSettings[key].editable) {
          this._editableConfiguredLayers.push(this.config.layerSettings[key].id);
        }
      }
      ...
      if (this._editableConfiguredLayers.length === 0) {
        ...
        domClass.add(this.additionalProjectCostWrapper,
          "esriCTHidden");
      }
    },
    ...
    getConfig: function () {
      ...
      if (this.additionalProjectCostObj) {
        this.additionalProjectCostObj.getConfig();
      }
    ...
    _createStatisticsStoreOptions: function () {
      ...
      this._commonStatisticsStoreOption = [{
        name: this.nls.statisticsType.averageLabel,
        value: "avg"
      }, {
        name: this.nls.statisticsType.countLabel,
        value: "count"
      }, {
        name: this.nls.statisticsType.maxLabel,
        value: "max"
      }, {
        name: this.nls.statisticsType.minLabel,
        value: "min"
      }, {
        name: this.nls.statisticsType.summationLabel,
        value: "sum"
      }];
      // polygon store option
      this._polygonStatisticsStoreOption = [{
        name: this.nls.statisticsType.areaLabel,
        value: "area"
      }];
      // polyline store option
      this._polylineStatisticsStoreOption = [{
        name: this.nls.statisticsType.lengthLabel,
        value: "length"
      }];
    },
    ...
    _attachNodeEvents: function () {
      ...
      this.own(on(this.btnAddStatisticsNode, 'keydown',
    ...
    _createStatisticsTable: function () {
      var args, fields, tableCheckboxheaderLabel, tableCheckboxheader, tableCheckboxInRow;
      fields = [{
        name: 'editable',
      ...
      tableCheckboxheader = query('.esriCTAddLayerTableNode th .jimu-checkbox')[0];
      tableCheckboxInRow = query('.esriCTAddLayerTableNode tr .jimu-checkbox');
      if (tableCheckboxheader) {
        domClass.add(tableCheckboxheader, "esriCTStatisticsHeaderCheckBox");
      }
    ...
    _addButtonClick: function () {
      if (this.showActionButtonsInRow === true) {
        ...
        this.own(on(this.btnAddNode, 'click', lang.hitch(this, function () {
          this._addStatisticsButtonClickEvent();
        })));
    ...
    /**
     * This function is used for add new statistics button event
     * @memberOf setting/StatisticsSettings
     */
    _addStatisticsButtonClickEvent: function () {
      //If layer exist, then only add the row in statistics table
      if (this._layerNameStore.data.length) {
        this._addStatisticsBtnClicked();
        this._handleActionButtonVisibility();
    ...
    /**
     * This function is used to handle up button click event of selected row
     * @memberOf setting/StatisticsSettings
     */
    _upButtonClickEvent: function () {
      var tr, trs;
    ...
    /**
     * This function is used to down button click event of selected row
     * @memberOf setting/StatisticsSettings
     */
    _downButtonClickEvent: function () {
      var tr, trs;
    ...
    _handleActionButtonVisibility: function () {
      var tableRows, editableRowCheckbox, editableRowCount = 0,
        editableHeaderCheckbox, editableHeaderCheckboxNode;
      tableRows = this._statisticsLayerTable.getRows();
    ...
    _addLayerRow: function (statistics) {
      ...
      //Set aria-label for 508 compliance
      domAttr.set(editableCheckBox, "aria-label", this.nls.statisticsSettings.layerCheckbox);
      if (fieldsColumn) {
        if (editableCheckBox) {
    ...
    _addLayerNameDropdown: function (fieldsColumn, editableLayerRow, statistics) {
      ...
      this.own(on(editableLayerRow.layerNameDropDown, "keydown",
        lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            Event.stop(evt);
            this._resetLayerNameWidth(editableLayerRow);
      ...
      this.own(on(editableLayerRow.layerNameDropDown, "change",
        lang.hitch(this, function (evt) {
          domAttr.set(editableLayerRow.layerNameDropDown, "aria-label", this.nls.common.layer + " " +
            editableLayerRow.layerNameDropDown._getSelectedOptionsAttr().label);
    ...
    _createLayerNameStore: function () {
      ...
      if (this.config.layerSettings.length && this._editableConfiguredLayers.length) {
      ...
      this._layerNameStore = new Memory({
        idProperty: "value",
        data: featureLayersArr
      });
      if(featureLayersArr.length) {
    ...
    _updateLayerNameOptions: function () {
      ...
      if (this._editableConfiguredLayers.length === 0 ) {
        domClass.remove(this.noEditableLayersAvailable,
          "esriCTHidden");
        domClass.add(this.btnAddStatisticsNodeWrapper,
          "esriCTHidden");
        domClass.add(this.statisticsLayerTableNode,
          "esriCTHidden");
        domClass.add(this.additionalProjectCostWrapper,
          "esriCTHidden");
      } else {
        domClass.add(this.noEditableLayersAvailable,
          "esriCTHidden");
        domClass.remove(this.btnAddStatisticsNodeWrapper,
          "esriCTHidden");
        domClass.remove(this.statisticsLayerTableNode,
          "esriCTHidden");
        domClass.remove(this.additionalProjectCostWrapper,
          "esriCTHidden");
      }
    },
    ...
    _addStatisticsDropdown: function (fieldsColumn, editableLayerRow, statistics) {
      ...
      this.own(on(editableLayerRow.statisticsTypeDropdown, 'keydown',
        lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            Event.stop(evt);
            this._resetStatisticsTypeWidth(editableLayerRow);
      ...
      this.own(on(editableLayerRow.statisticsTypeDropdown, 'change',
        lang.hitch(this, function (evt) {
          ...
          domAttr.set(editableLayerRow.statisticsTypeDropdown, "aria-label",
            "type " + editableLayerRow.statisticsTypeDropdown._getSelectedOptionsAttr().label);
        })));
      domAttr.set(editableLayerRow.statisticsTypeDropdown, "aria-label",
        "Type " + editableLayerRow.statisticsTypeDropdown._getSelectedOptionsAttr().label);
    ...
    _addLayerFieldDropdown: function (fieldsColumn, editableLayerRow) {
      ...
      this.own(on(editableLayerRow.layerFieldDropdown, "keydown",
        lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            Event.stop(evt);
            this._resetLayerFieldWidth(editableLayerRow);
      ...
      this.own(on(editableLayerRow.layerFieldDropdown, "change",
        lang.hitch(this, function () {
          this._resetLayerFieldWidth(editableLayerRow);
          domAttr.set(editableLayerRow.layerFieldDropdown, "aria-label",
            this.nls.statisticsSettings.fieldNameTitle + " " + editableLayerRow.layerFieldDropdown.value);
        })));
    ...
    _addStatisticsLabelTextBox: function (fieldsColumn, editableLayerRow, statistics) {
      ...
      domAttr.set(editableLayerRow.statisticsLabelTextBox, "aria-label",
        this.nls.statisticsSettings.statisticsTitle + " " +
        editableLayerRow.statisticsLabelTextBox.get('value'));
    },
    ...
    _updateStatisticsTable: function (updatedRowDetails) {
      ...
      if (layerIdToBeRemoved) {
        ...
        //remove element from editablecnfiguredLayer
        var layerIndex = this._editableConfiguredLayers.indexOf(layerIdToBeRemoved);
        if(layerIndex !== -1) {
          this._editableConfiguredLayers.splice(layerIndex,1);
        }
      }
      //If row needs to be added, add it and update store
      if (layerIdToBeAdded && layerIdToBeAdded !== "") {
        layerInfo = this.layerInfosObj.getLayerInfoById(layerIdToBeAdded);
        if (layerInfo && layerInfo.layerObject) {
          layerInfo = layerInfo.layerObject;
          ...
          this._editableConfiguredLayers.push(layerIdToBeAdded);
        }
      }
