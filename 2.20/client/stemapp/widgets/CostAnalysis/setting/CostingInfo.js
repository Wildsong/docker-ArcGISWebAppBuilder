...
define([
  ...
  return declare([BaseWidget, Evented, _WidgetsInTemplateMixin], {
    ...
    postCreate: function () {
      ...
      this._setUpdateEquationCheckBoxState();
      for (var key in this.config.layerSettings) {
        if(this.config.layerSettings[key].editable) {
          this._configuredLayers.push(this.config.layerSettings[key].id);
        }
      }
      this._init();
    ...
    /**
     * This function is used to set the state of update equation checkbox
     */
    _setUpdateEquationCheckBoxState: function () {
      if (this.config && this.config.hasOwnProperty('updateCostEquationCheckBoxStatus')) {
        this.updateCostEquationCheckBox.set('checked', this.config.updateCostEquationCheckBoxStatus);
      }
    },
    ...
    getConfig: function () {
      return {
        ...
        "updateCostEquationCheckBoxStatus": this.updateCostEquationCheckBox.get('checked')
      };
    },

    _init: function () {
      this._geographyOptions = [{
        "label": this.nls.costingInfo.noneValue,
        "value": ""
      }];
      ...
      if (this.config.projectSettings &&
        ...
        if(this.config.layerSettings && this.config.layerSettings.length) {
          this._createCostingTemplate();
        }
      }
    },
    ...
    _createCostingTemplate: function () {
      ...
      if (Object.keys(this.config.costingInfoSettings).length > 0) {
        for (layerId in this.config.costingInfoSettings) {
          if(this._configuredLayers.indexOf(layerId) !== -1) {
    ...
    _createCostingTemplateObj: function (currentLayer) {
      if(this._scenarioOptions.length === 0) {
        this._scenarioOptions.push({
          "label": this.nls.costingInfo.noneValue,
          "value": ""
        });
      }
    ...
    _checkEditCapabilities: function (currentLayer) {
      ...
      if (currentLayer && currentLayer.capabilities &&
        typeof (currentLayer.capabilities) === "string" &&
        currentLayer.capabilities.indexOf("Delete") !== -1 &&
          ...
          if (currentFeature.attributes[geographyField] !== "" &&
            ...
            this._geographyOptions.push({
              label: isNaN(currentFeature.attributes[geographyField]) ?
                currentFeature.attributes[geographyField] : currentFeature.attributes[geographyField].toString(),
    ...
    _validateCostingInfoTabData: function () {
      ...
      for (layer in this.costingTemplate) {
        defaultTemplatesArray = [];
        uniqueTemplateNamesArray = [];
        if (this.costingTemplate[layer]) {
          ...
          for (costingInfoLayer in this.costingInfoData[layer]) {
            ...
            if (layerWidgetLen > 1) {
              ...
              for (layerWidget = nextWidgetIndex; layerWidget < layerWidgetLen; layerWidget++) {
                var currentWidget;
          ...
          var featureTemplate = 0,
            uniqueTemplateNamesArrayLen;
     ...
     * @memberOf setting/CostingInfo
     */
    _updateCostingInfoTable: function (updatedRowDetails, fromProjectTab) {
    ...
    _showHideNoLayerMessage: function (length) {
      if (length === 0) {
        domClass.remove(this.noEditableLayersAvailable,
          "esriCTHidden");
        domClass.add(this.updateCostEquationChkContainer,
          "esriCTHidden");
        domClass.add(this.costingTemplateContainer,
          "esriCTHidden");
      } else {
        domClass.add(this.noEditableLayersAvailable,
          "esriCTHidden");
        domClass.remove(this.updateCostEquationChkContainer,
          "esriCTHidden");
        domClass.remove(this.costingTemplateContainer,
          "esriCTHidden");
      }
    },
    ...
    /**
     * This function is used to disable or enable update project equation checkbox
     */
    disableUpdateCostEquationChk: function (disable) {
      this.updateCostEquationCheckBox.set('disabled', disable);
      if (disable) {
        this.updateCostEquationCheckBox.set('value', false);
      }
    }
  });
});