...
function (declare, lang, array, html, on, Deferred, domQuery, editUtils, Button,
  ...
  PopupTemplate, jimuUtils, portalUrlUtils, SelectionManager, ConfigManager, DropdownMenu,
  LoadingIndicator, jimuLayerInfos) {
  /*jshint unused: false*/
  var Clazz = declare([_WidgetBase, _TemplatedMixin], {
    ...
    */
    ...
    _getOriRelationshipByDestLayer: function(operationData) {
      ...
      var queryRelationships = array.filter(operationData.oriJimuLayerInfo.layerObject.relationships,
                                            function(relationship) {
        if (relationship.relatedTableId === operationData.destJimuLayerInfo.layerObject.layerId) {//************
          return true;
        }
      }, this);
      queryRelationship = queryRelationships[operationData.relationshipIndex] ?
                          queryRelationships[operationData.relationshipIndex] :
    ...
    _getDestRelationshipByDestLayer: function(operationData) {
      ...
      // compatible with arcgis service 10.0.
      var destRelationships = array.filter(operationData.destJimuLayerInfo.layerObject.relationships,
                                           function(relationship) {
        if (relationship.relatedTableId === operationData.oriJimuLayerInfo.layerObject.layerId) {
          return true;
        }
      }, this);
      destRelationship = destRelationships[operationData.relationshipIndex] ?
                         destRelationships[operationData.relationshipIndex] :
                         destRelationships[0];
    ...
    _createATI: function(operationData) {
      ...
      if(tableInfoResult) {
        ...
        // enalbe/disable delete button
        var relatedLayerInfoParam = this._findTableInfoFromTableInfosParam(relatedJimuLayerInfo);
        canDelete = editUtils.getEditCapabilities(relatedJimuLayerInfo.layerObject,
                                                  relatedLayerInfoParam).canDelete;
        if(!canDelete) {
          //domQuery(".atiButton.atiDeleteButton", attributeInspector.domNode).style("display", "none !important");
          //domQuery(".atiButton.atiDeleteButton", attributeInspector.domNode).addClass("disable");
          domQuery(".atiButton.atiDeleteButton", attributeInspector.domNode).addClass("hidden");
        }
      }
    ...
    */
    ...
    showRelatedRecords: function(operationData) {
      ...
      .then(lang.hitch(this, function(relatedRecords) {
        // show or hide add new button
        var relationship = operationData.oriJimuLayerInfo.getOriRelationshipByDestLayer(
                                                          operationData.oriJimuLayerInfo.layerObject,
                                                          operationData.destJimuLayerInfo.layerObject);
        if(relationship && relationship.cardinality &&
            relationship.cardinality.toLowerCase &&
            relationship.cardinality.toLowerCase().indexOf("onetoone") >= 0 &&
        ...
        array.forEach(relatedRecords, function(relatedRecord, index) {
          ...
          var recordItem = html.create('div', {
            'class': 'item record-item enable ' + backgroundClass,
            innerHTML: jimuUtils.sanitizeHTML(displayTitle)
          }, this.contentBox);
    ...
    showInspector: function(operationData) {
      ...
      if(objectId === null || objectId === undefined) {
        ...
        operationData.destJimuLayerInfo.layerObject.clearSelection();
    ...
    showRelatedTables: function(operationData, previouOperationData, disableAccess) {
      ...
      .then(lang.hitch(this, function(layerInfoArray) {
        ...
        array.forEach(layerInfoArray, function(relatedLayerInfo, index) {
          ...
          var titleDivStr = '<div title="' + (relatedLayerInfo.title) + '">' +
                            jimuUtils.sanitizeHTML(relatedLayerInfo.title) + '</div>';
          ...
          if(relationshipCount[relatedLayerInfo.id] > 0) {
            var relationship = operationData.oriJimuLayerInfo.
                               getOriRelationshipByDestLayer(operationData.oriJimuLayerInfo.layerObject,
                                                              relatedLayerInfo.layerObject,
                                                              relationshipIndex);
            var relationshipName = (relationship.name || relationship.id);
            var relationshipNameDivStr = '<div class="relationshipName" title="' +
                    relationshipName + '">(' +  jimuUtils.sanitizeHTML(relationshipName) + ')</div>';
    ...
    _createOperationData: function(feature,
      ...
      var newOperationData = {
        ...
        relationshipIndex: relationshipIndex || 0
      };
    ...
    _showAddNewBtn: function(operationData) {
      ...
      var relatedLayerObject = operationData.destJimuLayerInfo.layerObject;
      var relatedLayerInfoParam = this._findTableInfoFromTableInfosParam(relatedJimuLayerInfo);
      if(editUtils.getEditCapabilities &&
         editUtils.getEditCapabilities(relatedLayerObject, relatedLayerInfoParam).canCreate) {
    ...
    _setTitle: function(title, className) {
      if(title) {
        html.create('div', {
          ...
          innerHTML: jimuUtils.sanitizeHTML(title)
        }, this.contentBox);
      }
    },

    _setOperationTitle: function(title) {
      html.setAttr(this.operationTitle, 'innerHTML', jimuUtils.sanitizeHTML(title));
    ...
    _showFieldSelector: function(relatedLayerInfo) {
      ...
      var fieldInfosOfTableInfoParam = this._findTableInfoFromTableInfosParam(relatedLayerInfo);

      if(fieldInfosOfTableInfoParam && fieldInfosOfTableInfoParam.fieldInfos) {
        array.forEach(fieldInfosOfTableInfoParam.fieldInfos, function(fieldInfo){
          if(fieldInfo.fieldName.toLowerCase() !== "globalid" &&
            fieldInfo.fieldName.toLowerCase() !== "shape"){
            items.push({
              label: fieldInfo.label || fieldInfo.fieldName,
      ...
      var fieldSelectorChangeHandle = on(fieldSelector,
        ...
        domQuery(".item.record-item", this.contentBox).forEach(lang.hitch(this, function(node) {
          ...
          var displayTitle = this._getDisplayTitleOfRelatedRecord(relatedLayerInfo,
                                                                  node.relatedRecord,
                                                                  newValue);
          node.innerHTML = jimuUtils.sanitizeHTML(displayTitle);
        }));
      }, relatedLayerInfo));
  ...
  Clazz.ATI = declare([AttributeInspector], {
    ...
    _getFields: function(featureLayer){
      // working around for inspector will consider the outFields.
      return featureLayer.fields;
    }
  });
