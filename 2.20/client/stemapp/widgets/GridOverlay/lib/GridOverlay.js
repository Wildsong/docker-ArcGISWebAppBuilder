...
define([
  ...
  return declare(null, {
    ...
    constructor: function (args) {
      ...
      this._labelGraphicsLayer = new GraphicsLayer({
        "className": "gridOverlay"
      });
      this._centerLabelGraphicsLayer = new GraphicsLayer({
        "className": "gridOverlay"
      });
    ...
    getSettings: function () {
      return {
        ...
        "colors": array.map(this.colors, function (itm) {
          return itm.toHex();
        }),
    ...
    _createExtentFromScreen: function (mapPt, screenWidth, screenHeight, spatRef) {
      var screenPt = this.map.toScreen(mapPt);
      var newPt = this.map.toMap(new ScreenPoint(screenPt.x + screenWidth, screenPt.y + screenHeight));
      return new Extent(mapPt.x, mapPt.y, newPt.x, newPt.y, spatRef);
    },

    _performOverlapCheck: function (labelList) {
      for (var i = 0; i < labelList.length; i++) {
        for (var j = i + 1; j < labelList.length; j++) {
          if (this._doLabelsOverlap(labelList[i], labelList[j])) {
            labelList[i].symbol.setOffset(0, 40);
          }
        }
      }
    },

    _doLabelsOverlap: function (label0, label1) {
      if (label0 && label1) {
        // Convert label0 map point to screen units
        var label0Ext = this._createExtentFromScreen(label0.geometry, label0.symbol.getWidth(),
          label0.symbol.getHeight(), this.map.spatialReference);
        var label1Ext = this._createExtentFromScreen(label1.geometry, label1.symbol.getWidth(),
          label1.symbol.getHeight(), this.map.spatialReference);
        return geometryEngine.intersects(label0Ext, label1Ext);
      }
      return false;
    },
    ...
    _drawGrid: function () {
      ...
      this._drawing = setTimeout(lang.hitch(this, function () {
        ...
        array.forEach([this._lineGraphics5, this._lineGraphics4, this._lineGraphics3,
          this._lineGraphics2, this._lineGraphics1, this._lineGraphics0
        ], function (arr) {
        ...
        // Check if labels overlap focusing on GZD and 100K labels
        this._performOverlapCheck(this._labelGraphics);
