  ...
  return declare([BaseWidgetSetting], {
    ...
    **/
    getConfig: function () {
      ...
      if (!this.config.searchSourceSettings.search ||
        !this.config.searchSourceSettings.search.sources ||
        this.config.searchSourceSettings.search.sources.length === 0) {
        problems.push(this.nls.problems.noSearchSourcesConfigured);
      }
      ...
      //when the selection tool is checked and no search source layer is configured
      //Then warn the users to configure search layer.
      if (this.useSelectionDrawToolsChk._inputControl.checked) {
        if (!this._isSearchSourceFeaureLayerConfigured()) {
          new Message({
            message: this.nls.problems.noSearchSourceFeaureLayerConfigured
          });
          return false;
        }
      }
    ...
    **/
    _createSearchSourceSettings: function () {
      ...
      this.useSelectionDrawToolsChk = new SettingCheckbox('select', nls.propertyLabels.useSelectionDrawTools);
      description.content = new SettingContainer('searchSourceSettings', '', 'majorTrailingVertGap', [
        ...
        new SettingSearchSourcesBlock('search', nls, '', this.map, this._webmapFeatureLayers, this.appConfig.portalUrl),
    ...
    **/
    _createAddressSourceSettings: function () {
      ...
      var reviewCheckbox = new SettingCheckbox('showReviewButton', nls.propertyLabels.reviewCheckboxLabel);
      domClass.add(reviewCheckbox._mainDiv, "esriCTMarginZero");
      ...
      description.content = new SettingContainer('addresseeSourceSettings', '', 'majorTrailingVertGap', [
        ...
        reviewCheckbox,
        new SettingStaticText(null, 'full-width hint', nls.hints.reviewButtonHint)
      ]);
    ...
    **/
      ...
      description.content = new SettingContainer('notificationSettings', '', 'majorTrailingVertGap', [
        ...
        new SettingContainer('labelPageOptions', 'flexbox', 'majorTrailingHorizGap',
          ...
          new SettingContainer(null, 'half-width inline-block', 'majorTrailingVertGap', [
            ...
            new SettingCheckbox('addCSVHeader', nls.propertyLabels.addCSVHeader)
          ]),
          new SettingContainer(null, 'half-width inline-block', 'majorTrailingVertGap', [
    ...
    /**
     * This function is used to add validation on Config Ok button click
     * when the selection tool is checked, and a search layer is configured
     * it will return true otherwise false
     */
    _isSearchSourceFeaureLayerConfigured: function () {
      var isSearchSourceFeaureLayerConfigured = false;
      var selectLayerIds = array.filter(
        array.map(this.config.searchSourceSettings.search.sources, function (source) {
      ...
      var mapSelectionLayers = array.filter(this._webmapFeatureLayers, function (layer) {
        return selectLayerIds.indexOf(layer.layerId) >= 0;
      });

      if (mapSelectionLayers.length > 0) {
        isSearchSourceFeaureLayerConfigured = true;
      }
      return isSearchSourceFeaureLayerConfigured;
    }
    //================================================================================================================//
  });
});
