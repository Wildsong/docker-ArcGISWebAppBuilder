...
function(declare, lang, html, array, on, query, Deferred, template, _WidgetBase, _TemplatedMixin,
  TitlePane, ViewStack, LoadingIndicator, Message, ParamSetting, ParamNodeList, LayerOrder,
  Options, gputils) {
  return declare([_WidgetBase, _TemplatedMixin], {
    ...
    postCreate: function(){
      ...
      this.own(on(this.options, 'result-map-service', lang.hitch(this, function(checked) {
        this._outputInResultMap(checked);
      })));
      ...
      this.loadingCover = new LoadingIndicator({hidden: true});
    ...
    setConfig: function(_config){
      ...
      if(this.config && this.config.taskUrl === config.taskUrl){
        ...
        if(!('serverInfo' in config) ||
          (this.config.serverInfo.hasResultMapServer &&
          !this.config.serverInfo.resultMapServerName)){
    ...
    _initNavPane: function(){
      ...
      this._createParamsSection('input')
        .then(lang.hitch(this, function() {
          return this._createParamsSection('output');
        }))
        .then(lang.hitch(this, function() {
    ...
    _createParamsSection: function(direction){
      ...
      if (direction === 'output' && this.config.serverInfo.resultMapServerName) {
        ...
        if (!exists) {
          gputils.getResultMapLayers(this.config.taskUrl, this.config.serverInfo.resultMapServerName)
          .then(lang.hitch(this, function(layerNames) {
            params.push({
              name: this.config.serverInfo.resultMapServerName,
              label: this.config.serverInfo.resultMapServerName,
            ...
            this._doCreateParamsSection(params, direction);
            def.resolve();
          }));
          return def;
        }
    ...
    _doCreateParamsSection: function(params, direction) {
      var paramNodes = new ParamNodeList({
        params: params,
        nls: this.nls,
        direction: direction,
        useResultMapServer: this.config.useResultMapServer
      });
    ...
    _outputInResultMap: function(checked) {
      if (checked === true) {
        this.outputParamNodes.hideOutputInResultMap();
      } else {
        this.outputParamNodes.showOutputInResultMap();
      }
    }
  });
});
