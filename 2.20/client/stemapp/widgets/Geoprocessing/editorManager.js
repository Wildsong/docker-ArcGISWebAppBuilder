...
define([
  ...
  './editors/RecordSetEditorChooser',
  './editors/SelectRecordSetFromTable'
],
function(array, on, aspect,
  ...
  DataFileEditor, RasterLayerEditor, RecordSetEditor, RecordSetEditorChooser,
  SelectRecordSetFromTable) {
  ...
  mo.createEditor = function(param, direction, context, options) {
    ...
    var o = {
      ...
      appConfig: options?options.appConfig:undefined,
      map: map,
      nls: nls,
      context: context,
      editorManager: mo,
    ...
    if(editorName === 'UnsupportEditor'){
      ...
      editor = new simpleEditors.UnsupportEditor(o);
    }else if(editorName === 'MapServiceLayer'){
      o.message = nls.useResultMapService;
      editor = new simpleEditors.UnsupportEditor(o);
    ...
    }else if(editorName === 'SelectFeatureSetFromFile'){
      editor = new SelectFeatureSetFromFile(o);
    }else if(editorName === 'FeatureSetEditorChooser'){
      editor = new FeatureSetEditorChooser(o);
    }else if(editorName === 'RecordSetEditorChooser'){
      editor = new RecordSetEditorChooser(o);
    }else if(editorName === 'SelectRecordSetFromTable') {
      editor = new SelectRecordSetFromTable(o);
  ...
  function geteditorName(param, context){
    if(param.dataType === 'GPMultiValue:GPFeatureRecordSetLayer'){
      ...
      if(context === 'setting'){
        return 'RecordSetEditorChooser';
      }else{
        if(param.recordSetMode === 'json'){
          return 'SimpleJsonEditor';
        }else if(param.recordSetMode === 'table'){
          return 'SelectRecordSetFromTable';
        }else{
          return 'UnsupportEditor';
        }
      }
  ...
  function getOutputEditorName(param){
    ...
    }else if(param.dataType === 'MapServiceLayer'){
