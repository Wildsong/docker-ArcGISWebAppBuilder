    ...
    var clazz = declare([BaseWidget, PoolControllerMixin], {
      ...
      _createIconNodes: function() {
        ...
        for (i = 0; i < allIconConfigs.length; i++) {
          ...
          if (name === "ZoomSlider" || name === "Home" || name === "MyLocation" ||
              name === "Navigate") {
        ...
        this.widgetManager.loadWidget(iconConfig).then(lang.hitch(this, function(widget) {
          ...
          switch (name) {
            ...
            case "Search":
              ...
              break;
            case "Navigate":
              domConstruct.place(widget.domNode, this.navigateNode);
              domAttr.set(this.navigateNode, "settingId", iconConfig.id);
              domStyle.set(this.navigateDiv, "display", "block");
          }
      ...
      _showIconContent: function(iconConfig) {
        if (iconConfig.inPanel === false) {
          ...
          this.widgetManager.loadWidget(iconConfig).then(lang.hitch(this, function(widget) {
            ...
            if (name !== "OverviewMap") {
              widget.setPosition(this.wPos, this.sceneView.map.id);
            }
            this.widgetManager.openWidget(widget);
