...
define([
  ...
  function (lang, on, html, utils, jimuUtils, keys) {
    var mo = {};
    ...
    mo.a11y_init = function () {
      ...
      html.setAttr(this.noTimeContentNode, "tabindex", 0);
      ...
      this.a11y_firstNode = this.noTimeContentNode;
      this.a11y_lastNode = this.noTimeContentNode;//this.closeBtn;

      jimuUtils.initFirstFocusNode(this.domNode, this.a11y_firstNode);
      jimuUtils.initLastFocusNode(this.domNode, this.a11y_lastNode);

      this.a11y_openForFirstTime = true;
    };

    //async data
    mo.a11y_update = function () {
      ...
      setTimeout(lang.hitch(this, function () {
        if(this.a11y_openForFirstTime && this.openAtStart){
          this.openAtStartAysn = true;
        }

        if (jimuUtils.isAutoFocusFirstNodeWidget(this)) {
          //console.log("1111111 has data focus");
          this.asyncFocusPlayBtn();// skip first time focus when openAtStart
        }

        this.a11y_openForFirstTime = false;
      }), 50);
    };
    mo.asyncFocusPlayBtn = function () {
      this.playBtn.focus();// play/pause
      this.openAtStartAysn = false;
    };

    //sync&async data
    mo.a11y_open = function (hasTimeLayer) {
      if(!(this.a11y_openForFirstTime && this.openAtStart)) {
        setTimeout(lang.hitch(this, function () {
          if ("none" !== html.getStyle(this.noTimeContentNode, 'display')) {
            //console.log("00000 no data focus");
            this.noTimeContentNode.focus();//no time data
          }
        }), 50);
      }
      ...
      if (jimuUtils.isInNavMode() && !utils.isRunInMobile()) {
    ...
    mo.a11y_initEvents = function () {
      ...
      this.own(on(this.noTimeContentNode, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ESCAPE) {
          evt.stopPropagation();
          this.closeBtn.focus();
        }
