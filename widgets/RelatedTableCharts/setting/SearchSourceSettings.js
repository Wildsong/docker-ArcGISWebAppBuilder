...
define([
  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin, Evented], {
    baseClass: 'jimu-widget-RelatedTableCharts-search-source-settings',
    ...
    postCreate: function () {
      ...
      this.shelter.show(this.nls.loading);
    ...
    getConfig: function () {
      ...
      if(this._currentSourceSetting) {
        config.inputValidationStatus = this._currentSourceSetting.validateAllNumberInputs();
        if (!config.inputValidationStatus.isInputValid) {
          return config;
        }
