...
define([
  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    postMixInProperties: function () {
      ...
      this.nls = lang.mixin(this.nls, window.jimuNls.loadingShelter);
    },
    ...
    getConfig: function () {
      ...
      if (searchSources.hasOwnProperty('inputValidationStatus') &&
        (!searchSources.inputValidationStatus.isInputValid)) {
        this._errorMessage(searchSources.inputValidationStatus.validationMessage, this.nls.searchSourceSettingLabel);
        return false;
      }
      ...
      if (!searchSources || searchSources.sources.length === 0) {
        this._errorMessage(this.nls.searchSourceSetting.invalidSearchSources,
          this.nls.searchSourceSettingLabel);
        return false;
      }
      config = {
        "charts": [],
    ...
    _createChartSettings: function (tr) {
      ...
      this.own(on(this.currentChartSettings, 'showLoadingIndicator',
        lang.hitch(this, function () {
          this.loadingIndicator.show(this.nls.loading);
        })));
    ...
    _initTabs: function () {
      ...
      // to handle RTL styling for text boxes
      if (jimuUtils.has('chrome')) {
        domClass.add(this.tab.containerNode, "esriRTLTabContainerNode");
      }
      this.own(on(this.tab.containerNode, "scroll",
        lang.hitch(this, function () {
          if (this.currentChartSettings) {
            this.currentChartSettings.closeColorPicker();
          }
        })
      ));
    },
