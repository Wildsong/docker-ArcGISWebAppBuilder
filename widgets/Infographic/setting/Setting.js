...
define([
  ...
  function(on, query, Deferred, declare, BaseWidgetSetting, _WidgetsInTemplateMixin, html, lang,
    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      startup: function() {
        ...
        if (dataSource) {
          ...
          this.updateChooseTemplateText(this.config);
        } else {
      ...
      _setDataSource: function(dataSource) {
        ...
        this._preprocessingDataSource(dataSource).then(function() {
          ...
          this.shelter.show();
          this.igUtils.getFeaturesForSetting(dataSource).then(function(features) {
            this.shelter.hide();
            this.features = features;
            this._afterFetchFeatures();
      ...
      _afterProcessDataSource: function(dataSource) {
        ...
        this._setDataSourceForMainDijit(dataSource); //dataSource
        this._setLayerInfoForMainDijit(); //layer definition...
        this._setLayerInfoForMainDijitSetting();
        this._setConfigToMainDijitSetting(); //config
        this._setDataSourceForMainDijitSettting(dataSource);
      },
      ...
      _afterFetchFeatures: function() {
        //now just called by chart type
        if (!this.mainDijit || !this.mainDijitSetting) {
          return;
        }
        if (this.mainDijit.type !== 'chart') {
          this._setFeaturesToMainDijit(this.features);
          this.mainDijit.startRendering(); //Try rendering for the first time.
        } else {
          this._setFeaturesToMainDijitSetting(this.features);
          this._setFeaturesToMainDijit(this.features);
          this.mainDijitSetting.render().then(function() {
            this.mainDijit.startRendering(); //Try rendering for the first time.
          }.bind(this), function(error) {
            console.error(error);
            this.mainDijit.showNodata();
      ...
      _setDataSourceForMainDijitSettting: function(dataSource) {
      ...
      _setLayerInfoForMainDijit: function() {
        if (this.mainDijit) {
      ...
      _setLayerInfoForMainDijitSetting: function() {
        if (this.mainDijitSetting) {
          this.mainDijitSetting.setLayerDefinition(this.definition);
          this.mainDijitSetting.setLayerObject(this.layerObject, this.popupInfo);
        }
      },

      _setFeaturesToMainDijit: function(features) {
        if (!this.mainDijit) {
          return;
        }
        if (this.mainDijit.type === 'gauge') {
          this.mainDijit.setMainFeatures(features);
        } else {
      ...
      _setFeaturesToMainDijitSetting: function(features) {
        if (!this.mainDijitSetting) {
          return;
        }
      ...
      _preprocessingDataSource: function(dataSource) {
        var deferred = new Deferred();
        var def = this.igUtils.preprocessingDataSourceForSetting(dataSource);
        def.then(function(res) {
          if (res) {
            this.layerObject = res.layerObject;
            this.popupInfo = res.popupInfo;
            this.featureLayerForFrameWork = res.featureLayerForFrameWork;
      ...
      _setDefaultbgColor: function(dijitJson, bgColor) {
        ...
        switch (dijitJson.type) {
          ...
          case 'number':
            ...
            break;
          case 'chart':
            var display = dijitConfig.display;
            if (display && !display.backgroundColor) {
      ...
      _setDefaultTextColor: function(dijitJson, color) {
        ...
        switch (dijitJson.type) {
          ...
          case 'number':
            ...
            break;
          case 'chart':
            this._settingChartDefaultTextColor(dijitConfig, color);
            break;
          case 'gauge':
            break;
        }
      ...
      _settingChartDefaultTextColor: function(chartConfig, color) {
        if (!chartConfig) {
          return;
        }
        ...
        try {
          if (!display.legend.textStyle.color) {
            display.legend.textStyle.color = color;
          }
          if (data.type === 'pie') {
            if (!display.dataLabel.textStyle.color) {
              display.dataLabel.textStyle.color = color;
            }
          } else {
            if (!display.xAxis.textStyle.color) {
              display.xAxis.textStyle.color = color;
            }
            if (!display.xAxis.nameTextStyle.color) {
              display.xAxis.nameTextStyle.color = color;
            }
            if (!display.yAxis.textStyle.color) {
              display.yAxis.textStyle.color = color;
            }
            if (!display.yAxis.nameTextStyle.color) {
              display.yAxis.nameTextStyle.color = color;
            }
          }
        } catch (error) {
          console.error(error);
      ...
      _isDijitSupportServerStat: function(chartConfig) {
        ...
        return chartConfig.mode === 'feature' ? false : !chartConfig.isDataType;
      },

      _updateFilterByExtentState: function(dataSource) {
        if (dataSource.dataSourceType === 'DATA_SOURCE_FEATURE_LAYER_FROM_MAP') {
          var chartConfig = this._getChartConfigProperties();
      ...
      _getChartConfigProperties: function() {
        var mainDijitJson = this._getMainDijitJson();
        if (mainDijitJson && mainDijitJson.type === 'chart') {
          var config = mainDijitJson.config;
          var data = {};
          if (config && config.data) {
            data.mode = config.data.mode || 'feature';
            data.isDataType = !!config.data.dateConfig;
          }
          if (!data.mode) {
            data.mode = 'feature';
          }
      ...
      _getTemplateJson: function() {
        ...
        template.config.name = template.name;
      ...
      updateChooseTemplateText: function(config) {
        var configured = config && config.dijits;
        var text = configured ? this.nls.changeTemplate : this.nls.chooseTemplate;
        this.chooseTemplateBtn.innerHTML = text;
        this.chooseTemplateBtn.title = text;
      },

      destroyDijits: function() {
        if (this.dijitSettings && this.dijitSettings.length) {
          this.dijitSettings.forEach(function(dijit) {
            dijit.destroy();
          });
        }
        if (this.dijits && this.dijits.length) {
