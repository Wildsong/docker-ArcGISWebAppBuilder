...
define([
  ...
  var clazz = declare([BaseDijit], {
    templateString: '<div tabindex="0" class="has-link" role="link" style="height:100%;width:100%;">' +
      ...
      '<a tabindex="-1" data-dojo-attach-point="numberContent" class="number-content no-trim" target="_blank">' +
    ...
    _setDataFormat: function(config) {
      ...
      if (dataFormat.prefix) {
        this.prefix.innerHTML = jimuUtils.sanitizeHTML(dataFormat.prefix);
      }
      if (dataFormat.suffix) {
        this.suffix.innerHTML = jimuUtils.sanitizeHTML(dataFormat.suffix);
      }
    },
