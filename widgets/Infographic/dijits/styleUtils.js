...
define([
  ...
  'dojo/_base/html',
  'jimu/utils'
], function (on, keys, lang, html, utils) {
  var mo = {};
  //1. background
  mo.background = {
    ...
    setLink: function (config, style, that) {
      if (that.clickHanlder) {
        ...
        if (this.keydownHanlder && that.keydownHanlder[0]) {
          that.keydownHanlder[0].remove();
        }
        that.keydownHanlder = null;
      ...
      if (config.link) {
        ...
        that.keydownHanlder = that.own(on(that.domNode, 'keydown', lang.hitch(that, function() {
          if (event.keyCode === keys.ENTER) {
            window.open(config.link);
          }
        })));
        style.cursor = "pointer";
        html.addClass(that.domNode, 'has-link');
        html.setAttr(that.domNode, 'tabindex', "0");
        html.setAttr(that.domNode, 'role', "link");
        if ("undefined" !== typeof config.alt) {
          html.setAttr(that.domNode, 'aria-label', config.alt);
        }
      }else{
        html.removeClass(that.domNode, 'has-link');
        html.removeAttr(that.domNode, 'tabindex');
        html.removeAttr(that.domNode, 'role');
        if ("undefined" === typeof config.alt) {
          html.removeAttr(that.domNode, 'aria-label');
        }
      }
    }
  };
  ...
  mo.font = {
    setText: function (config, that) {
      ...
      that.innerDom.innerHTML = utils.sanitizeHTML(text);
    },
