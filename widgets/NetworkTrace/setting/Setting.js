...
define([
  ...
  return declare([BaseWidgetSetting, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    postCreate: function () {
      this.tab = null;
      this._summarySettingInstance = null;
      ...
      setTimeout(lang.hitch(this, function () {
        if (this.config && this.config.geoprocessing && this.config
          ...
          this._validateGPServiceURL(true);
        }
      }), 200);
      this._initLoading();
    },
    ...
    _createProjectSettings: function () {
      ...
      this.own(on(this._projectSettingInstance, "settingsChanged",
        lang.hitch(this, function (projectSettings) {
          this._summarySettingInstance.onProjectSettingsChanged(projectSettings);
        })));
     ...
     **/
    _initTabs: function () {
      var projectSettingTab, inputOutputTab, summaryTab, tabs;
      projectSettingTab = {
      ...
      tabs = [projectSettingTab, inputOutputTab, summaryTab];
     ...
     * @memberOf widgets/isolation-trace/settings/settings.js
     */
    _validateGPServiceURL: function (restoringFromConfig) {
      this.gpServiceTasks = [];
        ...
        esriRequest(requestArgs).then(lang.hitch(this, function (
          ...
          if (response.hasOwnProperty("name")) {
            ...
            if (response.name !== null) {
              ...
              if (gpTaskParameters) {
                this._validateGpTaskResponseParameters(gpTaskParameters, restoringFromConfig);
              }
            }
          } else {
     ...
     * @memberOf widgets/isolation-trace/settings/settings.js
     */
    _validateGpTaskResponseParameters: function (gpTaskParameters, restoringFromConfig) {
      var i, recordSetValCheckFlag = true,
        inputParametersArr = [],
        inputGPParamFlag = true,
        hasPolygonOutputGeometry = false,
        hasOutputParameterWithGeometryType = false,
        errMsg;
      ...
      for (i = 0; i < gpTaskParameters.length; i++) {
        ...
        //validate if atleast one output parameter with valid Geometry type is present
        //and atleast one output parameter polygon geometry type is present
        if (gpTaskParameters[i].direction === "esriGPParameterDirectionOutput" &&
          gpTaskParameters[i].dataType === "GPFeatureRecordSetLayer" &&
          gpTaskParameters[i].defaultValue && gpTaskParameters[i].defaultValue.geometryType) {
          hasOutputParameterWithGeometryType = true;
          if (gpTaskParameters[i].defaultValue.geometryType === "esriGeometryPolygon") {
            hasPolygonOutputGeometry = true;
          }
        }
      }
      ...
      if (recordSetValCheckFlag && inputGPParamFlag && hasOutputParameterWithGeometryType) {
        this.validConfig = true;
        //if service is having polygon outut param hide the warning else show it
        if (hasPolygonOutputGeometry) {
          domClass.add(this.projectSettingWarning, "esriCTHidden");
        } else {
          domClass.remove(this.projectSettingWarning, "esriCTHidden");
        }
        this._showTaskDetails(gpTaskParameters, hasPolygonOutputGeometry, restoringFromConfig);
        this.loading.hide();
      } else {
        this._refreshConfigContainer();
        this.loading.hide();
        ...
        } else if (!hasOutputParameterWithGeometryType) {
          errMsg = this.nls.noOutputParameterWithGeometryType;
        }
     ...
     * @memberOf widgets/isolation-trace/settings/settings.js
     */
    _showTaskDetails: function (gpTaskParameters, hasPolygonOutputGeometry, restoringFromConfig) {
      var i;
      ...
      //enable disable project settings based on hasPolygonOutputGeometry
      this._projectSettingInstance.enableDisableProjectSettings(hasPolygonOutputGeometry);
      //also show error message in popup when no output param with poygon geometry
      if (!hasPolygonOutputGeometry && !restoringFromConfig) {
        this._errorMessage(this.nls.invalidOutputGeometry);
      }
      ...
      var inputOutputSettingsParams = {
        ...
        "outputConfig": this.outputSettingArray,
    ...
    _displaySummaryExpressionBuilder: function () {
      ...
      this.config.displayTextForRunButton = this.nls.summaryTab.displayTextforButtonDefaultValue;
      param = {
        "nls": this.nls,
        "config": this.config,
        ...
        "inputDataTypes": this.inputDataTypes,
        "projectSettings": this.config.projectSettings,
        "hasSkipLocationsInput": this._inputOutputSettings.hasSkipLocationsInput
      };
      this._summarySettingObjArr = [];
      this._summarySettingInstance = new SummarySetting(param, domConstruct.create("div", {}, this.summaryTextData));
      this._summarySettingInstance.startup();
      this._summarySettingObjArr.push(this._summarySettingInstance);
    },
          ...
          function (outputParameters, k) {
            ...
            if (!outputConfig) {
              outputConfig = {
                ...
                "displayText": this.nls.summaryTab.enterDisplayText,
                "MinScale": 0,
                "MaxScale": 0,
                "exportToCSV": false,
                "saveToLayer": ""
            ...
            this.outputSettingArray.push(outputConfig);
          }));
      }
    },
        ...
        array.forEach(this._summarySettingObjArr, lang.hitch(this,
          function (widgetNode) {
            if (widgetNode) {
              ...
              this.config.autoZoomAfterTrace = widgetNode.getAutoZoomAfterTraceConfigData();
              if (this.nls.summaryTab.displayTextforButtonDefaultValue) {
                this.config.displayTextForRunButton = this.nls.summaryTab.displayTextforButtonDefaultValue;
              }
            }
          }));
    ...
    _getOutputConfigParameters: function () {
      ...
      if (this.config && this.config.geoprocessing && this.config.geoprocessing.outputs) {
        ...
        // if input param object created
        if (this._summarySettingInstance) {
          this.config.geoprocessing.outputs = this._summarySettingInstance.getOutputSettings();
        }
    ...
    _getOtherConfigParameters: function () {
      ...
      if (!this._summarySettingObjArr && !this.config.SummaryExpression.autoZoomAfterTraceCheckedState) {
        this.config.autoZoomAfterTrace = this.config.SummaryExpression.autoZoomAfterTraceCheckedState;
      }
    },
    ...
    getConfig: function () {
      ...
      if (this.validConfig) {
        ...
        if (validateInputTask) {
          ...
          // Get input tab summary text
          this._getInputTabSummaryText();
        } else {
          return false;
        }
      } else {
    ...
    /**
     * This function is used to get the input tab summary expression
     */
    _getInputTabSummaryText: function () {
      if (this._inputOutputSettings !== '' &&
        this._inputOutputSettings !== null &&
        this._inputOutputSettings !== undefined) {
        var summaryText = this._inputOutputSettings.getInputSummaryText();
        this.config.inputSummaryText = summaryText;
      }
    }
  });
});