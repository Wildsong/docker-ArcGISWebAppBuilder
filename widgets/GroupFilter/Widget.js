...
function(declare, _WidgetsInTemplateMixin, BaseWidget, dijit, FilterParameters, dom,
  domConstruct, domClass, domAttr, domStyle, on, query, string, lang, array, locale, keys, Deferred, topic, Select, TextBox,
  DateTextBox, NumberTextBox, registry, LayerInfos, utils, FilterManager, EditorXssFilter, Query, QueryTask,
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    postCreate: function() {
      ...
      if(this.config.hasOwnProperty("headerText")) {
        if(this.config.headerText !== "") {
          this.groupSelectHeader.innerHTML = this.editorXssFilter.sanitize(this.config.headerText);
        }
      }
    ...
    loadAllURLParams: function(layer) {
      if(this.urlParameters !== null && this.urlParameters !== "") {
        ...
        var groupInMatrix = array.forEach(this.groupMatrix, lang.hitch(this, function(grp){
          w2wData = [];
          fieldList = [];
          valueList = [];
          if(keys.indexOf(grp.name) > -1) {
            array.forEach(grp.layers, lang.hitch(this, function(lyr, i){
              if(lyr.id === layer.id) {
    ...
    populateGroupState: function() {
      ...
      array.map(this.config.groups, lang.hitch(this, function(grp){
        var layerGroup = [];
        array.forEach(grp.layers, lang.hitch(this, function(lyr) {
          var exp = "";
          var val = [];
          var field = "";
          if(this.urlParameters !== null && this.urlParameters !== "") {
            ...
            if(keys.indexOf(grp.name) > -1) {
              ...
              if(item.length > 0) {
                if(item.length === 1) {
                  ...
                  array.forEach(item, lang.hitch(this, function(a, i) {
                    if(i === item.length -1) {
                      exp = exp + " (" + lyr.field + " = '" + utils.stripHTML(a) + "')";
                      val.push(utils.stripHTML(a));
                      field = lyr.field;
                    } else {
                      exp = exp + " (" + lyr.field + " = '" + utils.stripHTML(a) + "') " + this.config.slAppendChoice;
                      val.push(utils.stripHTML(a));
                      field = lyr.field;
                    }
                  }));
    ...
    btnNewRowAction: function() {
      var currGroup = array.filter(this.config.groups, lang.hitch(this, function(grp) {
      ...
      if(currGroup.length > 0) {
        ...
        this.createNewRow({operator:defaultOp, value:defaultVal, conjunc:"OR", state:"new", group:currGroup[0].name});
      } else {
        var defaultVal = this.checkDefaultValue(this.config.groups[0]);
        var defaultOp = this.checkDefaultOperator(this.config.groups[0]);
        this.createNewRow({operator:defaultOp, value:defaultVal, conjunc:"OR", state:"new", group:this.config.groups[0].name});
      }
    },
            ...
            array.forEach(this.layerList, lang.hitch(this, function(layer) {
              if((layer.originOperLayer.layerType !== "ArcGISTiledMapServiceLayer" &&
                ...
                if(typeof(layer.layerObject._defnExpr) !== 'undefined') {
                  ...
                  this.defaultDef.push({layer: layer.id, definition: "", visible: ""});
                }
    ...
    createGroupSelection: function() {
      ...
      if(this.urlParameters !== null && this.urlParameters !== "") {
        var filteredGrp = array.filter(this.config.groups, lang.hitch(this, function(grp) {
      ...
      if(this.urlParameters !== null && this.urlParameters !== "") {
        var groupList = array.map(this.config.groups, lang.hitch(this, function(grp) {
          return grp.name;
        }));
        var keys = Object.keys(this.urlParameters);
        var urlConfigMatch = (JSON.stringify(groupList) === JSON.stringify(keys))?true:false;
    ...
    createOperatorSelection: function(pCell, pValue) {
      var ObjList = [
        {'value': '=', 'label': this.nls.inputs.optionEQUAL, "aria-label":this.nls.inputs.optionEQUAL},
        {'value': '<>', 'label': this.nls.inputs.optionNOTEQUAL, 'aria-label': this.nls.inputs.optionNOTEQUAL},
        {'value': '>', 'label': this.nls.inputs.optionGREATERTHAN, 'aria-label': this.nls.inputs.optionGREATERTHAN},
        {'value': '>=', 'label': this.nls.inputs.optionGREATERTHANEQUAL, 'aria-label': this.nls.inputs.optionGREATERTHANEQUAL},
        {'value': '<', 'label': this.nls.inputs.optionLESSTHAN, 'aria-label': this.nls.inputs.optionLESSTHAN},
        {'value': '<=', 'label': this.nls.inputs.optionLESSTHANEQUAL, 'aria-label': this.nls.inputs.optionLESSTHANEQUAL},
        {'value': 'START', 'label': this.nls.inputs.optionSTART, 'aria-label': this.nls.inputs.optionSTART},
        {'value': 'END', 'label': this.nls.inputs.optionEND, 'aria-label': this.nls.inputs.optionEND},
        {'value': 'LIKE', 'label': this.nls.inputs.optionLIKE, 'aria-label': this.nls.inputs.optionLIKE},
        {'value': 'NOT LIKE', 'label': this.nls.inputs.optionNOTLIKE, 'aria-label': this.nls.inputs.optionNOTLIKE},
        {'value': 'd<=', 'label': this.nls.inputs.optionONORBEFORE, 'aria-label': this.nls.inputs.optionONORBEFORE},
        {'value': 'd>=', 'label': this.nls.inputs.optionONORAFTER, 'aria-label': this.nls.inputs.optionONORAFTER}
      ];
                ...
                if(grpLayer.layer === layer.id) {
                  if(grpLayer.useDomain === true) {
                    this._callFilterDijit(grpLayer, pValue, layer.layerObject.url,
                      layer.layerObject.toJson().layerDefinition, parts, filter, pDijit);
                  }
                }
              }
            }));
    ...
    removeTableRow: function(pCell, pRow, pCount) {
      if(pCount > 1) {
        ...
        this.own(on(dsNode, 'keydown', lang.hitch(this, function (evt) {
    ...
    reconstructRows: function(pValue) {
      if(pValue !== "") {
            ...
            if(typeof(group.def) !== 'undefined') {
              if(group.def.length > 0) {
                array.forEach(group.def, lang.hitch(this, function(def) {
                  this.createNewRow({value: def.value, operator: def.operator, conjunc: def.conjunc, state:"reload", group: group.name});
                }));
              } else {
                defaultVal = this.checkDefaultValue(group);
                defaultOp = this.checkDefaultOperator(group);
                this.createNewRow({operator:defaultOp, value:defaultVal, conjunc:"OR", state:"new", group: group.name});
              }
            } else {
              if(this.urlParameters !== null && this.urlParameters !== "") {
        ...
        this.createNewRow({operator:"=", value:"", conjunc:"OR", state:"new", group: this.grpSelect.value});
      }
    },
    ...
    _publishData: function(pValue, pCurrGroup) {
      ...
      var newRec = true;
      array.forEach(this.w2wHistory, lang.hitch(this, function(w) {
    ...
    _applyLayerFilter: function(layer, layerId, exp, howAppend, destroy) {
      ...
      FilterManager.getInstance().applyWidgetFilter(layerId, this.id, exp, checked, howAppend, this.config.zoomMode);
      if(!destroy) {
    ...
    addCriteriaByURL: function(param) {
      if(this.urlParameters !== null && this.urlParameters !== "") {
        ...
        if(typeof(group) !== "undefined") {
          if(this.config.simpleMode){
            if(group.length > 0) {
              group.forEach(lang.hitch(this, function(g,i){
              ...
              this.createNewRow({operator:defaultOp, value:defaultVal, conjunc:"OR", state:"new", group:param.name});
            }
          } else {
            group.forEach(lang.hitch(this, function(g,i){
              on.emit(this.btnCriteria, "click", {
    ...
    handleKeyDownEvents: function(){
      this.own(on(this.btnCriteria, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          this.btnNewRowAction();
        }
      })));
      this.own(on(this.btnApply, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          this.setFilterLayerDef();
        }
      })));
      this.own(on(this.btnReset, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
      ...
      this.own(on(this.optionsIcon, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          this.toggleSaveFilter();
        }
      })));
      this.own(on(this.closeSaveIcon, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          this.toggleSaveFilter();
        }
      })));
      this.own(on(this.refreshDataIcon, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          this.refreshData();
        } else if (evt.keyCode === keys.TAB) {
          this.closeSaveIcon.focus();
      ...
      this.own(on(this.chkPersistDef, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
      ...
      this.own(on(this.btnSaveToJson, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          this.saveJsonToFile();
        }
      })));
    ...
    registerEventHandler: function() {
      this.own(on(this.optionsIcon, 'click', lang.hitch(this, function (evt) {
        this.toggleSaveFilter();
      })));
      this.own(on(this.closeSaveIcon, 'click', lang.hitch(this, function (evt) {
        this.toggleSaveFilter();
      })));
    },
