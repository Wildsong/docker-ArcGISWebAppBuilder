  ...
  function (
    ...
    return declare([BaseWidgetSetting, Evented, _TemplatedMixin], {
      ...
      _createPresetContents: function () {
        ...
        //Loop though the preset order to ensure the order matches the configuration order 
        for (var i = 0; i < this.presetOrder.length; i++) {
          presetGroup = this.presetOrder[i];
          var row, groupInfo;
      ...
      _createDateTypeDOM: function (presetGroup, fieldNode) {
        ...
        this.own(on(editIcon, "click", lang.hitch(this, function () {
          var relativeDatesObj = new RelativeDates({
            ...
            selectedPresetValue: presetGroup.presetValue && presetGroup.presetValue.chooseFromLayerInfo ?
              (presetGroup.presetValue.chooseFromLayerInfo.selectedValue) : ""
          });
      ...
      _createStringTypeDOM: function (presetGroup, fieldNode) {
        var stringTextBox = new Textarea({
          row: 1,
        ...
        this.own(on(stringTextBox, "change", lang.hitch(this, function (newValue) {
          //Check if current value is not same as the previous value
          //If yes, the only proceed further
          if (presetGroup.presetValue !== newValue) {
            presetGroup.presetValue = newValue;
            this.emit("presetValueChanged");
          }
        })));
