  ...
  function (declare,
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      postCreate: function () {
        ...
        if (this.pageInstructionText) {
          this.pageInstruction.innerHTML = this.pageInstructionText;
        }
        domAttr.set(this.schemaMapInstructions, "aria-label", utils.stripHTML(this.pageInstruction.innerHTML));
        //Handled browse to file keydown event
        this.own(on(this.uploadLabel, "keydown", lang.hitch(this, function (evt) {
        ...
        this.own(on(this.creditsLink, "keydown", lang.hitch(this, function (evt) {
          if (evt.keyCode !== keys.ENTER && evt.keyCode !== keys.SPACE) {
            return;
          }
          this.creditsLinkTag.click();
        })));
        this.firstFocusNode = this.schemaMapInstructions;
        this.lastFocusNode = this.creditsLink;
      },
      ...
      startup: function () {
        ...
        if (this.disabled) {
          ...
          domAttr.set(this.uploadLabel, "tabindex", "-1");
        }
      ...
      onDrop: function (event) {
        if (this.disabled) {
          if (!this.parent._userCanEdit) {
            this.parent.showEditMessage();
          } else {
            var messages = this.parent._getMessages();
            this.parent._showPopup(messages);
          }
        } else {
          if (!this.inDrop) {
            this.inDrop = true;
      ...
      targetLayerUpdated: function () {
        if (this.fieldMappingView && this.csvStore) {
          //update editLayer
          this.csvStore.editLayer = this.parent.editLayer;
          //update current layers symbol
          this.csvStore.symbol = this.parent._symbol;
          //update fields of selected layer in csvStore
          this.csvStore.fsFields = lang.clone(this._fsFields);
          //find fields flagged to be used in duplicate search
          this.csvStore._getDuplicateFields(this.csvStore.fsFields);
          //call handle csv function
          this.csvStore.handleCsv().then(lang.hitch(this, function (obj) {
            var targetFields = obj.fsFields;
            var sourceFields = this._getFields(obj, false);
            //update field mapping fields table
            this.fieldMappingView.targetLayerUpdate(targetFields, sourceFields);
          }));
        }
      },
      ...
      _initCoordinatesView: function (obj) {
        return new Coordinates(lang.mixin({
          ...
          xField: this.config.xyFields[0].fieldName === 'X' ? this.config.xyFields[0] : this.config.xyFields[1],
          yField: this.config.xyFields[0].fieldName === 'Y' ? this.config.xyFields[0] : this.config.xyFields[1],
          firstField: this.config.xyFields[0].fieldName
        }, this._baseArgs));
      },
      ...
      _updatePageContainer: function (obj) {
        ...
        this.fieldMappingView = this._initFieldMappingView(obj);
        this.pageContainer.addView(this.fieldMappingView);
      ...
      _clearStore: function (resetContainer) {
        if (this.csvStore) {
          this.csvStore.clear();
        }
        ...
        this.fileForm.reset();
        if (resetContainer) {
          this.parent._initPageContainer(true);
          this.inDrop = false;
        }
      }
    });
