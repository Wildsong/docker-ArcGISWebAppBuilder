  ...
  function (declare,
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      postCreate: function () {
        ...
        //Add/remove styles to download button based on RTL mode
        if (window.isRTL) {
          domClass.add(this.progressNode, "margin-right-15");
        }
      },
      ...
      _addMatchedFeaturesList: function () {
        this.matchedFeatureList = this._initFeatureList(this.matchedList, this.matchedLayer,
          'MatchedFeatures', this.nls.review.reviewMatchedHint, this.nls.review.reviewMatched, false);
        ...
        this.own(on(this.matchedFeatureList, 'feature-list-updated', lang.hitch(this, function (v) {
          ...
          this._initReviewRow(this.matchedList, [this.matchedHintRow, this.matchedControlRow],
      ...
      _addUnmatchedFeatureList: function () {
        this.unMatchedFeatureList = this._initFeatureList(this.unMatchedList, this.unMatchedLayer,
          'UnMatchedFeatures', this.nls.review.reviewUnMatchedHint, this.nls.review.reviewUnMatched, false);
      ...
      _addDuplicateFeatureList: function () {
        var duplicateHint = esriLang.substitute({
          layerName: this.editLayer.name
        }, this.nls.review.reviewDuplicateHint);
        this.duplicateFeatureList = this._initFeatureList(this.duplicateList, this.duplicateLayer,
          'DuplicateFeatures', duplicateHint, this.nls.review.reviewDuplicate, true);
      ...
      _initFeatureList: function (features, layer, label, hint, title, isDuplicate) {
      ...
      _initReviewRows: function () {
        this._initReviewRow(this.matchedList,
          [this.matchedHintRow, this.matchedControlRow], this.matchedCount, "matched");
        this._initReviewRow(this.unMatchedList,
          [this.unMatchedHintRow, this.unMatchedControlRow], this.unMatchedCount, "unmatched");
        this._initReviewRow(this.duplicateList,
          [this.duplicateHintRow, this.duplicateControlRow], this.duplicateCount, "duplicate");
      },

      _initReviewRow: function (features, controlRows, countNode, rowLabel) {
        if (features.length > 0) {
          ...
          this._createOrUpdateReviewNodeAriaLabel(features.length, rowLabel);
        } else {
          array.forEach(controlRows, function (r) {
            if (!domClass.contains(r, 'display-none')){
      ...
      _createOrUpdateReviewNodeAriaLabel: function (totalFeatures, rowLabel) {
        var matchedStringWithCount, rowNode, headerString, hintString;
        //Based on the review section, get node and NLS strings
        switch (rowLabel) {
          case "matched":
            headerString = this.nls.review.reviewMatched;
            hintString = this.nls.review.reviewMatchedHint;
            rowNode = this.matchedControlRow
            break;
          case "unmatched":
            headerString = this.nls.review.reviewUnMatched;
            hintString = this.nls.review.reviewUnMatchedHint;
            rowNode = this.unMatchedControlRow;
            break;
          case "duplicate":
            headerString = this.nls.review.reviewDuplicate;
            hintString = this.nls.review.reviewDuplicateHint;
            rowNode = this.duplicateControlRow;
            break;
        }
        matchedStringWithCount = string.substitute(this.nls.review.reviewLabelWithCount, {
          totalMatchedFeatures: totalFeatures,
          headerString: headerString,
          headerStringHint: hintString
        });
        domAttr.set(rowNode, "aria-label", matchedStringWithCount);
      },
      ...
      _disableUI: function () {
        this._updateNode(this.submitButton, true);
        domClass.add(this.submitButton, 'control-disbaled');
        domAttr.set(this.submitButton, "tabindex", "-1");

        domClass.add(this.reviewRowTable, 'control-disbaled');
        domAttr.set(this.reviewDataRow, "tabindex", "-1");
        domAttr.set(this.matchedControlRow, "tabindex", "-1");
        domAttr.set(this.unMatchedControlRow, "tabindex", "-1");
        domAttr.set(this.duplicateControlRow, "tabindex", "-1");

        domClass.add(this.pageContainer.nextTd, 'control-disbaled');
        domAttr.set(this.pageContainer.nextImage, "tabindex", "-1");

        domClass.add(this.pageContainer.backTd, 'control-disbaled');
        domClass.add(this.pageContainer.backImage, 'control-disbaled');
        domAttr.set(this.pageContainer.backImage, "tabindex", "-1");
          ...
          array.forEach(featureLayer.graphics, lang.hitch(this, function (feature) {
            ...
            if (attrs.hasOwnProperty('hasDuplicateUpdates') && attrs.hasOwnProperty('duplicateState')) {
              if (attrs.hasDuplicateUpdates && attrs.duplicateState === 'make-change') {
                ...
                array.forEach(this.csvStore._matchFields, lang.hitch(this, function(matchField){
                  this._deleteProp(attrs, matchField);
                }));
                features.push(feature.toJson ? new Graphic(feature.toJson()) : feature);
              }
            } else {
              this._deleteProp(attrs, 'matchScore');
              this._deleteProp(attrs, 'DestinationOID');
      ...
      _reviewMatched: function (evt) {
        if (evt && evt.type === "keydown" && (evt.keyCode !== keys.ENTER && evt.keyCode !== keys.SPACE)) {
          return;
        }
        this.pageContainer.selectView(this._matchedListView.index);
      },

      _reviewUnMatched: function (evt) {
        if (evt && evt.type === "keydown" && (evt.keyCode !== keys.ENTER && evt.keyCode !== keys.SPACE)) {
          return;
        }
