  ...
  function (
    ...
    return declare([BaseWidget], {
      ...
      postCreate: function () {
        ...
        this._setFirstLastNodeInWidget();
      },
      ...
      */
      onClose: function () {
        ...
        if (this._mapTooltipHandler) {
          ...
          domAttr.set(this.newTraverseButton, "aria-pressed", "false");
          domAttr.set(this.editTraverseButton, "aria-pressed", "false");
      ...
      */
      ...
      /**
       * This function is called on click/keydown of new traverse button
       * @memberOf widgets/ParcelDrafter/Widget
       */
      _newTraverseButtonClicked: function () {
        this._mapTooltipHandler.disconnectEventHandler();
        ...
        if (domClass.contains(this.newTraverseButton, "esriCTNewTraverseActive")) {
          ...
          domAttr.set(this.newTraverseButton, "aria-pressed", "true");
          domAttr.set(this.editTraverseButton, "aria-pressed", "false");
          ...
          this._xyInputInstance.resetXYTextBoxes();
        }
        this._setFirstLastNodeInWidget();
      },

      /**
       * This function is called on click/keydown of edit traverse button
       * @memberOf widgets/ParcelDrafter/Widget
       */
      _editTraverseButtonClicked: function () {
        this._mapTooltipHandler.disconnectEventHandler();
        this._newTraverseInstance.clearAll();
        //check if button is active or not
        if (domClass.contains(this.editTraverseButton, "esriCTEditTraverseActive")) {
          ...
          domAttr.set(this.editTraverseButton, "aria-pressed", "true");
          domAttr.set(this.newTraverseButton, "aria-pressed", "false");
      ...
      **/
      _handleClickEvents: function () {
        ...
        //code for accessibility
        this.own(on(this.planSettingsButton, "keydown", lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            this._showPanel("planSettingsPage");
          }
        })));
        //handle start traverse button click
        this.own(on(this.newTraverseButton, "click", lang.hitch(this, this._newTraverseButtonClicked)));

        // code for accessibilty
        this.own(on(this.newTraverseButton, "keydown", lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            this._newTraverseButtonClicked();
          }
        })));

        //handle edit traverse button click
        this.own(on(this.editTraverseButton, "click", lang.hitch(this, this._editTraverseButtonClicked)));

        // code for accessibility
        this.own(on(this.editTraverseButton, "keydown", lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            this._editTraverseButtonClicked();
          }
        })));
        ...
        //code for accessibility
        this.own(on(this.planSettingsPanelBackButton, "keydown", lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            this._planSettingsInstance.onClose();
            this._showPanel("traversePage");
            focusUtils.focus(this.planSettingsButton);
          }
        })));
        ...
        //code for accessibility
        this.own(on(this.traversePanelBackButton, "keydown", lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            this._confirmCancelTraverse();
          }
        })));
        ...
        confirmationBox = new Message({
          ...
          buttons: [{
            ...
            "onClick": lang.hitch(this, function () {
              ...
              this._resetOnBackToMainPage(true);
            })
      ...
      * @memberOf widgets/ParcelDrafter/Widget
      * @param {string} do not focus to new traverse button while returning back to main page
      * after doing save or delete parcel actions coz msg is showing on screen at that time
      * focusing to new traverse button screen reader will read its aria-label first then msg so it may confusing
      * so only set focus to new traverse button after click on back button in new traverse page
      */
      _resetOnBackToMainPage: function (setFocusToNewTraverseButton) {
        ...
        //code for accessibility
        if (setFocusToNewTraverseButton) {
          focusUtils.focus(this.newTraverseButton);
        }
      },
      ...
      * @memberOf widgets/ParcelDrafter/Widget
      */
      _onMapPointSelected: function (mapPoint, startNewTraverseBtnClick) {
        ...
        if (this._mapTooltipHandler.toolTipText === this.nls.mapTooltipForScreenDigitization) {
          ...
          if (!this._startPoint) {
            if (this._mapTooltipHandler.toolTipText === this.nls.mapTooltipForEditNewTraverse) {
              ...
              this._showPanel("traversePage");
              //code for accessibility
              focusUtils.focus(this.traversePanelBackButton);
          ...
          this._newTraverseInstance.setStartPoint(this._startPoint, true, true, startNewTraverseBtnClick);
          //code for accessibility
          focusUtils.focus(this.traversePanelBackButton);
        }
      },
      ...
      * @memberOf widgets/ParcelDrafter/Widget
      */
      _getPolygonForEdits: function (mapPoint, polygonQuery) {
        ...
        if (polygonLayer) {
          polygonLayer.queryFeatures(polygonQuery ? polygonQuery : featureQuery,
            lang.hitch(this, function (featureSet) {
              var i;
              this.loading.hide();
              //if parcel found at the selected location
              //proceed to get the lines and navigate to traverse page
              //else check if lines of unclosed parcels are intersecting the selected mapPoint
              if (featureSet && featureSet.features.length > 0) {
                ...
                if (!polygonQuery) {
                  this._getPolygonFromLinesForEdits(mapPoint);
                } else {
                  this._startPoint = null;
      ...
      /**
      * This function is used to get extent from point
      * @memberOf widgets/ParcelDrafter/Widget
      **/
      ...
      /**
      * This function is used to query line feature from mappoint and
      * initiate edit workflow if for unclosed parcels
      * @memberOf widgets/ParcelDrafter/Widget
      **/
      _getPolygonFromLinesForEdits: function (mapPoint) {
        var featureQuery, polylineLayer, currentDateTime;
        this.loading.show();
        currentDateTime = new Date().getTime();
        featureQuery = new Query();
        polylineLayer = this.map.getLayer(this.config.polylineLayer.id);
        //create extent of 5px tolerance around the point
        featureQuery.geometry = this.pointToExtent(this.map, mapPoint, 5);
        featureQuery.outSpatialReference = this.map.spatialReference;
        featureQuery.returnGeometry = false;
        featureQuery.outFields = ["*"];
        featureQuery.where = currentDateTime + "=" + currentDateTime;
        ...
        if (polylineLayer) {
          polylineLayer.queryFeatures(featureQuery, lang.hitch(this, function (featureSet) {
            this.loading.hide();
            //if parcel lines found at the selected location
            //get GUID and create query to get unclosed polygon
            //else show error msg
            if (featureSet && featureSet.features.length > 0 &&
              featureSet.features[0].attributes &&
              featureSet.features[0].attributes[this.config.polylineLayer.relatedGUID.name]) {
              var guid, polygonQuery = new Query(), curDateTime = new Date().getTime();
              //get guid from the first line feature
              //Note: currently we are supporting only first feature when multiple intersected
              guid =
                featureSet.features[0].attributes[this.config.polylineLayer.relatedGUID.name];
              //create a where clause to fetch unclosed parcel of intersecting lines
              polygonQuery.where = curDateTime + "=" + curDateTime + " and " +
                this.config.polygonLayer.relatedGUID.name + "='" + guid + "'";
              //call get polygon for edits it will initiates edit workflow for unclosed polygon
              this._getPolygonForEdits(mapPoint, polygonQuery);
            } else {
              this._startPoint = null;
              this._showMessage(this.nls.unableToFetchParcelMessage);
            }
      ...
      **/
      _createNewTraverse: function () {
        ...
        //code for accessibility
        this.own(this._newTraverseInstance.on("showParcelSavedAndDeleteMsg",
          lang.hitch(this, this._showParcelSavedAndDeleteMsg)));
        ...
        this.own(this._newTraverseInstance.on("displayMainPageAfterSave",
          lang.hitch(this, function (setFocusToNewTraverseButton) {
            this._resetOnBackToMainPage(setFocusToNewTraverseButton);
          })));
     ...
     **/
      ...
      _createXYInput: function () {
        ...
        this.own(this._xyInputInstance.on("newPoint",
          lang.hitch(this, function (point) {
            this._onMapPointSelected(point, true);
          })));
      ...
      **/
       ...
       * @memberOf widgets/ParcelDrafter/Widget
       */
      _setFirstLastNodeInWidget: function () {
        //main page
        if (!domClass.contains(this.mainPageNode, "esriCTHidden") &&
          domClass.contains(this.newTraverseSelectMessageNodeWrapper, "esriCTHidden")) {
          jimuUtils.initFirstFocusNode(this.domNode, this.newTraverseButton);
          jimuUtils.initLastFocusNode(this.domNode, this.editTraverseButton);
          return;
        }

        //main page with start new traverse button
        if (!domClass.contains(this.mainPageNode, "esriCTHidden") &&
          !domClass.contains(this.newTraverseSelectMessageNodeWrapper, "esriCTHidden")) {
          jimuUtils.initFirstFocusNode(this.domNode, this.newTraverseButton);
          this._xyInputInstance._updateLastFocusNode();
          return;
        }

        //traverse page
        if (!domClass.contains(this.traversePageNode, "esriCTHidden")) {
          jimuUtils.initFirstFocusNode(this.domNode, this.traversePanelBackButton);
          this._newTraverseInstance.updateLastNodeInTraversePanel();
          return;
        }

        //plansettings page
        if (!domClass.contains(this.planSettingsPageNode, "esriCTHidden")) {
          jimuUtils.initFirstFocusNode(this.domNode, this.planSettingsPanelBackButton);
          focusUtils.focus(this.planSettingsPanelBackButton);
      ...
      /**
       * This function is used to show message related to parcel save and delete messages
       * code for accessibility
       * @memberOf widgets/ParcelDrafter/Widget
       */
