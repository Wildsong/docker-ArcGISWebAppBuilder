    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      postCreate: function () {
        ...
        this.own(on(this.hidePanelOptions, 'change', lang.hitch(this, function (v) {
          this.hidePanel = v;
          this.panelCountOptions.setStatus(!v);
          this.panelIconOptions.setStatus(!v);
          this.expandListOptions.setStatus(!v);
          this.showAllFeaturesOptions.setStatus(!v);
          ...
          if (v) {
            ...
            if (this.panelIconOptions.getValue()) {
        ...
        this.own(on(this.panelIconOptions, 'change', lang.hitch(this, this._updatePanelIcon)));
      },

      _updatePanelIcon: function () {
        this.displayPanelIcon = this.panelIconOptions.getValue();
        var containerClass = this.displayPanelIcon ? 'mainPanelPreviewContainerOff' : 'mainPanelPreviewContainerOn';
        var previewContainer = query('.' + containerClass, this.mainPanelPreviewContainer.domNode)[0];
        if (previewContainer) {
          html.removeClass(previewContainer, containerClass);
          html.addClass(previewContainer, this.displayPanelIcon ?
            'mainPanelPreviewContainerOn' : 'mainPanelPreviewContainerOff');
        }
      },

      _initCheckBoxes: function () {
        this.panelCountOptions = this._initCheckBox(this.nls.panelCountOption, this.div_panelCountOptions);
        this.panelIconOptions = this._initCheckBox(this.nls.panelIconOption, this.div_panelIconOptions);
        this.expandListOptions = this._initCheckBox(this.nls.expandList, this.div_expandListOptions);
        this.showAllFeaturesOptions = this._initCheckBox(this.nls.showAllFeatures, this.div_showAllFeaturesOptions);
        this.hidePanelOptions = this._initCheckBox(this.nls.hidePanel, this.div_hidePanelOptions);
        // code added for display cluster option
        this.displayClusterLayerOptions = this._initCheckBox(this.nls.displayClusterLayer,
          this.div_displayClusterLayerOptions);
        if(!this.config.hasOwnProperty("displayClusterLayer") || this.config.displayClusterLayer) {
          this.displayClusterLayerOptions.check();
        }
        else if(!this.config.displayClusterLayer) {
          this.displayClusterLayerOptions.uncheck();
        }
      },

      _initCheckBox: function (nls, node) {
        var cb = new _CheckBox();
        cb.setLabel(nls);
        cb.placeAt(node);
        return cb;
      },
      ...
      _setLayers: function (layerInfos) {
            ...
            if (rootLayerIDs.indexOf(rootLayerInfo.id) === -1) {
              ...
              rootLayerInfo.traversal(lang.hitch(this, function (subLayerInfo) {
                ...
                if (subLayerInfo.newSubLayers && subLayerInfo.newSubLayers.length > 0) {
                  ...
                  //added for #11550...
                  var _renderer = subLayerInfo.layerObject.renderer;
                  var _bypassDrawingInfo = false;
                  if (rootLayerInfo.layerObject && rootLayerInfo.layerObject.layerDrawingOptions) {
                    var ldo = rootLayerInfo.layerObject.layerDrawingOptions;
                    if (typeof (subLayerInfo.subId) !== 'undefined' && subLayerInfo.subId < ldo.length) {
                      if (ldo.hasOwnProperty(subLayerInfo.subId)) {
                        var layerLDO = ldo[subLayerInfo.subId];
                        if (layerLDO && layerLDO.renderer) {
                          _renderer = layerLDO.renderer;
                          _bypassDrawingInfo = true;
                        }
                      }
                    }
                  }
                  ...
                  //for issue #14978
                  // This test may not be completely necessary
                  //Only adding now as I am not completely familiar with the 'new object.constructor' approach
                  // Considering we have little time to fully test this approach will only be applied
                  //for the case that cloning of the infoTemplate fails.
                  //TODO investigate and reviedsdsw this pattern more fully at the next release
                  var infoTemp = subLayerInfo.getInfoTemplate();
                  var _infoTemp = infoTemp;
                  var canClone = true;
                  try {
                    lang.clone(infoTemp);
                  } catch (error) {
                    canClone = false;
                    if (infoTemp.constructor && infoTemp.toJson) {
                      _infoTemp = new infoTemp.constructor(infoTemp.toJson());
                    }
                  }
                  if (!canClone) {
                    _infoTemplates[subLayerInfo.id] = _infoTemp;
                  }
                  ...
                  options.push({
                    ...
                    renderer: _renderer,
                    rendererJson: _renderer && _renderer.toJson ? JSON.stringify(_renderer.toJson()) : false,
                    bypassDrawingInfo: _bypassDrawingInfo,
                    itemId: itemId,
                    infoTemplate: canClone ? _infoTemp : undefined,
                    lyrType: lyrType,
        ...
        //for issue #14978
        // add infoTemplate after clone when clone of infoTemplate fails
        //TODO investigate and review this pattern more fully at the next release
        array.forEach(Object.keys(_infoTemplates), lang.hitch(this, function (k) {
          layer_option_loop:
          for (var index = 0; index < this.layer_options.length; index++) {
            var opt = this.layer_options[index];
            if (opt.id === k) {
              opt.infoTemplate = _infoTemplates[k];
              break layer_option_loop;
            }
          }
        }));
                  ...
                  if (typeof (lIdx) !== 'undefined') {
                    if (resultInfo.geometryType) {
                      ...
                      if (typeof (resultInfo.drawingInfo) !== 'undefined') {
                        if (!this.layer_options[lIdx].bypassDrawingInfo) {
      ...
      _checkItemId: function (layerInfo, type) {
        ...
        if (type === "esri.layers.FeatureCollection") {
          ...
          itemId = itemId && itemId.itemId ? itemId.itemId : itemId;
      ...
      setConfig: function (config) {
        ...
        if (!this.config.mapID || (this.config.mapID && this.config.mapID === this.map.itemId)) {
          ...
          if (this.config.countEnabled) {
            this.panelCountOptions.setValue(this.config.countEnabled);
          }

          if (this.config.displayPanelIcon) {
            this.panelIconOptions.setValue(this.config.displayPanelIcon);
            this._updatePanelIcon();
          }
          if (this.config.expandList) {
            this.expandListOptions.setValue(this.config.expandList);
          }
          if(!this.config.hasOwnProperty("displayCluster")) {
            this.displayClusterLayerOptions.set('value', true);
          }
          else {
            this.displayClusterLayerOptions.set('value', this.config.displayClusterLayer);
          }

          var showAllFeatures = typeof (this.config.showAllFeatures) === 'undefined' ?
            false : this.config.showAllFeatures;
          this.showAllFeaturesOptions.setValue(showAllFeatures);
          ...
          this.hidePanelOptions.setValue(this.hidePanel);
          this.panelCountOptions.setStatus(!this.hidePanel);
      ...
      _updateRefresh: function (tr) {
        ...
        if (tr && tr.refreshBox) {
          ...
          if (lInfo && lInfo.lyrType === 'Feature Collection' && lInfo.itemId) {
      ...
      _pickSymbol: function (tr) {
        ...
        this.own(on(sourceDijit, "customSymbolButtonClicked", lang.hitch(this,
          function () {
            domClass.add(sourceDijit.btnOk, "jimu-state-disabled");
          })));
        this.own(on(sourceDijit, "onImageSelected", lang.hitch(this,
          function () {
            domClass.remove(sourceDijit.btnOk, "jimu-state-disabled");
          })));
        this.own(on(sourceDijit, "enableOkButton", lang.hitch(this,
          function () {
            domClass.remove(sourceDijit.btnOk, "jimu-state-disabled");
          })));
        this.own(on(sourceDijit, 'ok', lang.hitch(this, function (data) {
          if (domClass.contains(sourceDijit.btnOk, "jimu-state-disabled")) {
            return;
          }
      ...
      getConfig: function () {
        ...
        array.forEach(rows, lang.hitch(this, function (tr) {
          ...
          lInfo = {
            ...
            rendererJson: lo.rendererJson,
            bypassDrawingInfo: lo.bypassDrawingInfo
          };
        ...
        this.config.countEnabled = this.panelCountOptions.getValue();
        this.config.displayPanelIcon = this.panelIconOptions.getValue();
        this.config.hidePanel = this.hidePanelOptions.getValue();
        this.config.continuousRefreshEnabled = this.hidePanelOptions.getValue();
        this.config.expandList = this.expandListOptions.getValue();
        this.config.showAllFeatures = this.showAllFeaturesOptions.getValue();
        this.config.upgradeFields = false;
        this.config.displayClusterLayer = this.displayClusterLayerOptions.checked;
        return this.config;
      },
