...
function(declare, lang, domAttr, domStyle, on, JSON,
  ...
  Message, CheckBox, portalUrlUtils, symbolUtils, jimuUtils, BaseFeatureSetEditor, utils){
  return declare([BaseFeatureSetEditor, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    postCreate: function(){
      ...
      this.generalizeCheckbox = new CheckBox({
        checked: true,
        label: this.nls.generalize
      });

      this.generalizeCheckbox.placeAt(this.checkboxNode);
    },
    ...
    _onUpload: function(){
      ...
      this.fileInfo.innerHTML = jimuUtils.sanitizeHTML(fileName);
      ...
      var extent, resolution, numDecimals;
      if (this.generalizeCheckbox.getValue()) {
        // 1:40,000
        extent = scaleUtils.getExtentForScale(this.map, 40000);
        resolution = extent.getWidth() / this.map.width;
        ...
        // 1:400
        extent = scaleUtils.getExtentForScale(this.map, 400);
        resolution = extent.getWidth() / this.map.width;
        numDecimals = 0;
        while(resolution < 1) {
          resolution *= 10;
