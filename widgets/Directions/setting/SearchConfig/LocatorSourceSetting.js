  ...
  function(
    ...
    return declare([
      ...
      _setSourceItems: function() {
        ...
        this.searchDelay.set('value', config.searchDelay || 350);
        this.minCharacters.set('value', config.minCharacters || 3);

        if (config._oldConfig && !config._isUpgrade) {
          if (config._oldConfig.placeholder) {
            this.placeholder.set('value', jimuUtils.stripHTML(config._oldConfig.placeholder));
          }
          if(config._oldConfig.maxLocations){
            this.maxSuggestions.set('value', config._oldConfig.maxLocations);
          }
          if (config._oldConfig.autoComplete) {
            this.autoComplete.setValue(config._oldConfig.autoComplete);
          }
          if (config._oldConfig.searchDelay) {
            this.searchDelay.set('value', config._oldConfig.searchDelay);
          }
          if (config._oldConfig.minCharacters) {
            this.minCharacters.set('value', config._oldConfig.minCharacters);
          }

          config._isUpgrade = true;
        }
