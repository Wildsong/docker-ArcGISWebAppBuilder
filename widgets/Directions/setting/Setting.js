...
define([
  ...
  function (declare, lang, on, html, dojoConfig, Deferred, _WidgetsInTemplateMixin, BaseWidgetSetting,
    jimuUtils, portalUtils, PresetDestinations, SearchConfig, Message, searchUtils, BarrierLayers,
    TabContainer/*esriRequest, ,CheckBox*/) {
    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      postCreate: function () {
        ...
        this.barrierLayers = new BarrierLayers({
          nls: this.nls,
          map: this.map,
          barrierLayers: this.config.barrierLayers
        }, this.barrierLayersNode);
        this.barrierLayers.startup();
      ...
      startup: function () {
        ...
        this.tabContainer = new TabContainer({
          tabs: [ {
            title: this.nls.searchTabTitle,
            content: this.searchTabNode
          },{
            title: this.nls.routeTabTitle,
            content: this.routeTabNode
          }],
          isNested: true
        }, this.content);
        this.tabContainer.startup();
        this.own(on(this.tabContainer, 'tabChanged', lang.hitch(this, function(title) {
          if (title === this.nls.routeTabTitle) {
            this._setPresetDestinationsSources();//TODO this.shelter.show();
          } else {
        ...
        this.searchConfig = new SearchConfig({
          ...
          config: this.config//setConfig when init
        }, this.searchConfigNode);
        this.searchConfig.startup();
        this.own(on(this.searchConfig, 'isConfigValid', lang.hitch(this, function (res) {
          if ("true" === res) {
      ...
      destroy: function(){
        //close selector's dropDown first
        if (this.directionsLengthUnits && this.directionsLengthUnits.destroy) {
          this.directionsLengthUnits.closeDropDown();
          this.directionsLengthUnits.destroy();
        }
        if (this.barrierLayers && this.barrierLayers.destroy) {
          this.barrierLayers.destroy();
        }
      ...
      setConfig: function (config) {
        ...
        //   this.searchConfig.setConfig(this.config.searchOptions);
        // }

        if (this.config.barrierLayers) {
          this.barrierLayers.setValue(this.config.barrierLayers);
        }
      ...
      getConfig: function () {
        ...
        if (!this.barrierLayers.validate()) {
          this._showValidationErrorTip(this.barrierLayers);
          this._showParametersTip();
          return false;
        }
        ...
        if(false === this.searchConfig.validate()){
          return false;
        }

        var sc = this.searchConfig.getConfig();
        this.config = {
          ...
          barrierLayers: this.barrierLayers.getValue()
        };

        ///////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////
        searchUtils.version.upgradeConfig(this.config);
        ///////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////
        return this.config;
      },
      ...
      _setPresetDestinationsSources: function() {
        var sourcesConfig = this.searchConfig.getConfig();
        this.presetDestinations.setSources(sourcesConfig);
