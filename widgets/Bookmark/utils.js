...
define([
  ...
  function (lang, /*on,Deferred, */array, html, /*keys, */htmlEntities, jimuUtils, ItemNode,
    ...
    mo.findNextBookMarkByIdx = function (idx, bookmarks) {
      var nextBm = null;
      if ((idx + 1) <= (bookmarks.length - 1)) {
        nextBm = bookmarks[idx + 1];
      }
    ...
    mo.createBookMarkNode = function (bookmark, nls, folderUrl, display, a11y) {
      var thumbnail, node;
    ...
    mo.readBookmarksInWebmap = function (map) {
      ...
      array.forEach(map.itemInfo.itemData.bookmarks, function (bookmark) {
        ...
        copyedBookmark.name = htmlEntities.decode(copyedBookmark.name);
