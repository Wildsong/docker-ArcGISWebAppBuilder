  ...
  function (
    ...
    var clazz = dojoDeclare([jimuBaseWidget, dijitWidgetsInTemplateMixin], {
      ...
      postMixInProperties: function () {
        this.nls = lang.mixin(this.nls, window.jimuNls.coordinateControl);
      },

      destroy: function () {
        //clear everyting once current widget is destroyed
        if (this._visibilityControl) {
          this._visibilityControl.onClearBtnClicked();
        }
        this.inherited(arguments);
      },

      resize: function () {
        dojoTopic.publish("WIDGET_RESIZE", {});
      },

      startup: function () {
        this.inherited(arguments);
        if (this.config.portalUrl) {
          //for backward compatibility add default unit values in config object
          if (!this.config.hasOwnProperty("defaultObserverHeightUnit")) {
            this.config.defaultObserverHeightUnit = "meters";
          }
          if (!this.config.hasOwnProperty("defaultObservableDistanceUnit")) {
            this.config.defaultObservableDistanceUnit = "kilometers";
          }
          if (!this.config.hasOwnProperty("defaultAngleUnits")) {
            this.config.defaultAngleUnits = false;
          }
