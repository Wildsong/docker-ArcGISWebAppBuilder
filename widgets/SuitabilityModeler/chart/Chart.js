  ...
  var _def = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    destroy: function() {
      try {
        ...
        if (this._selGraphicsLayer) {
          this._selGraphicsLayer.clear();
          if (this.map) {
            this.map.removeLayer(this._selGraphicsLayer);
          }
        }
    ...
    _init: function() {
      ...
      this._selGraphicsLayer = new GraphicsLayer();
      this.map.addLayer(this._selGraphicsLayer);
    ...
    _reorderGraphicsLayer: function() {
      var idx;
      if (this.map && this._selGraphicsLayer) {
        idx = this.map.graphicsLayerIds.length;
        this.map.reorderLayer(this._selGraphicsLayer,idx);
      }
    ...
    _watchSubjectLayer: function(layer) {
      ...
      if (layer) {
        ...
        this._subjectLayerHandles.push(layer.on("selection-complete",function(result){
          ...
          if (layer.hasWebGLSurface()) {
            var features = result && result.features;
            if (features) {
              self._selGraphicsLayer.clear();
              array.forEach(features,function(graphic){
                self._selGraphicsLayer.add(graphic.clone());
              });
              self._selGraphicsLayer.setVisibility(true);
              self._reorderGraphicsLayer();
            }
          }
