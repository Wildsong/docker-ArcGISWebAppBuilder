...
function(on, sniff, query, Deferred, lang, html, array, all, declare, _WidgetsInTemplateMixin, esriLang,
  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    postCreate:function(){
      ...
      this.cbxShowFilterLabels.setLabel(this.nls.showFilterLabelTip);
      if(this.config){
      ...
      if (jimuUtils.has('ie') || jimuUtils.has('ff') || jimuUtils.has('edge')) {
        html.addClass(this.domNode, 'ie-ff');
      }
    },
    ...
    _createSingleSetting: function(target){
      ...
      this.singleSetting = new SingleSetting({
        ...
        _layerObject: target._layerObject,
    ...
    setConfig: function(config){
      ...
      //show filter label
      this.cbxShowFilterLabels.setValue(config.showFilterLabel);
      if (config.criteiraLabel) {
        this.criteiraTextBox.set("value", config.criteiraLabel);
      }
      if (config.spatialFilterLabel) {
        this.spatialTextBox.set("value", config.spatialFilterLabel);
      }
    ...
    getConfig: function(){
      ...
      var config = {
        ...
        labelTasks: jimuUtils.sanitizeHTML(this.labelTasksTextBox.get("value")),
        labelResults: jimuUtils.sanitizeHTML(this.labelResultsTextBox.get("value")),
        showFilterLabel:this.cbxShowFilterLabels.getValue(),
        criteiraLabel: jimuUtils.sanitizeHTML(this.criteiraTextBox.get("value")),
        spatialFilterLabel: jimuUtils.sanitizeHTML(this.spatialTextBox.get("value")),
        queries: []
      };
    ...
    //--for show filter labels
    _onShowFilterLabelCbxChanged:function(check){
      if(check){
        this._showFilterLabelOption();
      }else{
        this._hideFilterLabelOption();
      }
    },

    _showFilterLabelOption:function(){
      html.removeClass(this.filterLabelOptionContainer,'hide');
    },

    _hideFilterLabelOption:function(){
      html.addClass(this.filterLabelOptionContainer,'hide');
    }
