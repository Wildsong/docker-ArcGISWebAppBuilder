...
define([
  ...
  function(lang, array, Deferred, jimuUtils, portalUrlUtils, LayerInfos, esriRequest, esriNs,
    PictureMarkerSymbol, symbolJsonUtils) {
    var mo = {
      ...
      dynamicUpdateConfigIcon: function(config, defaultIcon) {
        var proHost = window.location.protocol + "//" + window.location.hostname;
        defaultIcon = proHost + defaultIcon;
        if (config.icon && typeof config.icon === 'object') {
          config.icon = symbolJsonUtils.fromJson(config.icon);
        } else if (typeof config.icon === 'string') {
          var icon = config.icon ? config.icon : defaultIcon;
          var symbolIcon = new PictureMarkerSymbol({
            "yoffset": 16,
          ...
          config.icon = symbolIcon;
        }
      },

      tryGetLayerObject: function(layerId) {
