...
define([
  ...
  grg.createGRG = function (HorizontalCells, VerticalCells, centerPoint,
      ...
      labelDirection, gridStyle, gridOrigin, geodesic, map, projection) {
        ...
        for (var j = 1; j <= HorizontalCells; j++) {
          if (geodesic) {
            ...
            */
            ...
            if (map.spatialReference.wkid && map.spatialReference.wkid !== 4326) {
              polygon = (projection.isLoaded()) ?
                projection.project(polygon, map.spatialReference) :
            ...
            if (gridStyle === "default") {
              ...
              startX = (hexHorizontalCells === HorizontalCells) ?
                startX + ((j - 1) * (cellWidth)) + (cellWidth / 2) :
                startX + ((j - 1) * (cellWidth));
              startY = (startY + radius) + ((i - 1) * (radius * 1.5));
              var hexagonCenter = new Point([startX, startY], new SpatialReference({
                wkid: 102100
              }));
              var hexagon = new EsriCircle(hexagonCenter, {
                radius: radius,
                numberOfPoints: 6
              });
            ...
            //project the geometry to the same spatial reference as the map
            if (map.spatialReference.wkid && map.spatialReference.wkid !== 102100) {
              polygonRotated = (projection.isLoaded()) ?
                projection.project(polygonRotated, map.spatialReference) :
                webMercatorUtils.webMercatorToGeographic(polygonRotated);
            }
    ...
    grg.addGraphicLabel = function (graphic, labelType, number, letterIndex, secondLetterIndex) {
      ...
      var secondLetter = grg.convertNumberToLetters(secondLetterIndex);
      var attr = {};
      switch (labelType) {
        case "alphaNumeric":
